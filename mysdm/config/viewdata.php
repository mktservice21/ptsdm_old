        ...
        <div class='modal-content'>
            ...
            <div class='modal-body'>
                ...
                if ($_GET['module']=="viewkaryawan"){
                    ...
                    echo "<tbody class='gridview-error'>";
                    $no=1;
                    $tampil = mysqli_query($cnit, "SELECT DISTINCT karyawanId, nama FROM dbmaster.karyawan where 1=1 $kdjbt order by nama, karyawanId");
                    while ($r=mysqli_fetch_array($tampil)){
                        echo "<tr scope='row'><td>$no</td>";
                        echo "<td><a data-dismiss='modal' href='#' onClick=\"getDataModalKaryawan('$_POST[udata1]', '$_POST[udata2]', '$r[karyawanId]', '$r[nama]')\">
                            $r[karyawanId]</a></td>";
                    ...
                    if (($icabangid=='30') or ($icabangid=='31') or ($icabangid=='0000000032')) {
                            $query = "select karyawanId,nama,jabatanid,icabangid from dbmaster.karyawan where (karyawanId='0000000154' or karyawanId='0000000159') AND aktif = 'Y' order by nama"; 
                    } else {
                            $query = "select karyawanId,nama,jabatanid,icabangid from dbmaster.karyawan where icabangid='$icabangid' AND aktif = 'Y' order by nama"; 
                    }
                    ...
                    echo "<tbody class='gridview-error'>";
                    $no=1;
                    $tampil = mysqli_query($cnit, "SELECT DISTINCT divprodid, kodeid, nama FROM dbbudget.v_akun_budget order by divprodid, nama, kodeid");
                    while ($r=mysqli_fetch_array($tampil)){
                        echo "<tr scope='row'><td>$no</td>";
                    ...
                    echo "<tbody class='gridview-error'>";
                    $no=1;
                    $tampil = mysqli_query($cnit, "SELECT DISTINCT divprodid, kodeid, nama FROM dbbudget.v_akun_budget where divprodid='$_POST[udivprod]' order by divprodid, nama, kodeid");
                    while ($r=mysqli_fetch_array($tampil)){
                        echo "<tr scope='row'><td>$no</td>";
                        echo "<td>$r[divprodid]</td>";
                        echo "<td><a data-dismiss='modal' href='#' onClick=\"getDataModalBgAkun('$_POST[udata1]', '$_POST[udata2]', '$r[kodeid]', '$r[nama]')\">
                    ...
                    echo "<tbody class='gridview-error'>";
                    $no=1;
                    $tampil = mysqli_query($cnit, "SELECT distinct iCabangId, nama from dbmaster.icabang where aktif='Y'");
                    while ($r=mysqli_fetch_array($tampil)){
                        echo "<tr scope='row'><td>$no</td>";
                    ...
                    echo "<tbody class='gridview-error'>";
                    $no=1;
                    $tampil = mysqli_query($cnit, "SELECT distinct dokterId, nama from dbmaster.dokter");
                    while ($r=mysqli_fetch_array($tampil)){
                        echo "<tr scope='row'><td>$no</td>";
                    ...
                    $query = "select iCabangId from dbmaster.karyawan where iCabangId='$icabangid'"; 
                    ...
                    if ($icabangid=="0000000001") {
                        ...
                        $query = "select dokter.dokterId, CONCAT(dokter.nama,' - ',dokter.dokterId) AS nama 
                                          FROM dbmaster.mr_dokt as mr_dokt 
                                          join dbmaster.karyawan as karyawan on mr_dokt.karyawanId=karyawan.karyawanId
                                          join dbmaster.dokter as dokter on mr_dokt.dokterId=dokter.dokterId
                    ...
                    echo "<tbody class='gridview-error'>";
                    $no=1;
                    $tampil = mysqli_query($cnit, "SELECT distinct subpost, nmsubpost from dbmaster.brkd_otc where ifnull(subpost,'') <> ''");
                    while ($r=mysqli_fetch_array($tampil)){
                        echo "<tr scope='row'><td>$no</td>";
                    ...
                    $tampil = mysqli_query($cnit, "SELECT distinct kodeid, nama from dbmaster.brkd_otc where ifnull(subpost,'') = '$_POST[kodesub]'");
                    while ($r=mysqli_fetch_array($tampil)){
                    ...
                    echo "<tbody class='gridview-error'>";
                    $no=1;
                    $tampil = mysqli_query($cnit, "SELECT distinct Distid, nama, alamat1 from dbmaster.distrib0");
                    while ($r=mysqli_fetch_array($tampil)){
                        echo "<tr scope='row'><td>$no</td>";
                        echo "<td><a data-dismiss='modal' href='#' "
                    ...
                    echo "<tbody class='gridview-error'>";
                    $no=1;
                    $tampil = mysqli_query($cnit, "select areaId, Nama, aktif from dbmaster.iarea where iCabangId='$_POST[ucabang]'");
                    while ($r=mysqli_fetch_array($tampil)){
                        echo "<tr scope='row'><td>$no</td>";
                        echo "<td><a data-dismiss='modal' href='#' "
                    ...
                    $tampil = mysqli_query($cnit, "SELECT c1.*, c2.*, c3.*, c4.*, c5.* FROM dbmaster.coa as c5 "
                    ...
                    $no=1;
                    //$tampil = mysqli_query($cnit, "SELECT COA4, COA_KODE, COA_NAMA FROM dbmaster.coa order by COA_KODE");
                    while ($r=mysqli_fetch_array($tampil)){
                        echo "<tr scope='row'><td>$no</td>";
                        echo "<td>$r[COA4]</td>";
