...
if ($_GET['module']=="viewdataareakaryawanbylevel"){
    ...
    if (trim($pdivisi)=="OTC") {
        ...
        if (empty($psel)) {
            $tmpl = mysqli_query($cnit, "select CONCAT(iCabangId,areaId) as lcfields from dbmaster.t_karyawan_posisi WHERE karyawanId='$karyawan'");
            $rt=mysqli_fetch_array($tmpl);
            $cabareaid = $rt['lcfields'];
            $psel = $cabareaid;
        }
    ...
    $sql=mysqli_query($cnit, "SELECT distinct iCabangId, nama from dbmaster.icabang $filter order by nama");
    while ($Xt=mysqli_fetch_array($sql)){
    ...
    $sql=mysqli_query($cnit, "select distinct kodeid, nama, subpost from dbmaster.brkd_otc $filt order by kodeid");
    while ($Xt=mysqli_fetch_array($sql)){
