...
if ($pilihdarims==true) {
    ...
    $query = "select * from sls.ytd where iddaerah NOT IN ('026') AND bulan='$bulan' $filterregion $filterdivpprod_exp";
    ...
    $date=date_create($bulan);
    $region=$_GET["region"];
    if ($region=="B"){ $namaregion="Barat";} else if ($region=="T"){$namaregion="Timur";} else { $namaregion=""; }
    $karyawanid=$_GET["id"];
    $namatabel="tmp.ytd_pm".$karyawanid;
    $link =$_GET["link"]."/report/ytd_pm.aspx";
    $mylink="../ptsdm/mysdm/";
    ...
    $now=date("mdYhis");
    $tmp00 =" dbtemp.TMPYTDDM00_".$karyawanid."_$now ";
...
<table id="datatable" class="display nowrap table table-striped table-bordered" style="width:100%">
    ...
    <tbody>
		...
		*/	  
            ...
            $pregnm=$namaregion;
            if ($namaregion=="All" AND !empty($pprodukmaklo)) $pregnm="";
            ...
            foreach ($result41 as $r41){
                echo "<tr>";
                echo "<td colspan='2'><b>Total $pregnm<b></td><td class='divnone'></td>";
            ...
            foreach ($result8 as $r8){
                echo "<tr>";
                echo "<td colspan='2'><b>Total $pregnm + OTHER<b></td><td class='divnone'></td>";
            ...
            if (!empty($pprodukmaklo)) {
                ...
                foreach ($result42 as $r42){
                    echo "<tr>";
                    echo "<td colspan='2'><b>Total $pregnm + MAKLON<b></td><td class='divnone'></td>";
