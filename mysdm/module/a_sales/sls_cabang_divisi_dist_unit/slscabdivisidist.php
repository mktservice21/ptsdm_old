...
<?php
    ...
    $blnthnfilt=date_format($date,"Y-m");

    if ($region=="B"){ $namaregion="Barat";} else if ($region=="T"){$namaregion="Timur";}
    if ($region=="B"){ $nmreg="R1";} else if ($region=="T"){$nmreg="R2";}
    ...
    echo "<table width='90%' align='center' border='0' class='table table-striped table-bordered'>";
        echo "<tr>";
            echo "<td valign='top'>";
                ...
                echo "<tr><td><b>SALES <u>$nminitialdist</u> PRODUK & DIVISI BY UNIT</b></td></tr>";
    ...
    $tmp4 ="DTSAESCABDIVDISTVDISTUNT04".$karyawanid."_$now$milliseconds";
    ...
    $query = "select date_format(s.bulan,'%Y%m') thnbln, d.initial, s.iProdId iprodid, s.divprodid, s.hna, SUM(s.qty) qty, SUM(s.value)/1000 as tvalue 
        from sls.ytd_dist s
    ...
    $query = "select date_format(s.bulan,'%Y%m') thnbln, s.divprodid, s.iProdId iprodid, s.hna_sales,
        sum(mtd_qty_sales) sqty1, sum(mtd_value_sales)/1000 stvalue1,
        sum(mtd_qty_target) tqty, sum(mtd_value_target)/1000 ttvalue,
        sum(mtd_qty_thnlalu) sqty2, sum(mtd_value_thnlalu)/1000 stvalue2
        from sls.ytd as s
        where date_format(s.bulan,'%Y%m') = '$filblnprod1'
        and s.region='$region'
        GROUP BY date_format(s.bulan,'%Y%m'), s.divprodid, s.iddaerah, s.iProdId, s.hna_sales";
    ...
    //UPDATE KATEGORI
    $query="select distinct iprodid, divprodid, kategori from sls.ytdprod a WHERE left(a.bulan,7)='$blnthnfilt' ";
    //where DATE_FORMAT(a.bulan,'%Y%m')=(select Max(DATE_FORMAT(b.bulan,'%Y%m')) from sls.ytdprod b where a.iprodid=b.iprodid)
    $results1 = DB::query("create temporary table $tmp4($query)");
    
    $query="update $tmp0 as a set a.kategori=ifnull((select distinct b.kategori from $tmp4 as b where a.iprodid=b.iprodid and a.divprodid=b.divprodid),0)";
    $results1 = DB::query($query);
    $results1 = DB::query("update $tmp0 as a set a.kategori='EXISTING' where ifnull(a.kategori,'')=''");
    
    $results1 = DB::query("drop temporary table $tmp4");
    
    
    //hna sesuai sales atau target bulan yang dipilih
    $results1 = DB::query("CREATE temporary table $tmp4 select distinct divprodid, iprodid, hna from $tmp1");
    $results1 = DB::query("INSERT INTO $tmp4 select distinct divprodid, iprodid, hna_sales from $tmp2 "
            . " WHERE CONCAT(divprodid,iprodid) NOT IN (select distinct CONCAT(divprodid,iprodid) from $tmp1)");
    $results1 = DB::query("UPDATE $tmp0 SET hna=IFNULL((SELECT hna FROM $tmp4 WHERE $tmp4.iprodid=$tmp0.iprodid AND $tmp4.divprodid=$tmp0.divprodid order by $tmp4.hna LIMIT 1),$tmp0.hna)");
    $results1 = DB::query("drop temporary table $tmp4");
    ...
    //sales skrang
    $query="update $tmp0 set QTOT1=ifnull((select sum(qty) from $tmp1 where $tmp0.iprodid=$tmp1.iprodid and $tmp0.divprodid=$tmp1.divprodid and $tmp1.thnbln='$filblnprod1'),0)";
    $results1 = DB::query($query);
    //sales lalu
    $query="update $tmp0 set QTOT2=ifnull((select sum(qty) from $tmp1 where $tmp0.iprodid=$tmp1.iprodid and $tmp0.divprodid=$tmp1.divprodid and $tmp1.thnbln='$filblnprod2'),0)";
    $results1 = DB::query($query);
    //target
    ...
    //VALUE
    //sales skrang
    $query="update $tmp0 set TOT1=ifnull((select sum(tvalue) from $tmp1 where $tmp0.iprodid=$tmp1.iprodid and $tmp0.divprodid=$tmp1.divprodid and $tmp1.thnbln='$filblnprod1'),0)";
    $results1 = DB::query($query);
    //sales lalu
    $query="update $tmp0 set TOT2=ifnull((select sum(tvalue) from $tmp1 where $tmp0.iprodid=$tmp1.iprodid and $tmp0.divprodid=$tmp1.divprodid and $tmp1.thnbln='$filblnprod2'),0)";
    $results1 = DB::query($query);
    //target
...
<table id="datatable1" class="display  table table-striped table-bordered" style="width:100%" border='0px'>
    <thead>
        <tr style="background-color: #cccfff;">
            ...
            <th><?PHP echo "Sales $nmreg $thnbln"; ?></th>
            <th>Ctr %</th>
            <th><?PHP echo "Sales $nminitialdist $thnbln2"; ?></th>
            <th>Growth %</th>
        </tr>
    </thead>
    <tbody>
        ...
        foreach ($results1 as $r1) {
            ...
            foreach ($resultsk as $rk) {
                ...
                if ($divisi=="PEACOX") {
                    ...
                    $querytot = "select sum(TOT1) TOT1, sum(TOTtarget) TOTtarget, sum(TOT2) TOT2 from $tmp0 where divprodid='$divisi' and kategori='$kategori'";
