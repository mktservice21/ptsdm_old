...
<?php
    ...
    $now=date("mdYhis");
    $tmp0 ="DTSAESCABDIVDISTVDISTUYT00".$karyawanid."_$now$milliseconds";
    $tmp1 ="DTSAESCABDIVDISTVDISTUYT01".$karyawanid."_$now$milliseconds";
    $tmp2 ="DTSAESCABDIVDISTVDISTUYT02".$karyawanid."_$now$milliseconds";
    $tmp3 ="DTSAESCABDIVDISTVDISTUYT03".$karyawanid."_$now$milliseconds";
    $tmp4 ="DTSAESCABDIVDISTVDISTUYT04".$karyawanid."_$now$milliseconds";
    ...
    $query = "select date_format(s.bulan,'%Y%m') thnbln, d.initial, s.iProdId iprodid, s.divprodid, 
        s.hna, SUM(s.qty) qty, SUM(s.value)/1000 as tvalue 
        from sls.ytd_dist s
    ...
    $query = "select date_format(s.bulan,'%Y%m') thnbln, s.divprodid, s.iProdId iprodid, s.hna_sales, 
        sum(ytd_qty_sales) sqty1, sum(ytd_value_sales)/1000 stvalue1,
        sum(ytd_qty_target) tqty, sum(ytd_value_target)/1000 ttvalue,
        sum(ytd_qty_thnlalu) sqty2, sum(ytd_value_thnlalu)/1000 stvalue2
        from sls.ytd as s
    ...
    //UPDATE KATEGORU 
    $query="select distinct iprodid, divprodid, kategori from sls.ytdprod a where DATE_FORMAT(a.bulan,'%Y%m')=(select Max(DATE_FORMAT(b.bulan,'%Y%m')) 
            from sls.ytdprod b where a.iprodid=b.iprodid)";
...
<table id="datatable1" class="display  table table-striped table-bordered" style="width:100%" border='0px'>
    <thead>
        <tr style="background-color: #cccfff;">
            ...
            <th>Produk</th>
            <th>HNA</th>
            <th><?PHP echo "Sales YTD $nminitialdist $thnbln"; ?></th>
            <th><?PHP echo "Sales YTD $nmreg $thnbln"; ?></th>
            <th>Ctr %</th>
            <th><?PHP echo "Sales YTD $nminitialdist $thnbln2"; ?></th>
            <th>Growth %</th>
        </tr>
    </thead>
    <tbody>
...
<?PHP
    $results1 = DB::query("drop temporary table $tmp0");
    $results1 = DB::query("drop temporary table $tmp1");
    $results1 = DB::query("drop temporary table $tmp2");
?>