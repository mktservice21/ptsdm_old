...
<?php
    ...
    $query="select s.bulan, s.icabangid, s.areaid, a.nama as nama_area, s.divprodid, s.iprodid, sum(s.qty_sales) as qty, sum(s.qty_sales*s.hna_sales) as tvalue from $tmp5 as s
        join sls.iarea as a on s.areaid=a.areaid and s.icabangid=a.icabangid
        where a.aktif='Y'
        and CONCAT(s.icabangid,s.areaid,s.divprodid) in (select distinct CONCAT(sp.icabangid,sp.areaid,sp.divisiid) from sls.ispv0 as sp  
    ...
    $query = "select t1.tgl, t1.icabangid, t1.areaId, t1.divprodid, t1.iprodid, sum(t1.target) as qty, sum(t1.hna*t1.target) as tvalue from $tmp6 as t1  
            ...
            join sls.iarea as a on sp.areaid=a.areaid where sp.karyawanid='$idspv' and a.aktif='Y')
            GROUP BY t1.tgl, t1.icabangid, t1.areaId, t1.divprodid, t1.iprodid";
    ...
    $query="UPDATE $tmp3 set ytd_ach=ifnull(case when ifnull(ytd_trg,0)=0 then 0 else (ytd_sls/ytd_trg*100) end,0)";
    $results1 = DB::query($query);
