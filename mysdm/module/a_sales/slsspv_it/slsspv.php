...
<?php
    ...
    $query="select s.icabangid, s.areaid, a.nama as nama_area, s.divprodid, s.iprodid, sum(s.qty_sales) as qty, sum(s.qty_sales*s.hna_sales) as tvalue from dbmaster.sales as s
        ...
        where bulan between '$tgl_pertama' AND '$tgl_terakhir' and a.aktif='Y'
    ...
    $query = "select t0.icabangid, t0.areaId, t0.divprodid, t1.iprodid, sum(t1.target) as qty, sum(t1.hna*t1.target) as tvalue from dbmaster.t_tgtarea1 as t1 join dbmaster.t_tgtarea0 as t0 
            on t1.targetId=t0.targetId
            where tgl between '$tgl_pertama' AND '$tgl_terakhir' 
    ...
    $query = "insert into $tmp4 (icabangid, areaid, nama_area, divprodid, iprodid, nama)
            select distinct a.icabangid, a.areaid, i.nama, p.divprodid, p.iprodid, p.nama from $tmp2 as a Join MKT.iproduk as p on p.iprodid=a.iprodid 
            join MKT.iarea as i on a.areaid=i.areaid and a.icabangid=i.icabangid
        where p.divprodid in (select distinct divisiid from MKT.ispv0 where karyawanid='$idspv')
    ...
    $query = "select distinct icabangid, areaid, nama_area, divprodid, iprodid, nama, SLS, TRG, ACH, sqty, tqty from $tmp4";
...
<?PHP
    $results1 = DB::query("drop TEMPORARY table $tmp3");
?>
