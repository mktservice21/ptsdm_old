...
<?PHP
    ...
    if (isset($_POST['chkbox_rpt15'])) $ppilihproskascab=$_POST['chkbox_rpt15'];
...
<BODY>
    ...
    if ($padaygdisimpan==true) {
        $query="DELETE FROM dbmaster.t_proses_data_bm_date WHERE tahun='$ptahun'";
        mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto queryfunction; }
        $query="INSERT INTO dbmaster.t_proses_data_bm_date (tahun)VALUES('$ptahun')";
        mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto queryfunction; }
