<?php
    ...
    */
    ...
    }elseif ($nmdb=="tesdatamodule") {
        if ($namatabel=="5") {
            mysqli_query($cnmy, "UPDATE dbmaster.t_suratdana_br SET stsnonaktif='Y' WHERE idinput in ('1478', '1480')");
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { $namatabel= $erropesan;}
        }elseif ($namatabel=="6") {
            mysqli_query($cnmy, "UPDATE dbmaster.t_suratdana_br SET stsnonaktif='Y' WHERE idinput in ('1507', '1508')");
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { $namatabel= $erropesan;}
        }elseif ($namatabel=="7") {
            mysqli_query($cnmy, "UPDATE dbmaster.t_suratdana_br SET stsnonaktif='N' WHERE idinput in ('1478', '1480')");
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { $namatabel= $erropesan;}
        }elseif ($namatabel=="8") {
            mysqli_query($cnmy, "UPDATE dbmaster.t_suratdana_br SET stsnonaktif='N' WHERE idinput in ('1507', '1508')");
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { $namatabel= $erropesan;}
        }
        header('location:../../../media.php?module='.$module.'&idmenu='.$idmenu.'&act=none&nmtbl='.$namatabel);
    }
