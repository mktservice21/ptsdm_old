<?php
    ...
    $pfilterkrypilih="";
    if ($pidjabatan=="38") {
        if (empty($pkaryawanid_pl) AND !empty($plstkry_pl)) {
            
            $pkrylst = explode(",", $plstkry_pl);
            foreach($pkrylst as $pidkr) {
                $pidkr = trim($pidkr);
                
                $pfilterkrypilih .="'".$pidkr."',";
            }
    ...
    $now=date("mdYhis");
    $tmp01 =" dbtemp.tmprtnadcblst01_".$puserid."_$now ";
    $tmp02 =" dbtemp.tmprtnadcblst02_".$puserid."_$now ";
    $tmp03 =" dbtemp.tmprtnadcblst03_".$puserid."_$now ";
    $tmp04 =" dbtemp.tmprtnadcblst04_".$puserid."_$now ";
    
    
    $query = "SELECT userid, idrutin, tgl, bulan, periode1, periode2, "
            . " divisi, karyawanid, icabangid, areaid, jumlah, keterangan, "
            . " jabatanid, atasan1, tgl_atasan1, atasan2, tgl_atasan2, atasan3, tgl_atasan3, atasan4, tgl_atasan4, validate, fin, tgl_fin, nama_karyawan ";
    ...
    if (empty($pkaryawanid_pl) OR $pkaryawanid_pl=="ALL") {
        if (empty($pfilterkrypilih)) $pfilterkrypilih="('$pidcard')";
        
        $query .=" AND karyawanid IN $pfilterkrypilih ";
    }else{
        $query .=" AND karyawanid='$pkaryawanid_pl' ";
    }
        ...
        <table id='datatablertnotcho' class='table table-striped table-bordered' width='100%'>
            <thead>
                <tr>
                    ...
                    <th width='30px'>Tgl. DM</th>
                    <th width='30px'>Tgl. SM</th>
                    <th width='30px'>Tgl. GSM</th>
            ...
            <tbody>
                ...
                while ($row= mysqli_fetch_array($tampil)) {
                    ...
                    if ($t_ats1=="0000-00-00 00:00:00" OR $t_ats1=="0000-00-00") $t_ats4="";
                    if ($t_ats2=="0000-00-00 00:00:00" OR $t_ats2=="0000-00-00") $t_ats4="";
                    if ($t_ats3=="0000-00-00 00:00:00" OR $t_ats3=="0000-00-00") $t_ats4="";
                    if ($t_ats4=="0000-00-00 00:00:00" OR $t_ats4=="0000-00-00") $t_ats4="";
                    
                    if ($patasan1==$pkaryawanid) {
                        $patasan1="";
                        $t_ats1="";
                    }
                    
                    if ($patasan2==$pkaryawanid) {
                        $patasan2="";
                        $t_ats2="";
                    }
                    
                    if ($patasan3==$pkaryawanid) {
                        $patasan3="";
                        $t_ats3="";
                    }
                    
                    if ($patasan4==$pkaryawanid) {
                        $patasan4="";
                        $t_ats4="";
                    }
                    
                    
                    if (!empty($ptglfin)) $ptglfin=date("d/m/Y H:i:s", strtotime($ptglfin));
                    if (!empty($t_ats1)) $t_ats1=date("d/m/Y H:i:s", strtotime($t_ats1));
                    if (!empty($t_ats2)) $t_ats2=date("d/m/Y H:i:s", strtotime($t_ats2));
                    if (!empty($t_ats3)) $t_ats3=date("d/m/Y H:i:s", strtotime($t_ats3));
                    if (!empty($t_ats4)) $t_ats4=date("d/m/Y H:i:s", strtotime($t_ats4));
                    ...
                    $phapus="<input type='button' class='btn btn-danger btn-xs' value='Hapus' onClick=\"ProsesDataHapus('hapus', '$pidno')\">";
                    ...
                    $pnmuploadbukti="Upload";
                    if ($pidcard<>$pkaryawanid) {
                        $phapus="";
                        $pedit="";
                        $pttdedit = "";
                        $pnmuploadbukti="Lihat";
                    }
                    ...
                    $upload="<a class='$warna' href='?module=$pmodule&act=uploaddok&idmenu=$pidmenu&nmun=$pidmenu&id=$pidno'>$pnmuploadbukti</a>";
                    ...
                    echo "<td nowrap>$t_ats2</td>";
                    echo "<td nowrap>$t_ats3</td>";
                    echo "<td nowrap>$t_ats4</td>";
                    echo "<td nowrap>$ptglfin</td>";
