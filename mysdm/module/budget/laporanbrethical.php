<?PHP
    ...
    if ($ppilihrpt=="excel") {
        ...
        if ($mact=="viewrptbrpcmeth")
            header("Content-Disposition: attachment; filename=PD_PC_M_BR_ETHICAL_$now_.xls");
        elseif ($mact=="viewrptbrklaimeth")
            header("Content-Disposition: attachment; filename=PD_KLAIM_BR_ETHICAL_$now_.xls");
        else
            header("Content-Disposition: attachment; filename=PD_CASH_ADVANCE_BR_$now_.xls");
    }
...
<HEAD>
    <?PHP 
        echo "<title>PERMINTAAN DANA BUDGET REQUERST - BR</title>";
...
<BODY>
    ...
    $query = "UPDATE $tmp01 as a JOIN dbmaster.t_kode_spd_pengajuan as b on a.jenis_rpt=b.jenis_rpt AND a.subkode=b.subkode SET "
            . " a.nama_pengajuan=b.nama_pengajuan, a.nama_report=b.nama_report, a.nama_ket=b.nama_ket";
    mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    $query_data = "select a.divprodid as divisi, a.COA4 as coa4, a.brId as brid, "
            . " a.karyawanid, c.nama as nama_karyawan, a.dokterId as dokterid, b.nama as nama_dokter, "
            . " a.jumlah, a.realisasi1, a.realisasi2, a.noslip, "
            . " a.tgl, a.tgltrans, a.tglrpsby, "
            . " a.aktivitas1, a.aktivitas2, a.batal, a.retur ";
    
    $query = $query_data." FROM hrd.br0 as a LEFT JOIN hrd.dokter b on a.dokterId=b.dokterId "
            . " LEFT JOIN hrd.karyawan as c on a.karyawanid=c.karyawanId "
            . " WHERE brId IN (select DISTINCT IFNULL(bridinput,'') bridinput FROM $tmp02)";
    $query = "create TEMPORARY table $tmp03 ($query)"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    
    $query_reject = $query_data." FROM dbmaster.backup_br0 as a LEFT JOIN hrd.dokter b on a.dokterId=b.dokterId "
                . " LEFT JOIN hrd.karyawan as c on a.karyawanid=c.karyawanId "
                . " WHERE brId IN (select DISTINCT IFNULL(bridinput,'') bridinput FROM $tmp02)";
    ...
    $query = "ALTER table $tmp03 ADD COLUMN nama_cabang varchar(150), ADD COLUMN nomorid varchar(20)";
    mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
    $query = "UPDATE $tmp03 as a JOIN hrd.br0_reject as b on a.brid=b.brId SET a.batal='Y'";
    mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    $pwewenang1=false;
    $query = "select karyawanid, subkode, divisi from dbmaster.t_kode_spd_wenang1 WHERE karyawanid='$pkaryawanidspd'";
    ...
    $pwewenang2=false;
    $query = "select karyawanid, subkode, divisi from dbmaster.t_kode_spd_wenang2 WHERE karyawanid='$pkaryawanidspd'";
    ...
    $pwewenang3=false;
    $query = "select karyawanid, subkode, divisi from dbmaster.t_kode_spd_wenang3 WHERE karyawanid='$pkaryawanidspd'";
    ...
    //echo "$papv_kar1, $papv_kar2, $pkaryawanidspd";
    
    
    $pstatusapprovespv=false;
    if ($pwewenang1==$pwewenang2) $pstatusapprovespv=true;
        ...
        <div id="isikiri">
            <table class='tjudul' width='100%'>
                <?PHP
                    ...
                    echo "<tr><td width='250px' nowrap><b>Budget Request Team $pdivisi : </b></td><td>$pnodivisi</td></tr>";
            ...
            <?PHP
                ...
                while ($row1= mysqli_fetch_array($tampil1)) {
                    ...
                    while ($row2= mysqli_fetch_array($tampil2)) {
                        ...
                        if (empty($purutan)) $purutan=$npilih_urutan;
                        
                        $padasubkode=true;
                        if ((INT)$purutan==(INT)$npilih_urutan OR (INT)$purutan==0) $padasubkode=false;
                    ...
                    if ((INT)$njml<=1) $padasubkode=false;
                    ...
                    $plewaturutan=false;
                    if ($purutanlebih == true AND $padasubkode==true) {
    ...
    if ($psudahadaadj == true){
        ...
        //echo "<br/>$pjumlahtranferspd<br/>$ptotalrincian";
        if ($pjumlahtranferspd==$ptotalrincian){
        }else{
    ...
    if ($ppilihrpt=="excel") {
        ...
        echo "<table class='tjudul' width='100%'>";
            echo "<tr>";
                ...
                if ($pstatusapprovespv==true) echo "Yang Membuat,";
                else echo "Checker,";
