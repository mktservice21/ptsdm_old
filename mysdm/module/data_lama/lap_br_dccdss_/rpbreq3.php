	  ...
	  exit;
		...
		if ($records) {
			...
			$query = "select areaid from MKT.ispv where icabangid='$icabangid_' and aktif='Y' 
					  and areaid not in (select areaid from hrd.br_area where cabangid='$cabangid' order by areaid)
					  group by areaid;";
			//echo "$query<br>";
			$result = mysqli_query($cnit, $query);
			$records = mysqli_num_rows($result);
		...
		*/
		...
		$query_cab = "SELECT nama FROM hrd.br_area WHERE cabangid='$cabangid'"; 
		//echo"$query_cab <br>";
		...
		*/
		...
		$query1 = "SELECT DISTINCT icabangid FROM hrd.br_area WHERE cabangid='$cabangid'";
		//echo"$query1<br>";
		...
		while ($j < $num_results1) {
			...
			$query_str = "SELECT * FROM hrd.br_area WHERE cabangid='$cabangid' AND icabangid='$icabangid' ORDER BY icabangid"; 
		...
		while ($i <= $records) {		
		   ...
		   while ( ($i<=$records) and ($bln_ == substr($row['tgltrans'],0,7)) ) {
			...
			$query_ar = "select br_area.nama
						from hrd.br0 br0  
						join hrd.br_area on br0.icabangid=br_area.icabangid and br0.areaid=br_area.areaid 
