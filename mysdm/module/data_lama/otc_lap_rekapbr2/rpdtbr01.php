...
<?php
	...
	$header6_ = add_space('Nama Realisasi',60);
	...
	$header11_ = add_space('Sub-Posting',30);
        ...
        $query = "select distinct icabangid_o from hrd.br_otc where ".$where_." AND IFNULL(batal,'')<>'Y' order by icabangid_o"; 
        //echo"$query";exit;
        $resultc = mysqli_query($cnit, $query);
        $num_rstcab = mysqli_num_rows($resultc);
        for ($c=0; $c < $num_rstcab; $c++) {
            $rwc = mysqli_fetch_array($resultc);
            $cicabangid_o = $rwc['icabangid_o'];
            $cnamacab="";
            
                    if ($cicabangid_o=='MD') {
                            $cnamacab = 'MD';	
                    }elseif ($cicabangid_o=='PM_CARMED') {
                            $cnamacab = 'PM - CARMED';
                    }elseif ($cicabangid_o=='PM_LANORE') {
                            $cnamacab = 'PM - LANORE';
                    }elseif ($cicabangid_o=='PM_MELANOX') {
                            $cnamacab = 'PM - MELANOX';
                    }elseif ($cicabangid_o=='PM_ACNEMED') {
                            $cnamacab = 'PM - ACNE MED';
                    }elseif ($cicabangid_o=='PM_PARASOL') {
                            $cnamacab = 'PM - PARASOL';
                    }elseif ($cicabangid_o=='JKT_MT') {
                            $cnamacab = 'JAKARTA - MT';
                    }elseif ($cicabangid_o=='JKT_RETAIL') {
                            $cnamacab = 'JAKARTA - RETAIL';
                    }else{
                            if ($cicabangid_o=='HO') {
                                    $cnamacab = 'HO';	
                            } else {
                                    $query_cab = "select nama from MKT.icabang_o where icabangid_o='$cicabangid_o'";  
                                    // echo"$query_cab<br>";
                                    $result_cab = mysqli_query($cnit, $query_cab);
                                    $num_results_cab = mysqli_num_rows($result_cab);
                                    $row_cab = mysqli_fetch_array($result_cab);	 
                                    if ($num_results_cab) {
                                            $cnamacab = $row_cab['nama'];
                                    }

                                    if (empty($cnamacab)) {
                                        $query_cab = "select initial nama from dbmaster.cabang_otc where cabangid_ho='$cicabangid_o'";
                                        $result_cab = mysqli_query($cnit, $query_cab);
                                        $num_results_cab = mysqli_num_rows($result_cab);
                                        if ($num_results_cab) {
                                                 $row_cab = mysqli_fetch_array($result_cab);
            ...
            $query = "select * from hrd.br_otc where ".$where_." AND icabangid_o='$cicabangid_o' AND IFNULL(batal,'')<>'Y' order by $order_"; 
             //echo"$query";exit;
            ...
            for ($i=0; $i < $num_results; $i++) {
                    ...
                    $g_tot_jumlah = $g_tot_jumlah + $jumlah;
                    $g_tot_jumlah1 = $g_tot_jumlah1 + $jumlah1;
                    $g_selisih = $jumlah - $jumlah1;
                    $g_tot_selisih = $g_tot_selisih + $g_selisih;
        ...
        /*
                if ($g_tot_jumlah<>0) {
                   echo "<tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>
                                 <td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td align='right'><b>Total :</b></td>
                                 <td align='right'><b><small>".number_format($g_tot_jumlah,0)."</small></b></td>
                                 <td align='right'><b><small>".number_format($g_tot_jumlah1,0)."</small></b></td>
                                 <td align='right'><b><small>".number_format($g_tot_selisih,0)."</small></b></td></tr>";
                }
          */  
        echo "</table>\n";
        echo "<br/>&nbsp;";
