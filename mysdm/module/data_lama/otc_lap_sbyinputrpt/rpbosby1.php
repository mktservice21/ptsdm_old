...
function disp_confirm(pText_)  {
	...
	if (ok_) {
		...
		if (r==true) {
			...
			document.getElementById("rpbosby1").action = "module/data_lama/otc_lap_sbyinputrpt/brosby0.php";
...
<?php
	...
	if (empty($_SESSION['IDCARD'])) {
	  ...
	  exit;
		...
		$tgl01 = $_POST['bulan'];
                ...
                $periode= date("Y-m", strtotime($tgl01));
                $tanggal= date("d", strtotime($tgl01));
                $bulan_= date("m", strtotime($tgl01));
                $nm_bln2 = nama_bulan($bulan_); //echo "$nm_bln1";
                $nm_bln = nama_bulan($bulan_); //echo "$nm_bln1";
		...
		echo "<b>INPUT REPORT SBY PERIODE $nm_bln2 $tahun_</b><br>";
		$query = "select * from hrd.br_otc where left(tgltrans,7)='$periode' and via<>'Y' and lampiran='Y' order by tgltrans,icabangid_o"; //echo"$query";// and tgl <= '$akhir'";
	...
	echo "&nbsp;&nbsp;<input type=button id=cmdBack name=cmdBack value='Back' onclick='goto2(\"rpbosby1\",\"rpbosby0.php\")'>";
