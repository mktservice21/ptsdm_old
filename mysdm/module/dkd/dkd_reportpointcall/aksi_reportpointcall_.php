...
$query = "UPDATE $tmp04 SET atasan1='' WHERE karyawanid=IFNULL(atasan1,'')";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
$query = "UPDATE $tmp04 SET atasan2='' WHERE karyawanid=IFNULL(atasan2,'')";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
$query = "UPDATE $tmp04 SET atasan3='' WHERE karyawanid=IFNULL(atasan3,'')";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
$query = "UPDATE $tmp04 SET atasan4='' WHERE karyawanid=IFNULL(atasan4,'')";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
$query = "UPDATE $tmp04 SET atasan2='' WHERE IFNULL(atasan1,'')=IFNULL(atasan2,'')";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

$query = "ALTER TABLE $tmp04 ADD COLUMN sudah_apv VARCHAR(1) DEFAULT 'N', ADD COLUMN jml_jv VARCHAR(1) DEFAULT '0'";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

$query = "UPDATE $tmp04 SET sudah_apv='Y' WHERE IFNULL(atasan1,'')<>'' AND IFNULL(tgl_atasan1,'') NOT IN ('', '0000-00-00', '0000-00-00 00:00:00') AND karyawanid<>IFNULL(atasan1,'')";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

$query = "UPDATE $tmp04 SET sudah_apv='Y' WHERE IFNULL(atasan2,'')<>'' AND IFNULL(tgl_atasan2,'') NOT IN ('', '0000-00-00', '0000-00-00 00:00:00') AND karyawanid<>IFNULL(atasan2,'')";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

$query = "UPDATE $tmp04 SET sudah_apv='Y' WHERE IFNULL(atasan3,'')<>'' AND IFNULL(tgl_atasan3,'') NOT IN ('', '0000-00-00', '0000-00-00 00:00:00') AND karyawanid<>IFNULL(atasan3,'')";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

$query = "UPDATE $tmp04 SET sudah_apv='Y' WHERE IFNULL(atasan4,'')<>'' AND IFNULL(tgl_atasan4,'') NOT IN ('', '0000-00-00', '0000-00-00 00:00:00') AND karyawanid<>IFNULL(atasan4,'')";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

$query = "DELETE FROM $tmp04 WHERE IFNULL(sudah_apv,'')='N'";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
...
$query = "INSERT INTO $tmp02 (idinput, karyawanid, jabatanid, tanggal, 
    jenis, dokterid, namalengkap, gelar, spesialis) SELECT"
        ...
        . " (SELECT DISTINCT IFNULL(CONCAT(karyawanid, tanggal, dokterid),'') FROM $tmp05 WHERE 1=1)";//1=1 => jadi : IFNULL(jenis,'') NOT IN ('JV')
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
...
for($ix=1; $ix<=(INT)$ptgl02;$ix++) {
    ...
    $query = "UPDATE $tmp05 as a JOIN (select '2' as idket, count(distinct dokterid) as jml from $tmp02 "
            . " WHERE 1=1 AND DATE_FORMAT(tanggal,'%d')='$pntgl' GROUP BY 1) as b on "//1=1  ==> jadi : IFNULL(jenis,'') NOT IN ('JV') 
            . " IFNULL(a.idket,'')=IFNULL(b.idket,'') SET "
...
$query = "INSERT INTO $tmp06 (karyawanid, idinput, jabatanid, tanggal)"
        . " SELECT DISTINCT karyawanid, idinput, jabatanid, tanggal FROM $tmp04 WHERE CONCAT(karyawanid, tanggal) NOT IN "
...
$query = "UPDATE $tmp06 as a JOIN (select karyawanid, tanggal, count(distinct dokterid) as jml FROM 
    $tmp04 WHERE IFNULL(jenis,'') NOT IN ('JV') GROUP BY 1,2) as b on a.karyawanid=b.karyawanid AND a.tanggal=b.tanggal SET a.totvisit=b.jml";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

$query = "UPDATE $tmp06 as a JOIN (select karyawanid, tanggal, count(distinct dokterid) as jml FROM 
    $tmp04 WHERE IFNULL(jenis,'') IN ('EC') GROUP BY 1,2) as b on a.karyawanid=b.karyawanid AND a.tanggal=b.tanggal SET a.totec=b.jml";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }


$query = "UPDATE $tmp06 as a JOIN (select karyawanid, tanggal, count(distinct dokterid) as jml FROM 
    $tmp04 WHERE IFNULL(jenis,'') IN ('JV') GROUP BY 1,2) as b on a.karyawanid=b.karyawanid AND a.tanggal=b.tanggal SET a.totjv=b.jml";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
