...
if ($ppilihrpt=="excel") {
    ...
    header("Content-Disposition: attachment; filename=INCENTIVE MR.xls");
}
...
$tmp04 ="dbtemp.tmprptincmr04_".$puser."_$now$milliseconds";
...
$query = "select jenis, jenis2, sales, target, ach, incentive from ms.incentive_mr WHERE bulan between '$pbln1' AND '$pbln2' AND karyawanid='$pkaryawanid'";
...
<div id='n_content'>
    ...
    <div id="kotakjudul">
        <div id="isikiri">
            <table class='tjudul' width='100%'>
                ...
                if ($ppilihrpt=="excel") {
                    echo "<tr><td colspan=5 width='150px'><b>Incentive MR</b></td></tr>";
                    ...
                    echo "<tr><td width='150px'><b><h3>Incentive MR</h3></b></td></tr>";
    ...
    <hr/>
    <div class="ijudul"><h2>Area & Group Produk</h2></div>
    <table id='mydatatable1' class='table table-striped table-bordered' width="100%" border="1px solid black">
        ...
        <tbody>
            ...
            while ($row= mysqli_fetch_array($tampil)) {
                ...
                $pdivpordid=$row['nama_area'];
                $ndivsiid=$row['divprodid'];
                
                echo "<tr>";
                echo "<td nowrap>$nicabangnm</td>";
                echo "<td nowrap>$pdivpordid</td>";
    ...
    <div>
        ...
        <?PHP
            $query = "select id, parentid, urutan, untuk, catatan from ms.catatan_incentive WHERE parentid='0' AND untuk IN ('', 'MR', 'MRAM', 'MRAMDM', 'MRDM') order by urutan";
            $tampil=mysqli_query($cnms, $query);
            while ($row=mysqli_fetch_array($tampil)) {
                $pid=$row['id'];
                $pjudul=$row['catatan'];
                ...
                $query = "select id, parentid, urutan, untuk, catatan from ms.catatan_incentive WHERE parentid='$pid' AND untuk IN ('', 'MR', 'MRAM', 'MRAMDM', 'MRDM') order by urutan";
