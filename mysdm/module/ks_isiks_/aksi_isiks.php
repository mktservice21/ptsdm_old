...
elseif ($module=='isikartustatus' AND ($act=='input' OR $act=='update') )
{
    ...
    //echo "GRP : $pidgrpuser, $kodenya, bln : $pbln, kry : $pidkry, dokt : $piddokt, apt : $pidapt ($pidapttipe), cn : $pcn, ttl : $ptotal<br/>"; exit;
    ...
    foreach ($_POST['chk_kodeid'] as $no_brid) {
        ...
        if ((DOUBLE)$pdet_qty==0 AND (DOUBLE)$pdet_jml==0) {
            ...
            $pinsert_data_detail_pl = "('$pidkry', '$pbln', '$piddokt', '$pidapt', '$pidapttipe', '$no_brid', '$pdet_qty', '$pdet_hna', '', '$pcn')";
            
            $query_detail="INSERT INTO hrd.ks1 (srid,bulan,dokterid,aptid,apttype,iprodid,qty,hna,approved,cn_ks1) VALUES ".$pinsert_data_detail_pl;
