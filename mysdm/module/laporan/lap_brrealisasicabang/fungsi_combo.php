...
function cBoxIsiDivisiProd($onclick){
    ...
    $sql=mysqli_query($cnmy, "SELECT DivProdId, nama FROM dbmaster.divprod where br='Y' AND DivProdId Not IN ('OTHER', 'OTC') order by nama");
...
function cBoxIsiKode(){
    ...
    $sql=mysqli_query($cnmy, "select kodeid,nama,divprodid from dbmaster.br_kode WHERE 1=1 $filtipe order by nama");
    while ($Xt=mysqli_fetch_array($sql)){
...
function cBoxIsiCabang(){
    include "config/koneksimysqli.php";
    $sql=mysqli_query($cnmy, "SELECT distinct iCabangId, nama from dbmaster.icabang order by nama");
    while ($Xt=mysqli_fetch_array($sql)){
...
function cBoxIsiCabangDaerah(){
    include "config/koneksimysqli.php";
    $sql=mysqli_query($cnmy, "SELECT distinct idcabang, nama from MKT.cbgytd order by nama");
    while ($Xt=mysqli_fetch_array($sql)){
