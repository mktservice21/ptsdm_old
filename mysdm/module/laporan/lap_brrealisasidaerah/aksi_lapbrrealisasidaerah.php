<?php
    ...
    if ($ppilihrpt=="excel") {
        ...
        header("Content-Disposition: attachment; filename=Laporan_Realisasi_Budget_Request_Ethical_Per_Daerah.xls");
    }
...
<?PHP
    ...
    $now=date("mdYhis");
    $tmp00 =" dbtemp.tmpbrrealdaerah00_".$puserid."_$now ";
    $tmp01 =" dbtemp.tmpbrrealdaerah01_".$puserid."_$now ";
    $tmp02 =" dbtemp.tmpbrrealdaerah02_".$puserid."_$now ";
    $tmp03 =" dbtemp.tmpbrrealdaerah03_".$puserid."_$now ";
    $tmp04 =" dbtemp.tmpbrrealdaerah04_".$puserid."_$now ";
    $tmp05 =" dbtemp.tmpbrrealdaerah05_".$puserid."_$now ";
    ...
    $query = "select distinct region, divprodid, idcabang, nama_daerah, cast(NULL as DECIMAL(20,2)) as DCC, "
    ...
    $query = "UPDATE $tmp04 a SET a.DCC=(select sum(b.jumlah) jumlah from $tmp03 b WHERE a.region=b.region AND a.divprodid=b.divprodid AND "
            . " a.idcabang=b.idcabang AND b.nama_kode like '%DCC%' )"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    $query = "UPDATE $tmp04 a SET a.DSS=(select sum(b.jumlah) jumlah from $tmp03 b WHERE a.region=b.region AND a.divprodid=b.divprodid AND "
            . " a.idcabang=b.idcabang AND b.nama_kode like '%DSS%' )"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
...
<HTML>
<HEAD>
    <title>Laporan Realisasi Budget Request Ethical Per Daerah</title>
...
<div id='n_content'>
    ...
    <center><div class='h1judul'>Laporan Realisasi Budget Request Ethical Per Daerah</div></center>
            ...
            <?PHP
                ...
                while ($row1= mysqli_fetch_array($group1)) {
                    ...
                    while ($row2= mysqli_fetch_array($group2)) {
                        ...
                        $query = "select * from $tmp04 WHERE divprodid='$pdivisi' AND region='$pnreg' order by divprodid, region, nama_daerah";
                        $tampil= mysqli_query($cnmy, $query);
                        while ($row3= mysqli_fetch_array($tampil)) {
                            
                            $pidcabang=$row3['idcabang'];
                            $pnmdaerah=$row3['nama_daerah'];
