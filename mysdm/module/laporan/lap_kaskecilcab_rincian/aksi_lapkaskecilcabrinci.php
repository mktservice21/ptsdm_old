...
$puserid=$_SESSION['USERID'];
$now=date("mdYhis");
$tmp01 =" dbtemp.tmprptkkcbrinci01_".$puserid."_$now ";
$tmp02 =" dbtemp.tmprptkkcbrinci02_".$puserid."_$now ";
$tmp03 =" dbtemp.tmprptkkcbrinci03_".$puserid."_$now ";
$tmp04 =" dbtemp.tmprptkkcbrinci04_".$puserid."_$now ";
...
if (!empty($pstsreport)) {
    if ($pstsreport=="apvfin") {
        $pfilterapv=" AND IFNULL(a.tgl_fin,'') <>'' ";
...
if (!empty($pidkaryawan)) {
    $pfilterkry=" AND a.karyawanid='$pidkaryawan' ";
...
$query = "select a.nmrealisasi, a.norekening, a.pengajuan, a.idkascab, a.tanggal, a.bulan, a.COA4, c.NAMA4, a.karyawanid, d.nama nama_karyawan,
    ...
    WHERE IFNULL(a.stsnonaktif,'')<>'Y' AND DATE_FORMAT(a.bulan,'%Y-%m') BETWEEN '$pperiode1' AND '$pperiode2'"
        . " $pfilterapv $pfilterkry ";

if (!empty($piddivisi)) {
    if ($piddivisi=="OTC" OR $piddivisi=="CHC") {
        $query .=" AND IFNULL(a.pengajuan,'') IN ('OTC', 'CHC') ";
    }else{
        $query .=" AND IFNULL(a.pengajuan,'') NOT IN ('OTC', 'CHC') ";
    }
}
...
$query = "Update $tmp01 SET icabangid=icabangid_o, nama_cabang=namacab_o, areaid=areaid_o, nama_area=namaarea_o WHERE IFNULL(pengajuan,'') IN ('OTC', 'CHC')";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

$query = "Update $tmp01 SET nama_cabang=icabangid_o WHERE IFNULL(pengajuan,'') IN ('OTC', 'CHC') AND IFNULL(nama_cabang,'')=''";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }


$query = "select a.nourut, a.idkascab, a.kode, b.nama nama_kode, a.jumlahrp, a.notes, 
        a.coa4 as coa, c.NAMA4 as nama_coa4, 
        e.bulan, e.karyawanid, e.nama_karyawan, e.nama_cabang, e.nama_area, e.nmrealisasi, e.keterangan 
        ...
        JOIN $tmp01 e on a.idkascab=e.idkascab 
        where a.idkascab=e.idkascab AND a.kode in $filterjenis";
$query = "create TEMPORARY table $tmp03 ($query)"; 
mysqli_query($cnmy, $query);
$erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        

$query = "DELETE FROM $tmp03 WHERE IFNULL(jumlahrp,0)=0";
mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

$query = "select b.bulan, b.karyawanid, b.nama_karyawan, b.nama_cabang, b.nama_area, b.nmrealisasi, b.keterangan, "
        . " a.idkascab, a.kode, a.nama_kode, a.coa4 as coa, a.NAMA4 as nama_coa4, a.notes, "
        . " a.jumlahrp "
        . " from $tmp02 as a "
        . " JOIN $tmp01 as b on a.idkascab=b.idkascab";
//$query = "create TEMPORARY table $tmp03 ($query)";
//mysqli_query($cnmy, $query);
//$erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
...
<HTML>
<HEAD>
    <title>Laporan Rincian Kas Kecil Cabang</title>
...
<div id='n_content'>
    ...
    <center><div class='h1judul'>Laporan Rincian Kas Kecil Cabang</div></center>
    ...
    <table id='mydatatable1' class='table table-striped table-bordered' width="100%" border="1px solid black">
        <thead>
            <tr>
                ...
                if ($psortby=="periode") {
                    ...
                    echo "<th align='center' nowrap>CABANG</th>";
                    echo "<th align='center' nowrap>AREA</th>";
                }else{
                    echo "<th align='center' nowrap>ID KARYAWAN</th>";
                    echo "<th align='center' nowrap>KARYAWAN</th>";
                    echo "<th align='center' nowrap>CABANG</th>";
        ...
        <tbody>
            ...
            if ($psortby=="periode") {
                $query .= " order by DATE_FORMAT(bulan,'%Y%m'), nama_karyawan, idkascab, kode";
            }else{
                $query .= " order by nama_karyawan, DATE_FORMAT(bulan,'%Y%m'), idkascab, kode";
            }
            $tampil=mysqli_query($cnmy, $query);
            while ($row= mysqli_fetch_array($tampil)) {
                
                $pidurut=$row['nourut'];
                $pidkode=$row['idkascab'];
                $pnoid=$row['kode'];
                $pidkary=$row['karyawanid'];
                $pnmkary=$row['nama_karyawan'];
                $pblnp=$row['bulan'];
                
                $pnamaid=$row['nama_kode'];
                $pcoakode=$row['coa'];
                ...
                $psimpan="<input type='button' id='btnsave[]' name='btnsave[]' value='Save' "
                        . " onclick=\"SimpanData('$pidurut', '$pidkode', '$pnoid', 'cb_coa[$pidurut]')\">";
                    
                if ($pidgrouppil=="23" OR $pidgrouppil=="40" OR $pidgrouppil=="26" OR $pidgrouppil=="1" OR $pidgrouppil=="24") {
                }else{
                    $pcbselect="";
                    $psimpan="";
                }
                ...
                echo "<td nowrap class='str'>$pidkode</td>";
    ...
    <script>
        function SimpanData(unourut, unoid, unoiddet, ucoa) {
            ...
            if (r==true) {
                ...
                $.ajax({
                    type:"post",
                    url:"module/laporan/lap_kaskecilcab_rincian/simpandataeditkascab.php?module="+module+"&act=input&idmenu="+idmenu,
                    data:"unourut="+unourut+"&uidkascab="+unoid+"&uinoid="+unoiddet+"&ucoa="+icoa,
