<?php
    ...
    if ($ppilihrpt=="excel") {
        ...
        header("Content-Disposition: attachment; filename=Laporan_Sewa_Rumah.xls");
    }
...
<?PHP
    ...
    $pdiviid = $_POST['cb_divisi'];
    $preportby = $_POST['cb_rpttype'];
    ...
    $query = "select distinct b.idinput, b.divisi, b.nodivisi, a.kodeinput, a.bridinput, b.pilih, b.kodeid, b.subkode from dbmaster.t_suratdana_br1 a "
            ...
            . " IFNULL(b.stsnonaktif,'')<>'Y' AND IFNULL(b.nodivisi,'')<>'' AND a.kodeinput IN ('S') "
    ...
    if (!empty($pdiviid)) $query .= " AND divisi='$pdiviid' ";
    $query = "create TEMPORARY table $tmp01 ($query)";
    mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    
    mysqli_query($cnmy, "UPDATE $tmp01 SET COA4='750-08' WHERE divisi='HO'"); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    mysqli_query($cnmy, "UPDATE $tmp01 SET COA4='751-08' WHERE divisi='EAGLE'"); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    mysqli_query($cnmy, "UPDATE $tmp01 SET COA4='752-08' WHERE divisi='PIGEO'"); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    mysqli_query($cnmy, "UPDATE $tmp01 SET COA4='753-08' WHERE divisi='PEACO'"); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    mysqli_query($cnmy, "UPDATE $tmp01 SET COA4='754-08' WHERE divisi='OTC'"); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    mysqli_query($cnmy, "UPDATE $tmp01 SET COA4='755-08' WHERE divisi='CAN'"); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    $query = "UPDATE $tmp02 a JOIN (select distinct pilih, nodivisi, idinput, bridinput, kodeid, subkode FROM $tmp00) b on a.idsewa=b.bridinput "
    ...
    $query = "ALTER table $tmp02 ADD COLUMN npilihrpt VARCHAR(50), ADD COLUMN npilihrpt2 VARCHAR(50)";
    mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    if ($preportby=="D") {
        $query = "UPDATE $tmp02 SET npilihrpt=nodivisi, npilihrpt2=nodivisi";
        mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
        mysqli_query($cnmy, "UPDATE $tmp02 SET nodivisi='000000', npilihrpt='000000', npilihrpt2='Blm Ada No. Divisi' WHERE IFNULL(nodivisi,'')=''"); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    }elseif ($preportby=="T") {
        $query = "UPDATE $tmp02 SET npilihrpt=DATE_FORMAT(tgltrans,'%Y%m%d'), npilihrpt2=DATE_FORMAT(tgltrans,'%d/%m/%Y') WHERE IFNULL(tgltrans,'')<>'' AND IFNULL(tgltrans,'0000-00-00')<>'0000-00-00'";
        mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
    }else{
        $query = "UPDATE $tmp02 SET npilihrpt=DATE_FORMAT(tgltrans,'%Y%m'), npilihrpt2=DATE_FORMAT(tgltrans,'%M %Y') WHERE IFNULL(tgltrans,'')<>'' AND IFNULL(tgltrans,'0000-00-00')<>'0000-00-00'";
        mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
    }
    
    if ($preportby!="D") {
        $query = "UPDATE $tmp02 SET npilihrpt='000000', npilihrpt2='Blm Transfer' WHERE IFNULL(tgltrans,'')='' OR IFNULL(tgltrans,'0000-00-00')='0000-00-00'";
        mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
        $query = "UPDATE $tmp02 SET npilihrpt='000000', npilihrpt2='' WHERE IFNULL(tgltrans,'')='' OR IFNULL(tgltrans,'0000-00-00')='0000-00-00' AND DATE_FORMAT(tgltrans,'%Y%m')<='2019'";
        mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    }
...
<HTML>
<HEAD>
    <title>Laporan Sewa Kontrakan Rumah</title>
...
<div id='n_content'>
    ...
    <center><div class='h1judul'>Laporan Sewa Kontrakan Rumah</div></center>
    ...
    <table id='datatable2' class='table table-striped table-bordered example_2' border="1px solid black">
        <thead>
            ...
            <?PHP
            if ($preportby=="D") {
                echo "<th align='center'>No. Divisi /<br/>No. Bukti</th>";
            }else{
                echo "<th align='center' nowrap>Periode Trsnf. /<br/>No. Bukti</th>";
            }
            ?>
        ...
        <tbody>
            ...
            $pgrandtotal=0;
            $no=1;
            $query = "select distinct npilihrpt, npilihrpt2 from $tmp02 ORDER BY npilihrpt";
            $tampil1= mysqli_query($cnmy, $query);
            while ($row1= mysqli_fetch_array($tampil1)) {
                $ppilihtype=$row1['npilihrpt'];
                $ppilihtype2=$row1['npilihrpt2'];
                
                $pinmpilihtype=$ppilihtype2;
                //if ($ppilihtype=="000000") $pinmpilihtype="Belum Ada NoDivisi";
                
                echo "<tr style='font-weight:bold;'>";
                echo "<td nowrap>$no</td>";
                echo "<td nowrap colspan='11'>$pinmpilihtype</td>";
                echo "</tr>";
                ...
                $query = "select * from $tmp02 WHERE npilihrpt='$ppilihtype' ";
                $query .= " ORDER BY npilihrpt, nobuktibbk, tgltrans, divisi, nama_karyawan, nama_cabang, NAMA4";
                ...
                while ($row= mysqli_fetch_array($tampil)) {
                    ...
                    if ($pdivisi=="ETH") $pnmdivisi="CANARY";


                    $pgrtotal1=(double)$pgrtotal1+(double)$pjmlrp1;
                    $pgrandtotal=(double)$pgrandtotal+(double)$pjmlrp1;

                    if ($ptgltrans=="0000-00-00") $ptgltrans="";

                    $ptglbr = date("F Y", strtotime($ptglbr));
                    $ptglbr2 = date("F Y", strtotime($ptglbr2));
                    ...
                    echo "<td nowrap>$ptglbr s/d. $ptglbr2</td>";
                ...
                echo "<tr style='font-weight:bold;'>";
                echo "<td nowrap colspan='9' align='right'>Total $pinmpilihtype : </td>";
                echo "<td nowrap align='right'>$pgrtotal1</td>";
                echo "<td nowrap colspan='1'></td>";
                echo "</tr>";
            ...
            echo "<tr style='font-weight:bold;'>";
            echo "<td nowrap colspan='9' align='right'>G R A N D  &nbsp;&nbsp;&nbsp; T O T A L : </td>";
            echo "<td nowrap align='right'>$pgrandtotal</td>";
            echo "<td nowrap colspan='1'></td>";
            echo "</tr>";
