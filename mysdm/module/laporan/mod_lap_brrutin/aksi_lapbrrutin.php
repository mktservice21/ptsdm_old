...
<?php
    ...
    $query = "DELETE FROM $tmp01 WHERE IFNULL(rptotal,0)=0";
    mysqli_query($cnit, $query);
    ...
    $query = "select distinct tgl, nodivisi from dbmaster.t_suratdana_br WHERE DATE_FORMAT(tglf,'%Y%m')='$periode1' AND kodeid='1' AND subkode='03' and kodeperiode='$pkodeperiode' ";
    ...
    $pbukti="";
    if (!empty($nodivisi)) {
        $query = "select nobukti, tanggal from dbmaster.t_suratdana_bank WHERE stsnonaktif<>'Y' AND stsinput='K' AND nodivisi='$nodivisi' ORDER BY tanggal LIMIT 1";
        ...
        <table id='datatable2' class='table table-striped table-bordered example_2' border="1px solid black">
            ...
            <tbody>
                <?PHP
                    ...
                    $gtotalsaldo=0;
                    $totalcredit=0;
                    //$query = "select distinct IFNULL(divisi,'') divisi from $tmp01 order by IFNULL(divisi,'')";
                    //$resultd = mysqli_query($cnit, $query);
                    //while ($rdiv = mysqli_fetch_array($resultd)) {
                    //    $nndivisi=$rdiv['divisi'];
                    
                        $query = "select distinct divisi, karyawanid, nama, idrutin, jumlah from $tmp01 order by divisi, nama, idrutin";
