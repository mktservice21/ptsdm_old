...
<?php
    ...
    $ntgltransfer="";
    $atglklaim_="";
    $query = "select a.tgl, a.tglspd, a.nomor, a.nodivisi, b.nobukti, b.tanggal from 
        dbmaster.t_suratdana_br a LEFT JOIN 
        (select idinput, tanggal, nobukti from dbmaster.t_suratdana_bank WHERE divisi='OTC' AND CONCAT(kodeid,subkode)='103'
        AND stsnonaktif<>'Y' AND stsinput='K' LIMIT 1) as b 
        on a.idinput=b.idinput
        WHERE a.stsnonaktif<>'Y' AND a.divisi='OTC' AND CONCAT(a.kodeid,a.subkode)='103'  AND IFNULL(a.nodivisi,'')<>''
        and DATE_FORMAT(a.tglf,'%Y%m')='$periode1'";
    //echo $query;
    ...
    if ($ketemu>0) {
        ...
        $nnobukti=$rw['nobukti'];
        $nnomordivisi=$rw['nodivisi'];
        $ntgl_=$rw['tanggal'];
        if (!empty($ntgl_)) $ntgltransfer = date("d F Y", strtotime($ntgl_));
        
        $atglklaim_=$rw['tgl'];
        $atglklaim_ = date("d F Y", strtotime($atglklaim_));
    }
    
    
    $ptgltrasn="";
    $pbukti="";
    if (!empty($nnomordivisi)) {
        $query = "select nobukti, tanggal from dbmaster.t_suratdana_bank WHERE stsnonaktif<>'Y' AND stsinput='K' AND nodivisi='$nnomordivisi' ORDER BY tanggal LIMIT 1";
        $tampil=mysqli_query($cnmy, $query);
    ...
    <div id="kotakjudul">
        <div id="isikiri">
            <table class='tjudul' width='100%'>
                ...
                <tr><td><b>Biaya Rutin Per <?PHP echo "$per1"; ?></b></td><td>:</td><td><?PHP echo "$nnomordivisi"; ?></td></tr>
                <tr><td><b>** Klaim </b></td><td>:</td><td><?PHP echo "$atglklaim_"; ?></td></tr>
            </table>
        </div>
        <div id="isikanan">
