<?PHP
    ...
    if ($ppilihrpt=="excel") {
        ...
        header("Content-Disposition: attachment; filename=LAPORAN LUAR KOTA PER TAHUN.xls");
    }
    ...
    $now=date("mdYhis");
    $tmp01 =" dbtemp.DBRROTCPBLLLK01_".$_SESSION['IDCARD']."_$now ";
    $tmp02 =" dbtemp.DBRROTCPBLLLK02_".$_SESSION['IDCARD']."_$now ";
    $tmp03 =" dbtemp.DBRROTCPBLLLK03_".$_SESSION['IDCARD']."_$now ";
    $tmp04 =" dbtemp.DBRROTCPBLLLK04_".$_SESSION['IDCARD']."_$now ";
    $tmp05 =" dbtemp.DBRROTCPBLLLK05_".$_SESSION['IDCARD']."_$now ";
    
    
    $pfilterjabatan=('');
    if (!empty($_POST['chkbox_posisi'])){
        $pfilterjabatan=$_POST['chkbox_posisi'];
    ...
    //AND ifnull(tgl_fin,'') <> '' AND ifnull(tgl_fin,'0000-00-00') <> '0000-00-00' 
...
<html>
<head>
    <title>LAPORAN BIAYA RUTIN PER TAHUN</title>
...
<div id='n_content'>
    ...
    <div id="kotakjudul">
        <div id="isikiri">
            <table class='tjudul' width='100%'>
                <tr><td width="150px"><b>LAPORAN LUAR KOTA PER TAHUN JANUARI - DESEMBER <?php echo $ptahun; ?></b></td></tr>
            </table>
        </div>
        <div id="isikanan">
    ...
    <table id='datatable3' class='table table-striped table-bordered' width="100%" border="1px solid black">
            <thead>
                <tr>
                    <th width='6px'></th>
                    <th width='6px'>NAMA</th>
                    <th width='30px'>Jan</th>
                    <th width='30px'>Feb</th>
                    <th width='30px'>Mar</th>
                    <th width='30px'>Apr</th>
            ...
            <?PHP
                ...
                $no=1;
                $query = "select divisi karyawanid, divisi nama, sum(bln_1) bln_1,"
                        . " sum(bln_2) bln_2, sum(bln_3) bln_3, sum(bln_4) bln_4, sum(bln_5) bln_5,"
                        . " sum(bln_6) bln_6, sum(bln_7) bln_7, sum(bln_8) bln_8, sum(bln_9) bln_9,"
                        . " sum(bln_10) bln_10, sum(bln_11) bln_11, sum(bln_12) bln_12, sum(vtotal) vtotal from $tmp03 GROUP BY 1,2 order by divisi";
                $tampil= mysqli_query($cnmy, $query);
                while ($row= mysqli_fetch_array($tampil)) {
                    $pkaryawanid=$row['karyawanid'];
                    $pnama=$row['nama'];
