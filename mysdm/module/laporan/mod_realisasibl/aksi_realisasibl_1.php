...
<?php
    ...
    $query ="select a.jml_adj, a.idca1 idca, a.idrutin, a.keterangan, a.tgltrans, 
        ...
        a.saldo jumlah, b.qty, b.rp, b.rptotal, CAST('' as CHAR(150)) as nama_atasan_pengaju 
    ...
    $query = "SELECT karyawanid, atasan1, atasan2, atasan3, atasan4, jabatanid from dbmaster.t_brrutin0 WHERE idrutin in (
            select DISTINCT idrutin from $tmp04 WHERE IFNULL(idrutin,'')<>''
            )";
    $query = "create TEMPORARY table $tmp01 ($query)"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    $query = "SELECT karyawanid, atasan1, atasan2, atasan3, atasan4, jabatanid from dbmaster.t_ca0 WHERE idca in (
            select DISTINCT idca from $tmp04 WHERE IFNULL(idrutin,'')='' AND IFNULL(idca,'')<>''
            )";
    $query = "create TEMPORARY table $tmp03 ($query)"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    $query = "SELECT karyawanid, atasan1, atasan2, atasan3, atasan4, jabatanid from dbmaster.t_ca0 WHERE karyawanid in (
        select DISTINCT karyawanid from $tmp04 WHERE IFNULL(idrutin,'')='' AND IFNULL(idca,'')=''
        ) AND DATE_FORMAT(periode,'%Y-%m')='$pperiode_nccx' AND stsnonaktif<>'Y'";
    $query = "create TEMPORARY table $tmp08 ($query)"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    mysqli_query($cnit, "DELETE FROM $tmp08 WHERE karyawanid IN (select distinct karyawanid from $tmp01)");
    mysqli_query($cnit, "DELETE FROM $tmp08 WHERE karyawanid IN (select distinct karyawanid from $tmp03)");
    
    mysqli_query($cnit, "INSERT $tmp01 (karyawanid, atasan1, atasan2, atasan3, atasan4, jabatanid) select karyawanid, atasan1, atasan2, atasan3, atasan4, jabatanid from $tmp03");
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    mysqli_query($cnit, "INSERT $tmp01 (karyawanid, atasan1, atasan2, atasan3, atasan4, jabatanid) select karyawanid, atasan1, atasan2, atasan3, atasan4, jabatanid from $tmp08");
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    mysqli_query($cnit, "alter table $tmp01 ADD COLUMN nama_atasan CHAR(150)");
    mysqli_query($cnit, "UPDATE $tmp01 a SET a.nama_atasan ='FARIDA SOEWANTO' WHERE jabatanid IN ('05', '06', '22')");
    mysqli_query($cnit, "UPDATE $tmp01 a SET a.nama_atasan =(select b.nama from hrd.karyawan b WHERE a.atasan3=b.karyawanid) WHERE jabatanid='08'");
    mysqli_query($cnit, "UPDATE $tmp01 a SET a.nama_atasan =(select b.nama from hrd.karyawan b WHERE a.atasan2=b.karyawanid) WHERE jabatanid IN ('10', '18')");
    mysqli_query($cnit, "UPDATE $tmp01 a SET a.nama_atasan =(select b.nama from hrd.karyawan b WHERE a.atasan3=b.karyawanid) WHERE jabatanid IN ('10', '18') AND IFNULL(nama_atasan,'')=''");
    //mysqli_query($cnit, "UPDATE $tmp01 a SET a.nama_atasan =(select b.nama from hrd.karyawan b WHERE a.atasan1=b.karyawanid) WHERE jabatanid='15'");
    mysqli_query($cnit, "UPDATE $tmp01 a SET a.nama_atasan =(select b.nama from hrd.karyawan b WHERE a.atasan2=b.karyawanid) WHERE jabatanid='15'");
    //mysqli_query($cnit, "UPDATE $tmp01 a SET a.nama_atasan =(select b.nama from hrd.karyawan b WHERE a.atasan2=b.karyawanid) WHERE jabatanid='15' AND IFNULL(nama_atasan,'')=''");
    mysqli_query($cnit, "UPDATE $tmp01 a SET a.nama_atasan ='' WHERE jabatanid IN ('15', '10', '18') AND (nama_atasan like '%NN - DM%' OR nama_atasan like '%DM - %')");
    mysqli_query($cnit, "UPDATE $tmp01 a SET a.nama_atasan =(select b.nama from hrd.karyawan b WHERE a.atasan3=b.karyawanid) WHERE jabatanid IN ('15', '10', '18') AND IFNULL(nama_atasan,'')=''");
    mysqli_query($cnit, "UPDATE $tmp01 a SET a.nama_atasan =(select b.nama from hrd.karyawan b WHERE a.atasan4=b.karyawanid) WHERE jabatanid IN ('20')");
    
    mysqli_query($cnit, "UPDATE $tmp04 a SET a.nama_atasan_pengaju =(select b.nama_atasan from $tmp01 b WHERE a.karyawanid=b.karyawanid AND IFNULL(nama_atasan,'')<>'' LIMIT 1)");
    
    
    $pnomordivisi="";
    if ($stsreport=="C" OR $stsreport=="S") {
        $query = "select b.nodivisi 
                from dbmaster.t_suratdana_br1 a JOIN dbmaster.t_suratdana_br b on a.idinput=b.idinput
                where b.stsnonaktif<>'Y' AND DATE_FORMAT(tgl,'%Y%m')='$bulan' AND a.kodeinput='I' AND a.bridinput IN (select distinct idrutin from $tmp04 WHERE IFNULL(idrutin,'')<>'') LIMIT 1";
        $tampil=mysqli_query($cnit, $query);
        $nr= mysqli_fetch_array($tampil);
        if (!empty($nr['nodivisi'])) {
            $pnomordivisi=$nr['nodivisi'];
        }
    }
    ...
    <div id="kotakjudul">
        <div id="isikiri">
            <table class='tjudul' width='100%'>
                ...
                <?PHP if (!empty($pnomordivisi)) { ?>
                <tr><td width="210px"><b>No. <?PHP echo $pnomordivisi; ?> </b></td><td></td></tr>
                <?PHP } ?>
