...
<?php
    ...
    include ("module/mod_br_closing_lkcaotc/seleksi_data_lk_caotc.php");
    
    
    //$now=date("mdYhis");
    //$tmp21 =" dbtemp.DTBRRETRLCLS21_".$_SESSION['IDCARD']."_$now ";
    
    //$query = "create table $tmp21 (select * from $tmp01)"; 
    ...
    $query ="select a.jml_adj, a.idca1 idca, a.idrutin, a.keterangan, a.tgltrans, 
        ...
        a.saldo jumlah, b.qty, b.rp, b.rptotal, CAST('' as CHAR(1)) as batal, CAST('' as CHAR(200)) as alasan_batal
    ...
    $query="select idca, batal, alasan_batal from dbmaster.t_ca0 where IFNULL(batal,'')='Y' and idca in "
            . " (select distinct IFNULL(idca,'') FROM $tmp04 WHERE IFNULL(idca,'')<>'')";
    $query = "create TEMPORARY table $tmp01 ($query)"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    
    $query="UPDATE $tmp04 as a JOIN $tmp01 as b on a.idca=b.idca SET a.batal=b.batal, a.nama_des=b.alasan_batal where IFNULL(b.batal,'')='Y'";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    mysqli_query($cnit, "drop TEMPORARY table $tmp01");
    ...
    if ($stsreport=="C" OR $stsreport=="S") {
        $query = "select b.nodivisi 
                ...
                where b.stsnonaktif<>'Y' AND a.kodeinput='M' AND a.bridinput IN (select distinct idrutin from $tmp04 WHERE IFNULL(idrutin,'')<>'') LIMIT 1";
    ...
    $ptgltrasn="";
    $pbukti="";
    if (!empty($pnomordivisi)) {
        $query = "select nobukti, tanggal from dbmaster.t_suratdana_bank WHERE stsnonaktif<>'Y' AND stsinput='K' AND nodivisi='$pnomordivisi' ORDER BY tanggal LIMIT 1";
        $tampil=mysqli_query($cnit, $query);
        $nr= mysqli_fetch_array($tampil);
        
        $pbukti=$nr['nobukti'];
        $ptgltrasn=$nr['tanggal'];
        if (!empty($ptgltrasn) AND $ptgltrasn<>"0000-00-00") $ptgltrasn = date('d/m/Y', strtotime($ptgltrasn));
            ...
            <tbody>
                <?PHP
                    ...
                    if ($records) {
                        ...
                        echo "<td nowrap align='right'><b>$totalrpadj</b></td>";//utang piutang
