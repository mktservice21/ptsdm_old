...
<?php
    ...
    if (isset($_GET['ispd'])) {
        ...
        if ($ketemu>0) {
			...
			$zkdperiode = $_POST['e_periode'];
			$pperiodepuktttd = $_POST['bulan1'];
			if (!empty($pperiodepuktttd)) {
				$pperiodepuktttd = date("Ymd", strtotime($pperiodepuktttd));
					if ((double)$pperiodepuktttd>='20200701') {
						$nnama_ss_mktdir=$nnama_ss_mktdir2;
					}elseif ((double)$pperiodepuktttd=='20200601' AND $zkdperiode=="2") {
						$nnama_ss_mktdir=$nnama_ss_mktdir2;
					}
			}
	}
...
?>
    ...
    <div class="clearfix"></div>
        ...
        echo "<tr align='center'>";
        echo "<td>(..................)</td> <td></td> <td>(Marianne Prasanti)</td> <td></td> <td>($nnama_ss_mktdir)</td>";
        echo "</tr>";
