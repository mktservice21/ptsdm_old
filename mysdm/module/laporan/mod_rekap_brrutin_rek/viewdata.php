...
if ($_GET['module']=="caribuktisudahada"){
    ...
    if (!empty($piddata)) {
        ...
        $query = "SELECT tgl_proses FROM dbmaster.t_suratdana_br WHERE stsnonaktif<>'Y' AND kodeid='$pkode' AND "
                . " subkode='$psubkode' and nodivisi='$piddata' AND tglf='$tglinput'";
        $result = mysqli_query($cnit, $query);
        $records = mysqli_num_rows($result);
        if ($records>0){
        ...
        while ($sh= mysqli_fetch_array($result)) {
            ...
            if (!empty($kodenya)){
                $query = "INSERT INTO $dbname.t_suratdana_br (idinput, divisi, kodeid, subkode, tgl, nodivisi, jumlah, userid, coa4, kodeperiode, tglf, tglt, karyawanid)values"
                        . "('$kodenya', '$ndivisi', '$pkode', '$psubkode', '$tglinput', '$pdivno', '$pjumlah', '$userid', '$pcoa', '$kdperiode', '$myinpperiode1', '$myinpperiode2', '0000000143')";
                mysqli_query($cnmy, $query);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo "Error..."; exit; }
                ...
                while ($dv= mysqli_fetch_array($tampildiv)) {
                    ...
                    $query="SELECT idrutin nobrid, jumlah from $tmp01 WHERE divisi='$ddivisi' order by idrutin";
