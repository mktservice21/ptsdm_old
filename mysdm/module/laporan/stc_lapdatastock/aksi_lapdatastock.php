...
<?PHP
    ...
    $query_prod = "";
    if (!empty($pdivisipil)) {
        $query_prod = "select a.id, a.kdproduk, a.nmproduk, b.divprodid, a.iprodid from sls.imaping_produk a LEFT JOIN sls.iproduk b "
                . " on a.iprodid=b.iprodid "
                . " WHERE (b.divprodid='$pdivisipil' OR IFNULL(a.iprodid,'')='') ";
        if (!empty($pprodpil)) $query_prod .= " AND a.kdproduk='$pprodpil'";
    }else{
        if (!empty($pprodpil)) {
            $query_prod = "select a.id, a.kdproduk, a.nmproduk, b.divprodid, a.iprodid from sls.imaping_produk a JOIN sls.iproduk b "
                    . " on a.iprodid=b.iprodid "
                    . " WHERE a.kdproduk='$pprodpil' ";
        }
    }
    if (!empty($query_prod)) {
...
<div id='n_content'>
            ...
            <?PHP
                ...
                if ($ketemu1>0) {
                    while ($row1= mysqli_fetch_array($tampil1)) {
                        ...
                        if ($nfile0=="OTHER") $pnmdivisi="OTHER";
                        
                        if (empty($nfile0)) $pnmdivisi="uncategorized";
    ...
    <br/>
    <u><b>Notes :</b></u><br/>
    Total Produksi merupakan jumlah hasil dari produksi dengan nomor batch tersebut.<br/>
    Jika Total Produksi kosong : <br/>
    - Data produksinya sudah terlalu lama jadi tidak bisa tampil atau ada juga kemungkinan salah input nomor batch.<br/>
    - kemungkinan lain, yaitu data hasil produksi belum masuk ke Database, tetapi kemungkinan ini kecil, tapi bisa terjadi.
