...
<?PHP
    ...
    if (!empty($pbreth)) {
        ...
        //NODIVISI
        $query = "select distinct a.idinput, a.bridinput, b.nodivisi, b.pilih, a.amount, a.jml_adj, b.kodeid, b.subkode "
                . " from dbmaster.t_suratdana_br1 a JOIN dbmaster.t_suratdana_br b on a.idinput=b.idinput "
                . " WHERE b.stsnonaktif<>'Y' AND a.kodeinput IN ('A', 'B', 'C') AND b.divisi<>'OTC' AND a.bridinput IN (select distinct IFNULL(brId,'') FROM $tmp03)";
        $query = "create TEMPORARY table $tmp04 ($query)";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
        $query = "UPDATE $tmp03 a JOIN $tmp04 b on a.brId=b.bridinput "
        ...
        $query = "INSERT INTO $tmp01 (kodeinput, idinput, divisi, tgltrans, coa, dokter, noslip, pengajuan, keterangan, nmrealisasi, kredit, dpp, ppn, pph, tglfp, nodivisi, idinputdiv)"
                . " SELECT 'A' kodeinput, brId, divprodid, tgltrans, COA4, nama_dokter, noslip, nama_karyawan, aktivitas1, realisasi1, jumlah, dpp, ppn_rp, pph_rp, tgl_fp, nodivisi, idinput FROM $tmp03";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    if (!empty($pklaim)) {
        ...
        $query = "select distinct a.idinput, a.bridinput, b.nodivisi, b.pilih, a.amount, a.jml_adj, b.kodeid, b.subkode "
                . " from dbmaster.t_suratdana_br1 a JOIN dbmaster.t_suratdana_br b on a.idinput=b.idinput "
                . " WHERE b.stsnonaktif<>'Y' AND a.kodeinput IN ('E') AND b.divisi='EAGLE' AND a.bridinput IN (select distinct IFNULL(klaimId,'') FROM $tmp03)";
        $query = "create TEMPORARY table $tmp04 ($query)";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
        $query = "UPDATE $tmp03 a JOIN $tmp04 b on a.klaimId=b.bridinput "
                . " SET a.nodivisi=b.nodivisi, a.idinput=b.idinput WHERE b.pilih='Y'"; 
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    if (!empty($pkas)) {
        ...
        $query = "select 'HO' as divprodid, a.periode1 tgltrans, a.kasId, a.kode, '105-02' as COA4, a.karyawanid, f.nama nama_karyawan, a.nobukti,
                a.aktivitas1, a.jumlah
                FROM hrd.kas a LEFT JOIN dbmaster.posting_coa_kas b on a.kode=b.kodeid
        ...
        $query="select 'HO' as divisi, a.tgl, a.idkasbon, '' as kode, '105-02' as COA4, a.karyawanid, nama as nama_karyawan, '' as nobukti,
        ...
        $query = "select distinct a.idinput, a.bridinput, b.nodivisi, b.pilih, a.amount, a.jml_adj, b.kodeid, b.subkode "
                . " from dbmaster.t_suratdana_br1 a JOIN dbmaster.t_suratdana_br b on a.idinput=b.idinput "
                . " WHERE b.stsnonaktif<>'Y' AND a.kodeinput IN ('T', 'K') AND a.bridinput IN (select distinct IFNULL(kasId,'') FROM $tmp03)";
        $query = "create TEMPORARY table $tmp04 ($query)";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
        $query = "UPDATE $tmp03 a JOIN $tmp04 b on a.kasId=b.bridinput "
                . " SET a.nodivisi=b.nodivisi, a.idinput=b.idinput WHERE b.pilih='Y'"; 
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    if (!empty($pbrotc)) {
        ...
        $query = "select distinct a.idinput, a.bridinput, b.nodivisi, b.pilih, a.amount, a.jml_adj, b.kodeid, b.subkode "
                . " from dbmaster.t_suratdana_br1 a JOIN dbmaster.t_suratdana_br b on a.idinput=b.idinput "
                . " WHERE b.stsnonaktif<>'Y' AND a.kodeinput IN ('D') AND b.divisi='OTC' AND a.bridinput IN (select distinct IFNULL(brOtcId,'') FROM $tmp03)";
        $query = "create TEMPORARY table $tmp04 ($query)";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
        
        $query = "UPDATE $tmp03 a JOIN $tmp04 b on a.brOtcId=b.bridinput "
                . " SET a.nodivisi=b.nodivisi, a.idinput=b.idinput WHERE b.pilih='Y'"; 
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    if (!empty($prutin) OR !empty($pblk)) {
        ...
        $query = "select distinct a.idinput, a.bridinput, b.nodivisi, b.pilih, a.amount, a.jml_adj, b.kodeid, b.subkode "
                . " from dbmaster.t_suratdana_br1 a JOIN dbmaster.t_suratdana_br b on a.idinput=b.idinput "
                . " WHERE b.stsnonaktif<>'Y' AND a.kodeinput IN ('F', 'I') AND a.bridinput IN (select distinct IFNULL(idrutin,'') FROM $tmp03)";
        $query = "create TEMPORARY table $tmp06 ($query)";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
        $query = "UPDATE $tmp03 a JOIN $tmp06 b on a.idrutin=b.bridinput "
