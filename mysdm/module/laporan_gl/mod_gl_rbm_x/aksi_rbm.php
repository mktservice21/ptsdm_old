            ...
            <tbody>
                ...
                while ($row0= mysqli_fetch_array($tampil0)) {
                    ...
                    for ($x=1;$x<=12;$x++) {
                        ...
                        //cari total br
                        $query = "select IFNULL(sum(jumlah),0) jumlah From $tmp05 WHERE divprodid='$divisi' AND DATE_FORMAT(tgltrans,'%Y-%m')='$zbulan'";
                        $rowslb=mysqli_query($cnmy, $query);
                        $ketemubr= mysqli_num_rows($rowslb);
                        if ($ketemubr>0) {
                            $rslb= mysqli_fetch_array($rowslb);
                ...
                */
                ...
                for ($x=1;$x<=12;$x++) {
                    ...
                    //cari total br
                    $query = "select IFNULL(sum(jumlah),0) jumlah From $tmp05 WHERE DATE_FORMAT(tgltrans,'%Y-%m')='$zbulan'";
                    $rowslb=mysqli_query($cnmy, $query);
                    $ketemubr= mysqli_num_rows($rowslb);
                    if ($ketemubr>0) {
                        $rslb= mysqli_fetch_array($rowslb);
