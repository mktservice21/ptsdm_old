...
<?PHP
    ...
    if ($ppildivisiid=="OTC") { //fe_it.otc_etl
        ...
        $query = "select icabangid, date_format(bulan,'%Y-%m') bulan, divprodid, sum(value_sales) as rpsales from dbmaster.sales_local WHERE YEAR(bulan)='$periode' GROUP BY 1,2,3";// DATE_FORMAT(bulan,'%Y%m')='$npppperi'
        $query = "create TEMPORARY table $tmp04 ($query)";
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
