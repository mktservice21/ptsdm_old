    ...
    if ($_GET['ket']=="bukan") {
        ...
        */
        ...
        if ($pnjenisrpt=="D" OR $pnjenisrpt=="C") {
            $query = "select '' lampiran, '' as kode, '' as ca, '' as via,
                a.DIVISI divprodid, a.pengajuan as divpengajuan, a.klaimId brId, a.karyawanid karyawanId, c.nama nama_karyawan,
            ...
            $query ="UPDATE $tmp02 a JOIN dbmaster.coa_level4 d on a.COA4=d.COA4 SET a.NAMA4=d.NAMA4";
            mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
		...
		//PAJAK TAMBAHAN
		$query = "select * from dbmaster.t_br0_pajak  WHERE brid IN 
			(SELECT DISTINCT IFNULL(brid,'') FROM $tmp03)";
		$query = "create TEMPORARY table $tmp06 ($query)";
		mysqli_query($cnit, $query);
		$erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
		...
		if ($pnjenisrpt=="D") {
            ...
            $query = "select a.divprodid, b.brId, a.tgl, a.noslip, a.tgltrans, a.tglrpsby, 
					a.karyawanId, a.nama_karyawan, a.dokterId, a.dokter, 
					a.nama_dokter, a.aktivitas1, a.aktivitas2, a.realisasi1, a.jumlah, a.jumlah1, 
					a.realisasi2, a.COA4, a.NAMA4, 
					a.kode, a.nama_kode, a.lampiran, a.ca, a.via, 
					a.pajak, b.dpp, b.ppn, b.ppn_rp, b.pph_jns, b.pph, b.pph_rp, b.tgl_fp, b.noseri, b.pembulatan, 
					b.jasa_rp, b.materai_rp, b.jenis_dpp, a.nama_cabang, a.urutan, a.nodivisi, a.amount, 
					a.nobbm, a.nobbk, a.jns_pajak, a.stsbatal from $tmp06 b JOIN 
					$tmp03 a on a.brid=b.brid";
            $query = "create TEMPORARY table $tmp07 ($query)"; 
            mysqli_query($cnit, $query);
            $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; }
			
			
        //INSERT PAJAK TAMBAHAN
		
            $query = "INSERT INTO $tmp03 (divprodid, brId, tgl, noslip, tgltrans, tglrpsby, 
					karyawanId, nama_karyawan, dokterId, dokter, 
					nama_dokter, aktivitas1, aktivitas2, realisasi1, jumlah, jumlah1, 
					realisasi2, COA4, NAMA4, 
					kode, nama_kode, lampiran, ca, via, 
					pajak, dpp, ppn, ppn_rp, pph_jns, pph, pph_rp, tgl_fp, noseri, pembulatan, 
					jasa_rp, materai_rp, jenis_dpp, nama_cabang, urutan, nodivisi, amount, 
					nobbm, nobbk, jns_pajak, stsbatal)
					select divprodid, brId, tgl, noslip, tgltrans, tglrpsby, 
					karyawanId, nama_karyawan, dokterId, dokter, 
					nama_dokter, aktivitas1, aktivitas2, realisasi1, jumlah, jumlah1, 
					realisasi2, COA4, NAMA4, 
					kode, nama_kode, lampiran, ca, via, 
					pajak, dpp, ppn, ppn_rp, pph_jns, pph, pph_rp, tgl_fp, noseri, pembulatan, 
					jasa_rp, materai_rp, jenis_dpp, nama_cabang, urutan, nodivisi, amount, 
					nobbm, nobbk, jns_pajak, stsbatal from $tmp07";
            ...
            <?PHP
                ...
                while ($row= mysqli_fetch_array($tampil2)) {
                    ...
                    if ($ppajak=="T") {
						...
						if ($ppajak=="Y" AND $pjnsdpp=="A") $pkredit = (double)$row['jasa_rp']+(double)$row['dpp'];
                    }
                    $psaldo=$pkredit;
