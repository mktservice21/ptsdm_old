<?php
    ...
    $query = "UPDATE $tmp03 SET amount='62096739.20' WHERE brOtcId='0000014766'";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    $query = "select * from dbmaster.t_br_otc_pajak  WHERE brOtcId IN 
        (SELECT DISTINCT IFNULL(brOtcId,'') FROM $tmp03)";
    $query = "create TEMPORARY table $tmp06 ($query)";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    $query = "
        SELECT a.brOtcId, a.tgltrans, a.COA4, a.NAMA4, a.icabangid_o, a.nama_cabang, a.noslip, a.kodeid, a.subpost,
        a.keterangan1, a.real1, a.jumlah, a.realisasi, 
        a.pajak, a.jns_pajak, b.dpp, b.ppn, b.ppn_rp, 
        b.pph_jns, b.pph, b.pph_rp, b.tgl_fp, b.noseri, b.pembulatan, 
        b.materai_rp, b.jasa_rp, b.jenis_dpp, 
        a.lampiran, a.ca, a.via, a.tglbr, a.tglrpsby, a.tglreal, 
        a.stsbatal, a.alasan_batal, a.nama_posting, a.urutan,
        a.nodivisi, a.amount, a.nobbm, a.nobbk
        FROM $tmp06 b JOIN
        $tmp03 a on a.brOtcId=b.brOtcId";
    $query = "create TEMPORARY table $tmp07 ($query)";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    
    $query = "INSERT INTO $tmp03 (brOtcId, tgltrans, COA4, NAMA4, icabangid_o, nama_cabang, noslip, kodeid, subpost,
        keterangan1, real1, jumlah, realisasi, 
        pajak, jns_pajak, dpp, ppn, ppn_rp, 
        pph_jns, pph, pph_rp, tgl_fp, noseri, pembulatan, 
        materai_rp, jasa_rp, jenis_dpp, 
        lampiran, ca, via, tglbr, tglrpsby, tglreal, 
        stsbatal, alasan_batal, nama_posting, urutan,
        nodivisi, amount, nobbm, nobbk) SELECT * FROM $tmp07";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
