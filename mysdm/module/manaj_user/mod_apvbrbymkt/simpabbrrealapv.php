    ...
    if ($pmodule=="approvebrquestbymkt" AND $pact=="simpanbrrealbymkt") {
        ...
        if (!empty($piddokt)) {
            ...
            $query = "UPDATE hrd.dokter SET spid='$pidspesial', tgllahir='0000-00-00', alamat1='$palamat', kota='$pkota', hp='$pnohape', "
                    . " nowa='$pnowea', norek_bank='$pidbank', norek_user='$pnorekening', norek_atas='$pnorekatasnama', updateby='$pidcard', updatedate=NOW() where dokterid='$piddokt' LIMIT 1";
            mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { $pketeksekusi="error update user"; mysqli_close($cnmy); exit; }
            ...
            if (!empty($ptgllahir)) {
                ...
                mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { $pketeksekusi="error update tgl. lahir user"; mysqli_close($cnmy); exit; }
            ...
            if ((INT)$ketemu_rek<=0) {
                ...
                mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { $pketeksekusi="error insert norekening"; mysqli_close($cnmy); exit; }
            }
