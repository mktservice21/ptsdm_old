...
if ($module=='spgmastergaji' AND $act=='input') {
    mysqli_query($cnmy, "DELETE FROM $dbname.t_spg_gaji WHERE DATE_FORMAT(periode,'%Y-%m')='$ptglnya' AND id_spg='$pidspg' AND $fcabang");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
    
    $query="INSERT INTO $dbname.t_spg_gaji (periode, id_spg, icabangid, alokid, gaji, umakan, sewakendaraan, pulsa, parkir, bbm, userid)VALUES"
            . "('$pbulan', '$pidspg' ,'$pcabang', '$palokid', '$pgaji', '$pmakan', '$psewa', '$ppulsa', '$pparkir', '$pbbm', '$_SESSION[IDCARD]')";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
    $berhasil="Berhasil disimpan";
}elseif ($module=='spgmastergaji' AND $act=='hapus') {
    mysqli_query($cnmy, "DELETE FROM $dbname.t_spg_gaji WHERE DATE_FORMAT(periode,'%Y-%m')='$ptglnya' AND id_spg='$pidspg' AND $fcabang");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
