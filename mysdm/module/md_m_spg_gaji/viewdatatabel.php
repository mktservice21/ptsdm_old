...
<?PHP
    ...
    $_SESSION['SPGMSTGJICAB']=$pidcabang;
    $_SESSION['SPGMSTGJTGL']=date("F Y", strtotime($date1));
    ...
    $query = "select * from dbmaster.t_spg_gaji_cabang where DATE_FORMAT(periode,'%Y%m')='$bulan' $fcabang";
    $tampilc= mysqli_query($cnmy, $query);
    $gc= mysqli_fetch_array($tampilc);
    $cpgaji=$gc['gaji'];
    $cpumakan=$gc['umakan'];
    $cpsewakendaraan=$gc['sewakendaraan'];
    $cppulsa=$gc['pulsa'];
    $cpparkir=$gc['parkir'];
    $cpbbm=$gc['bbm'];
    ...
    <div class='x_content'>
        <table id='datatable' class='datatable table nowrap table-striped table-bordered' width="100%">
            ...
            <tbody>
                <?PHP
                    $no=1;
                    $query = "select id_spg, nama, penempatan from MKT.spg where IFNULL(aktif,'') = 'Y' $fcabang ";
                    $query .=" ORDER BY nama";
                    ...
                    while ($sp= mysqli_fetch_array($tampil)) {
                        ...
                        $queryg="select * from dbmaster.t_spg_gaji WHERE id_spg='$pidspg' AND DATE_FORMAT(periode,'%Y%m')='$bulan' $fcabang";
                        ...
                        if (empty($pumakan)) $pumakan=0;
                        if (empty($psewakendaraan)) $psewakendaraan=0;
                        if (empty($ppulsa)) $ppulsa=0;
                        if (empty($pparkir)) $pparkir=0;
                        if (empty($pbbm)) $pbbm=0;
                        
                        if ( (INT)$pgaji==0 ) $pgaji=$cpgaji;
                        if ( (INT)$pumakan==0 ) $pumakan=$cpumakan;
                        if ( (INT)$psewakendaraan==0 ) $psewakendaraan=$cpsewakendaraan;
                        if ( (INT)$ppulsa==0 ) $ppulsa=$cppulsa;
                        if ( (INT)$pparkir==0 ) $pparkir=$cpparkir;
                        if ( (INT)$pbbm==0 ) $pbbm=$cpbbm;
                        ...
                        $fsimpan="'txtidspg$no', 'txtbulan$no', 'txtcabang$no', 'txtgaji$no', 'txtumkn$no', 'txtsewa$no', 'txtpulsa$no', 'txtparkir$no', 'txtbbm$no'";
                        ...
                        echo "<tr>";
                        echo "<td>$no</td>";
                        echo "<td>$pnmspg $finidspg $finbulan $fincabang</td>";
                        echo "<td>$ptempatspg</td>";
                        echo "<td>$fingaji</td>";
...
<script>
    ...
    function SimpanData(eact, aidspg, abulan, acabang, agaji, amakan, asewa, apulsa, aparkir, abbm)  {
        ...
        if (ok_) {
            ...
            if (r==true) {
                ...
                $.ajax({
                    type:"post",
                    url:"module/md_m_spg_gaji/aksi_spggaji.php?module="+module+"&act="+eact+"&idmenu="+idmenu,
                    data:"uidspg="+espg+"&ubulan="+ebulan+"&ucabang="+ecabang+"&ugaji="+egaji+"&umakan="+emakan+"&usewa="+esewa+"&upulsa="+epulsa+"&uparkir="+eparkir+"&ubbm="+ebbm,
