...
function CariDataSPGGajiTJ($bulan, $pidcabang, $pnoid, $status, $periodeins) {
    ...
    $tmp04 =" dbtemp.DSPGCR04_".$userid."_$now ";
        ...
        if ((INT)$status==1) {
            ...
            //$query = "select a.* from MKT.spg a WHERE 1=1 $fcabang $filspgnya";
            $query = "SELECT a.*, CAST(NULL as CHAR(1)) as id_zona FROM dbmaster.t_spg_data a WHERE DATE_FORMAT(a.periode,'%Y%m')='$bulan' $fcabang $filspgnya";
            $query = "create TEMPORARY table $tmp01 ($query)"; 
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }


            $query = "UPDATE $tmp01 a JOIN dbmaster.t_spg_gaji_area_zona b on a.icabangid=b.icabangid AND a.areaid=b.areaid SET a.id_zona=b.id_zona"; 
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }

            $query = "DELETE FROM $tmp01 WHERE IFNULL(id_zona,'')='' OR IFNULL(id_zona,0)='0'";
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }



            $query = "UPDATE dbmaster.t_spg_gaji_br0 a JOIN $tmp01 b on a.id_spg=b.id_spg SET "
                    . " a.icabangid=b.icabangid, a.alokid=b.alokid, a.areaid=b.areaid, a.jabatid=b.jabatid, a.id_zona=b.id_zona WHERE "
                    . " DATE_FORMAT(a.periode,'%Y%m')='$bulan' $fcabang $filspgnya"; 
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
    ...
    if ($status=="1") {
        $query = "UPDATE $tmp01 a SET a.insentif=(select sum(IFNULL(b.inct_total,0)-IFNULL(b.inct_booster,0)) FROM $tmp02 b WHERE a.id_spg=b.id_spg)";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
        
        $query = "UPDATE $tmp01 a SET a.insentif_tambahan=(select sum(IFNULL(b.inct_booster,0)) FROM $tmp02 b WHERE a.id_spg=b.id_spg)";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
    ...
    //if ((double)$pjmlkerja>0) {
		...
		//ASPR 1.	Biaya Sewa Kendaraan 2.	Uang Pulsa 3.	BBM  4.	Parkir
		//dihitung FULL per 22 OKTOBER 2021, email UTAMI

        $query= "UPDATE $tmp01 SET ntunjangan=IFNULL(tot_tujangan,0) WHERE jabatid IN ('003')";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
		...
		//======================================
            ...
            //RUMUS >= JULY 2021, SESUAI SURAT DAN EMAIL 26 07 2021 - SELAIN ASPR 
        ...
        // hilangkan AND jabatid NOT IN ('003') per 22 oktober 2021
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
...
function CariDataSPGBR($dari, $bulan, $pidcabang, $pnoid, $status) {
    ...
    $query = "SELECT
        ...
        a.keterangan,
        a.sts, a.apvtgl3,
        a.total insentif, a.total insentif_tambahan, a.total lebihkurang, a.total gaji, a.keterangan hk, a.total rpmakan, a.total makan, a.total sewa, a.total pulsa, a.total bbm, a.total parkir,
        a.total rinsentif, a.total rinsentif_tambahan, a.total rlebihkurang, a.total rgaji, a.total rmakan, a.total rsewa, a.total rpulsa, a.total rbbm, a.total rparkir,
        a.total sinsentif, a.total sinsentif_tambahan, a.total slebihkurang, a.total sgaji, a.total smakan, a.total ssewa, a.total spulsa, a.total sbbm, a.total sparkir
        FROM
    ...
    mysqli_query($cnmy, "UPDATE $tmp01 SET insentif=0, insentif_tambahan=0, lebihkurang=0, gaji=0, hk='', rpmakan=0, makan=0, sewa=0, pulsa=0, bbm=0, parkir=0");
    mysqli_query($cnmy, "UPDATE $tmp01 SET rinsentif=0, rinsentif_tambahan=0, rlebihkurang=0, rgaji=0, rmakan=0, rsewa=0, rpulsa=0, rbbm=0, rparkir=0");//ralisasi
    mysqli_query($cnmy, "UPDATE $tmp01 SET sinsentif=0, sinsentif_tambahan=0, slebihkurang=0, sgaji=0, smakan=0, ssewa=0, spulsa=0, sbbm=0, sparkir=0");//selisih
