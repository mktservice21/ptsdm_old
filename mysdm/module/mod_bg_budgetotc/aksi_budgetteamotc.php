...
// Hapus menu
if ($module=='entrydatabudgetotc' AND $act=='hapus'){
    $pid=$_GET['id'];
    mysqli_query($cnmy, "UPDATE dbmaster.t_budget_otc SET stsnonaktif='Y' WHERE idbudget='$pid'");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
    header('location:../../media.php?module='.$module.'&idmenu='.$idmenu.'&act=complete');
}

// Input modul
elseif ($module=='entrydatabudgetotc'){
    $pid=$_POST['e_id'];
    ...
    if ($act=='input') {
        ...
        $sql=  mysqli_query($cnmy, "select idbudget from dbmaster.t_budget_otc WHERE kodeid='$pkodeid' and tahun='$ptahun' AND g_divisi='$pdivisi'");
        ...
        //mysqli_query($cnmy, "delete from dbmaster.t_budget_otc WHERE kodeid='$pkodeid' and tahun='$ptahun' AND g_divisi='$pdivisi'");
        mysqli_query($cnmy, "insert into dbmaster.t_budget_otc (tglinput, kodeid, tahun, g_divisi, userid, jumlah) values"
        ...
        mysqli_query($cnmy, "UPDATE dbmaster.t_budget_otc SET kodeid='$pkodeid', g_divisi='$pdivisi', jumlah='$pjumlah' WHERE idbudget='$pid'");
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
    }
    ...
    for ($k=0;$k<count($ketbln);$k++) {
        ...
        if (!empty($nupdate)) {
            mysqli_query($cnmy, "UPDATE dbmaster.t_budget_otc SET $nfield='$prp' WHERE $nupdate");
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
        }
        //echo "$k. $prp<br/>";
    }
