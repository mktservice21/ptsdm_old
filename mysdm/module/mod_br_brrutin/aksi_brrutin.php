...
elseif ($module=='entrybrrutin')
{
    ...
    $pidcab_drnew="";
    $pidarea_drnew="";
    $pblncari_00 = date('Ym', strtotime('-2 month', strtotime($date1)));
    $pblncari_01 =  date("Ym", strtotime($date1));
	
    if ($pdivprodid!="OTC") {
        if ((int)$pjabatanid==10 OR (int)$pjabatanid==18 OR (int)$pjabatanid==8 OR (int)$pjabatanid==20 OR (int)$pjabatanid==15 ) {
            
            $query = "select bulan, icabangid as icabangid, areaid as areaid, divisi as divisi from ms.t_penempatan_marketing_auto "
                    . " where karyawanid='$pkaryawan' AND jabatanid='$pjabatanid' AND bulan BETWEEN '$pblncari_00' AND '$pblncari_01' "
                    . " ORDER BY bulan desc";
            
            $tampilp= mysqli_query($cnmy, $query);
            $prow= mysqli_fetch_array($tampilp);
            $pdivisi_drnew=$prow['divisi'];
            $pidcab_drnew=$prow['icabangid'];
            $pidarea_drnew=$prow['areaid'];
            
            if ($pdivisi_drnew=="undefined") $pdivisi_drnew="";
            if ($pidcab_drnew=="undefined") $pidcab_drnew="";
            if ($pidarea_drnew=="undefined") $pidarea_drnew="";
            
            
            
            if ( (int)$pjabatanid==15 ) {
                if (!empty($pdivisi_drnew)) $pdivprodid=$pdivisi_drnew;
                if (!empty($pidcab_drnew)) $pidcabang=$pidcab_drnew;
                if (!empty($pidarea_drnew)) $pareaid=$pidarea_drnew;
            }elseif ( (int)$pjabatanid==10 OR (int)$pjabatanid==18 ) {
                $pdivprodid="CAN";
                if (!empty($pidcab_drnew)) $pidcabang=$pidcab_drnew;
                if (!empty($pidarea_drnew)) $pareaid=$pidarea_drnew;
            }
            
        }elseif ( (int)$pjabatanid==38 ) {
            $query = "select icabangid as icabangid, areaid as areaid from hrd.karyawan WHERE karyawanid='$pkaryawan'";
            $tampilp= mysqli_query($cnmy, $query);
            $prow= mysqli_fetch_array($tampilp);
            $pdivisi_drnew="HO";
            $pidcab_drnew=$prow['icabangid'];
			...
			if (!empty($pidcab_drnew) AND empty($pidarea_drnew)) $pidarea_drnew = getfieldcnmy ("select areaid as lcfields from MKT.iarea where icabangid='$pidcab_drnew' and ifnull(aktif,'')<>'N'");
        ...
        if ((int)$pjabatanid==5 OR $pjabatanid=="05") {
            ...
            mysqli_query($cnmy, $query);
					...
					if ($phusus_karyawan==true) {
							$query = "update $dbname.t_brrutin0 set tgl_atasan3=NOW() WHERE idrutin='$kodenya'";
							mysqli_query($cnmy, $query);
					}
	
	
                                        
    if ($pkaryawan=="0000000184" OR $pkaryawan=="0000001164" OR $pkaryawan=="0000002329") {
