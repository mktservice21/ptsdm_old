...
if ($module=='closingbrlkcaotc' AND $act=='input') {
    if (!empty($pnourt)) {
        ...
        */
        ...
        $pidinputdiv=$_POST['uidinputdiv'];
        $pnodivid=$_POST['uidindivno'];
        $pjmltrf=$_POST['ujmltrsf'];
        if (empty($pjmltrf)) $pjmltrf=0;
        $pjmltrf=str_replace(",","", $pjmltrf);
        
        $query = "UPDATE dbmaster.t_suratdana_br SET jumlah3='$pjmltrf' WHERE IFNULL(stsnonaktif,'') <> 'Y' AND divisi ='OTC' 
            AND CONCAT(kodeid,subkode)='221' AND idinput='$pidinputdiv' AND nodivisi='$pnodivid' LIMIT 1";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
