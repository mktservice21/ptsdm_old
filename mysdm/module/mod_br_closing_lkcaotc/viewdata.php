...
$query = "select idinput, nodivisi from dbmaster.t_suratdana_br WHERE IFNULL(stsnonaktif,'') <> 'Y' AND divisi ='OTC' 
    AND CONCAT(kodeid,subkode)='221' AND DATE_FORMAT(tglf,'%Y-%m')='$periode1' LIMIT 1";
$tampil=mysqli_query($cnmy, $query);
$ketemu= mysqli_num_rows($tampil);
if ($ketemu>0){
    $nr= mysqli_fetch_array($tampil);

    $pidinputdiv=$nr['idinput'];
    $pnoiddivisi=$nr['nodivisi'];

    $query = "select idinputbank, nobukti, tanggal from dbmaster.t_suratdana_bank where idinput='$pidinputdiv' AND stsinput='K' AND IFNULL(stsnonaktif,'')<>'Y'";
    ...
    <div class='x_content'>
        <div class='col-md-12 col-sm-12 col-xs-12'>
            ...
            <div class='form-group'>
                ...
                <div class='col-xs-3'>
                    <input type='hidden' id='txt_idinputdiv' name='txt_idinputdiv' class='form-control col-md-7 col-xs-12' value='<?PHP echo $pidinputdiv; ?>' Readonly>
                    <input type='text' id='txt_idnodiv' name='txt_idnodiv' class='form-control col-md-7 col-xs-12' value='<?PHP echo $pnoiddivisi; ?>' Readonly>
                </div>
            </div>
