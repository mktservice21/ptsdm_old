<?php
    ...
    $fstsapv = "";//khusus OTC belum ada approve
    ...
    if (!empty($filterdarispd)) {
        $query = "select * from dbmaster.t_brrutin_ca_close_otc WHERE idinput IN $filterdarispd";
    }else{
        $query = "select * from dbmaster.t_brrutin_ca_close_otc WHERE DATE_FORMAT(bulan,'%Y-%m')='$periode1' $filterstsclose $filterdivisi";
    }
    ...
    if ($ketemu==0){
        ...
        $query = "select idca2 from dbmaster.t_brrutin_ca_close_otc WHERE ifnull(idca2,'') <> '' AND DATE_FORMAT(bulan,'%Y-%m')='$periodesebelumnya' $filterdivisi";
        //echo "$query<br/>";
        ...
        $query = "select DISTINCT idrutin, 'LK1', sts ket from dbmaster.t_brrutin_ca_close_otc WHERE ifnull(idrutin,'') <> '' AND DATE_FORMAT(bulan,'%Y-%m')='$periode1' $filterstsclose $filterdivisi";
        $query .= " UNION select idca1, 'CA1', sts ket from dbmaster.t_brrutin_ca_close_otc WHERE ifnull(idca1,'') <> '' AND DATE_FORMAT(bulan,'%Y-%m')='$periode1' $filterstsclose $filterdivisi";
        $query .= " UNION select idca2, 'CA2', sts ket from dbmaster.t_brrutin_ca_close_otc WHERE ifnull(idca2,'') <> '' AND DATE_FORMAT(bulan,'%Y-%m')='$periode1' $filterstsclose $filterdivisi";
        ...
        //RUTIN
        $query = "select idrutin, karyawanid, divisi, icabangid, areaid, keterangan, tgltrans, nobukti, jumlah, 
            nama_karyawan nama_kontrak, atasan1, atasan2, atasan3, atasan4 from dbmaster.t_brrutin0 
        ...
        //CA 1
        $query = "select idca, karyawanid, divisi, icabangid, areaid, keterangan, tgltrans, jumlah,
            nama_karyawan nama_kontrak, atasan1, atasan2, atasan3, atasan4 from dbmaster.t_ca0 
        ...
        $query = "DELETE FROM $tmp03 WHERE ifnull(idca,'') NOT IN (select distinct ifnull(idca2,'') FROM $tmp09)";
        //mysqli_query($cnit, $query);
        //mysqli_query($cnit, "drop TEMPORARY table $tmp09");
        
        
        //CA 2
        $query = "select idca, karyawanid, divisi, icabangid, areaid, keterangan, jumlah,
            nama_karyawan nama_kontrak, atasan1, atasan2, atasan3, atasan4 from dbmaster.t_ca0 
                WHERE jenis_ca='lk' AND stsnonaktif <> 'Y' and divisi='OTC' AND 
        ...
        $query = "update $tmp01 a JOIN dbmaster.t_karyawan_kontrak b ON a.nama_kontrak=b.nama AND "
        ...
        $query = "update $tmp03 a JOIN dbmaster.t_karyawan_kontrak b ON a.nama_kontrak=b.nama AND "
                . " a.atasan1=b.atasan1 AND a.atasan2=b.atasan2 AND a.atasan3=b.atasan3 AND a.atasan4=b.atasan4 "
                . " SET a.karyawanid=b.id WHERE a.karyawanid='0000002200'";
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }

        $query = "update $tmp04 a JOIN dbmaster.t_karyawan_kontrak b ON a.nama_kontrak=b.nama AND "
                . " a.atasan1=b.atasan1 AND a.atasan2=b.atasan2 AND a.atasan3=b.atasan3 AND a.atasan4=b.atasan4 "
                . " SET a.karyawanid=b.id WHERE a.karyawanid='0000002200'";
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
        ...
        $query = "select distinct divisi, idrutin, karyawanid, keterangan, nama_kontrak, atasan1, atasan2, atasan3, atasan4 from $tmp01";
        $query = "create TEMPORARY table $tmp06 ($query)"; 
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }

        //insert data karyawan dari ca 1
        $query = "INSERT INTO $tmp06 (karyawanid, divisi, keterangan, nama_kontrak, atasan1, atasan2, atasan3, atasan4)"
                . " select DISTINCT a.karyawanid, a.divisi, a.keterangan, a.nama_kontrak, a.atasan1, a.atasan2, a.atasan3, a.atasan4 FROM $tmp03 as a WHERE "
        ...
        //insert data karyawan dari ca 1
        $query = "INSERT INTO $tmp06 (karyawanid, divisi, keterangan, nama_kontrak, atasan1, atasan2, atasan3, atasan4)"
                . " select DISTINCT a.karyawanid, a.divisi, a.keterangan, a.nama_kontrak, a.atasan1, a.atasan2, a.atasan3, a.atasan4 FROM $tmp04 as a WHERE "
        ...
        $query = "update $tmp06 set nama_karyawan=nama_kontrak WHERE IFNULL(nama_karyawan,'')=''"; 
        mysqli_query($cnit, $query);
        ...
        $query = "select distinct karyawanid, saldo, ca1, (IFNULL(ca1,0)-IFNULL(saldo,0)) selisih, CAST(0 as DECIMAL(20,2)) as kembali_rp from dbmaster.t_brrutin_ca_close_otc WHERE (IFNULL(ca1,0)-IFNULL(saldo,0)) > 0 AND DATE_FORMAT(bulan,'%Y-%m')='$periodesebelumnya' $filterdivisi";
        ...
        if (isset($_GET['module'])) {
            if ($_GET['module']=="realisasibl" OR $_GET['module']=="realisasiblotc" OR $_GET['module']=="glrekapbrluarkota") $hapustmp2=false;
        }
        ...
        $query = "select a.divisi, a.idrutin, a.karyawanid, b.nama as nama_karyawan, a.credit, 
            ...
            from $tmp08 a LEFT JOIN hrd.karyawan b on a.karyawanid=b.karyawanId ";
        $query = "create TEMPORARY table $tmp01 ($query)"; 
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
        ...
        $query = "UPDATE $tmp01 a SET a.nama_karyawan=(select b.nama FROM dbmaster.t_karyawan_kontrak b WHERE a.karyawanid=b.id) WHERE "
                . " IFNULL(nama_karyawan,'')=''";
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
        ...
        if (isset($_GET['module'])) {
            if ($_GET['module']=="realisasibl" OR $_GET['module']=="realisasiblotc" OR $_GET['module']=="glrekapbrluarkota") {
