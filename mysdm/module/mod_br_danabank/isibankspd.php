<?PHP
    ...
    if ($pses_grpuser=="1" OR $pses_grpuser=="24" OR $pses_grpuser=="25") {
        if ($pket_input=="2" AND $pses_grpuser=="25") {
            $n_filterkaryawan=" AND a.divisi<>'OTC' AND ( a.karyawanid='$pses_idcard' OR a.subkode='25' ) ";
        } 
    }else{
        if ($pses_divisi=="OTC") {
    ...
    $fil_keluar_=" DATE_FORMAT(tglspd,'%Y%m') between '$periode1' AND '$periode2' ";
    if ($pket_input==2) {
        $fil_keluar_ = "(
                    (DATE_FORMAT(tglspd,'%Y%m') between '$periode1' AND '$periode2')
                        OR
                        (
                        (DATE_FORMAT(tgl,'%Y%m') between '$periode1' AND '$periode2') AND ( a.subkode IN ('22', '23', '36') OR a.pilih='N')
                        )
                    )";
    }
    
    $query = "select a.*, a.kodeid as nkodeid, a.subkode as nsubkode from dbmaster.t_suratdana_br a WHERE IFNULL(a.stsnonaktif,'') <> 'Y' AND $fil_keluar_  $n_filterkaryawan ";
	if ($pses_idcard=="0000000143") $query .= " AND a.kodeid<>'3' ";
    $query = "create TEMPORARY table $tmp08 ($query)";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    $query = "UPDATE $tmp08 SET kodeid=kodeid2, subkode=subkode2, divisi=divisi2, nodivisi=nodivisi2 WHERE kodeid='3'";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    //and a.pilih='Y' 
    $query = "select CAST('' as char(1)) as sdh, a.idinput, a.tgl, a.tglspd, a.kodeid, b.nama, a.subkode, b.subnama, a.divisi, 
        IFNULL(a.nomor,'') nomor, a.nodivisi, a.nodivisi2, a.jumlah, a.jumlah3, CAST(NULL as DECIMAL(20,2)) as jml_adj,
        a.kodeid2, a.subkode2, a.divisi2, a.nkodeid, a.nsubkode
        from $tmp08 a JOIN dbmaster.t_kode_spd b on a.kodeid=b.kodeid and a.subkode=b.subkode
        ";//WHERE IFNULL(a.stsnonaktif,'') <> 'Y' AND $fil_keluar_  $n_filterkaryawan
    //echo $query;
    $query = "create TEMPORARY table $tmp01 ($query)";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

    $query = "UPDATE $tmp01 SET kodeid=kodeid2, subkode=subkode2, divisi=divisi2, nodivisi=nodivisi2 WHERE kodeid='3'";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    //ubah jumlah menjadi jumlah3=jumlah transfer, khusus LK
    if ($pket_input==2) {
        $query = "UPDATE $tmp01 a SET a.jumlah=jumlah3 WHERE nsubkode='21' AND nkodeid='2'";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    }
    
    $query = "select * from dbmaster.t_suratdana_br1 WHERE idinput in (select distinct idinput from $tmp01 WHERE IFNULL(nodivisi,'')<>'')";
    $query = "create TEMPORARY table $tmp05 ($query)";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
       
    $query = "select b.nodivisi, b.nodivisi2, sum(a.jml_adj) as jml_adj FROM $tmp05 a JOIN $tmp01 b on a.idinput=b.idinput GROUP BY 1";
    $query = "create TEMPORARY table $tmp06 ($query)";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    $query = "DELETE FROM $tmp06 WHERE IFNULL(jml_adj,0)=0";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    //$query = "UPDATE $tmp06 SET jml_adj=0-IFNULL(jml_adj,0) WHERE IFNULL(jml_adj,0)>0";
    //mysqli_query($cnmy, $query);
    //$erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    
    $query = "UPDATE $tmp01 a SET a.jml_adj=(select b.jml_adj FROM $tmp06 b WHERE a.nodivisi=b.nodivisi) WHERE nodivisi IN "
            . "(select IFNULL(nodivisi,'') FROM $tmp07)";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    
    $query = "UPDATE $tmp01 a SET a.jumlah=IFNULL(jumlah,0)+IFNULL(jml_adj,0)";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    $query = "SELECT kodeid, nama, subkode, subnama, divisi, tglspd, nomor, nodivisi, jumlah, nodivisi2 FROM $tmp01 LIMIT 1";
    $query = "create TEMPORARY table $tmp03 ($query)";
    mysqli_query($cnmy, $query);
    mysqli_query($cnmy, "ALTER TABLE $tmp03 ADD nourut INT(4), ADD jml_adj DECIMAL(20,2)");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    mysqli_query($cnmy, "DELETE FROM $tmp03");
    
    if ($pket_input=="2") {//perubahan untuk staus N jadi bank keluar 
        mysqli_query($cnmy, "UPDATE $tmp01 set sdh='1' WHERE idinput in (select distinct idinput from dbmaster.t_suratdana_bank WHERE "
                . " ifnull(stsnonaktif,'')<>'Y' AND ( IFNULL(stsinput,'')='K' ) )");//OR (IFNULL(stsinput,'')='N' AND IFNULL(parentidbank,'')<>'') 
    }else{
        mysqli_query($cnmy, "UPDATE $tmp01 set sdh='1' WHERE idinput in (select distinct idinput from dbmaster.t_suratdana_bank WHERE "
                . " ifnull(stsnonaktif,'')<>'Y' AND IFNULL(stsinput,'')='N')");
    }
    $x=1;
    ...
    while ($r= mysqli_fetch_array($sql)) {
        ...
        $query = "INSERT INTO $tmp03 (nourut, kodeid, nama, subkode, subnama, divisi, tglspd, nomor, nodivisi, jumlah, nodivisi2)"
                . "SELECT $x nourut, kodeid, nama, subkode, subnama, divisi, tglspd, nomor, nodivisi, jumlah, nodivisi2 FROM $tmp01 WHERE "
...
<form method='POST' action='<?PHP echo "?module='saldosuratdana'&act=input&idmenu=258"; ?>' id='d-form2' name='form2' data-parsley-validate class='form-horizontal form-label-left'>
    ...
    <?PHP
                            ...
                            if ($ketemu>0) {
                                while ($row6= mysqli_fetch_array($tampil6)) {
                                    ...
                                    if ( ($pkodeid=="2" AND ($pnsubdiv=="23x" OR $pnsubdiv=="22x"))) {
                                        ...
                                        echo "<td nowrap><a href='eksekusi3.php?module=glreportspddetail&act=input&idmenu=258&ket=bukan&divisi=$n_div&nodivisi=$ndivisino&idinspd=$pnidinput' target='_blank'>$ndivisino</a></td>";
                                    }
        ...
        $query = "select tglspd, nomor, sum(jumlah) jumlah FROM $tmp01 where nkodeid NOT IN  ('3') group by 1,2 order by nomor, tglspd";
        ...
        if ($pket_input!="2") {
            // total adjusment
            $query = "select distinct tglspd, nomor FROM $tmp01 where nkodeid NOT IN  ('3') order by nomor, tglspd";
            $tampilsmr=mysqli_query($cnmy, $query);
            echo "<tr>";
            ...
            while ($smr= mysqli_fetch_array($tampilsmr)) {
                ...
                $query = "select tglspd, nomor, sum(jumlah) jumlah FROM $tmp01 where nomor='$pnospd_smr' AND nkodeid IN ('3') group by 1,2 order by nomor, tglspd";
