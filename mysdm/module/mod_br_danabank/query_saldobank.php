...
function seleksi_query_bank($nkodeneksi, $pperiode){
    ...
    $query = "select a.nobukti, a.idservice, a.tanggal as tgl_kembali, a.jumlah kembali_rp, a.coa, "
    ...
    $query = "INSERT INTO $tmp01 (stsinput, idinputbank, tanggal, coa4, 
        kodeid, subkode, divisi, keterangan, nodivisi, jumlah, sts, nobukti)"
            ...
            . " '' as nodivisi, kembali_rp, '1' sts, nobukti FROM $tmp03";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    $query = "SELECT a.*, b.subnama, c.nama nama_user, b.igroup, b.inama FROM $tmp04 a LEFT JOIN dbmaster.t_kode_spd b on a.subkode=b.subkode "
