<?PHP
    ...
    $sql = "SELECT a.nodivisi, a.nomor, a.stsinput, a.idinputbank, DATE_FORMAT(a.tanggal,'%d %M %Y') as tanggal, a.kodeid, a.subkode, b.subnama, b.ibank, "
            ...
            . " a.keterangan, a.userid, a.brid, a.noslip, a.parentidbank, a.realisasi, a.customer, a.aktivitas1 ";
    ...
    if ($pses_grpuser=="1" OR $pses_grpuser=="24" OR $pses_grpuser=="25") {// OR $pses_grpuser=="25" anne
        ...
        //$sql.=" AND CONCAT(IFNULL(a.nomor,''),IFNULL(a.nodivisi,'')) IN (SELECT CONCAT(IFNULL(nomor,''),IFNULL(nodivisi,'')) FROM dbmaster.t_suratdana_br WHERE karyawanid='$pses_idcard')";
        $sql.=" AND (CONCAT(IFNULL(a.nomor,''),IFNULL(a.nodivisi,'')) IN (SELECT CONCAT(IFNULL(nomor,''),IFNULL(nodivisi,'')) FROM dbmaster.t_suratdana_br WHERE "
                . " karyawanid='$pses_idcard') OR (userid='$pses_idcard' AND IFNULL(stsnonaktif,'')<>'Y') ) ";
    }
    $query = "create TEMPORARY table $tmp01 ($sql)"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
    
    $query = "select a.*, a.tanggal as tgltrans, a.realisasi as nmrealisasi, a.customer as nama_dokter, d.nama nama_user "
            . " from $tmp01 a LEFT JOIN hrd.karyawan d on a.userid=d.karyawanId";
...
<script>
    function ProsesData(ket, noid){
        ...
        if (ok_) {
            ...
            if (r==true) {
                ...
                if (ket=="transferulang") {

                    var textket = prompt("No SLIP BARU : ", "");
                    txt = textket;
                ...
                document.getElementById("d-form2").action = "module/mod_br_danabank/aksi_danabank.php?module="+module+"&idmenu="+idmenu+"&act="+ket+"&kethapus="+"&ket="+ket+"&id="+noid+"&utxt="+txt;
    ...
    function getInputDataTU(didinput){
        $.ajax({
            type:"post",
            url:"module/mod_br_danabank/tambah_bank_tu.php?module=transferulang",
...
<form method='POST' action='<?PHP echo "?module='saldosuratdana'&act=input&idmenu=258"; ?>' id='d-form2' name='form2' data-parsley-validate class='form-horizontal form-label-left'>
    ...
    <div class='x_content'>
        <table id='datatablednbank' class='table table-striped table-bordered' width='100%'>
            ...
            <?PHP
                $no=1;
                $query = "select * from $tmp05 order by idinputbank DESC";
                ...
                while( $row=mysqli_fetch_array($query) ) {  // preparing an array
                    ...
                    $nbtnhapus = "<input type='button' value='Hapus' class='btn btn-danger btn-xs' onClick=\"ProsesData('hapus', '$idno')\">";
                    $nbutton = ""
                    ...
                    $nbtntrans="<button type='button' class='btn btn-info btn-xs' title='Transfer Ualng' data-toggle='modal' "
                            . " data-target='#myModal' "
                            . " onClick=\"getInputDataTU('$idno')\">Transfer</button>";
                                    
                    //$nbtntrans="<input type='button' value='Transfer' class='btn btn-info btn-xs' onClick=\"ProsesData('transferulang', '$idno')\">";
                    if (!empty($nparentidb)) $nbtntrans="";
                    if ($pstsinput=="K" OR $pstsinput=="M" OR $pstsinput=="N") $nbtntrans="";
                        
                    $nbutton=$nbutton."".$nbtntrans;
                    ...
                    if ($psubkodeibank=="Y" AND $puserid==$pses_idcard) {
                        ...
                        if ( ($pstsinput=="K" OR $pstsinput=="M") AND $pses_grpuser!="1" ) {
                            if (($pses_grpuser=="1" OR $pses_grpuser=="25") AND empty($psubkode)) {
                    ...
                    if ( ($pstsinput=="M" OR $pstsinput=="N") AND $pses_grpuser!="1") {
                    ...
                    if ($pstsinput=="T" AND empty($pket)) {
                        $pket = "Transfer Ulang";
                    }
                    
                    if ($pstsinput=="N" AND empty($pket)) {
                        $pket = "Minta Dana";
                    }
