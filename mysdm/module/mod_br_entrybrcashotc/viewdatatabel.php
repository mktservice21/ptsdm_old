<?PHP
    ...
    //$_SESSION['FINCAATGLTIPEOTC']=$_POST['utgltipe'];
    $_SESSION['FINCAAPERENTYOTC1']=$_POST['uperiode1'];
    $_SESSION['FINCAAPERENTYOTC2']=$_POST['uperiode2'];
    //$_SESSION['FINCAADIVOTC']=$_POST['udivisi'];
    //$_SESSION['FINCAACABOTC']=$_POST['uarea'];
...
<form method='POST' action='<?PHP echo "?module='$pmodule'&act=input&idmenu=$pidmenu"; ?>' id='d-form2' name='form2' data-parsley-validate class='form-horizontal form-label-left'>
    ...
    <div class='x_content'>
        <table id='datatablecaotc' class='table table-striped table-bordered' width='100%'>
            <thead>
                <tr>
                    <th width='7px'>No</th>
            ...
            <tbody>
                ...
                $query = "SELECT
                    ...
                    br.userid AS userid
                    FROM dbmaster.t_ca0 br JOIN hrd.karyawan k ON br.karyawanid = k.karyawanId ";
                
                
                $sql = "SELECT divisi, idca, DATE_FORMAT(tgl,'%d/%m/%Y') as tgl, DATE_FORMAT(bulan,'%M %Y') as bulan, "
                        . " DATE_FORMAT(periode,'%M %Y') as periode, "
                        . " divisi, karyawanid, nama, areaid, FORMAT(jumlah,0,'de_DE') as jumlah, "
...
<script>
    $(document).ready(function() {
        var dataTable = $('#datatablecaotc').DataTable( {
            //"stateSave": true,
            //"order": [[ 2, "desc" ]],
    ...
    function ProsesData(ket, noid){
        ...
        if (ok_) {
            ...
            if (r==true) {
                ...
                document.getElementById("d-form2").action = "module/mod_br_entrybrcashotc/aksi_entrybrcashotc.php?module="+module+"&act=hapus&idmenu="+idmenu+"&kethapus="+txt+"&ket="+ket+"&id="+noid;
