<?php
    ...
    $_SESSION['FINCAAPERENTYOTC1']=$pdate1;
    $_SESSION['FINCAAPERENTYOTC2']=$pdate2;
    ...
    $query = "SELECT userid, idca, tgl, periode, "
            . " divisi, karyawanid, icabangid_o icabangid, areaid_o areaid, jumlah, keterangan, "
            . " jabatanid, tgl_atasan1, tgl_atasan2, tgl_atasan3, tgl_atasan4, validate, fin, tgl_fin, nama_karyawan, jenis_ca ";
    $query .=" FROM dbmaster.t_ca0 WHERE IFNULL(stsnonaktif,'')<>'Y' AND "
            . " ( (tgl BETWEEN '$ptgl1' AND '$ptgl2') OR (periode BETWEEN '$ptgl1' AND '$ptgl2') ) ";
    ...
    $query = "select distinct a.bridinput from dbmaster.t_suratdana_br1 a JOIN dbmaster.t_suratdana_br b on "
            ...
            . " bridinput IN (select distinct IFNULL(idca,'') FROM $tmp02)";
    ...
    $query = "select distinct idca from dbimages.img_ca0 WHERE idca IN (select distinct IFNULL(idca,'') FROM $tmp02)";
    $query = "create TEMPORARY table $tmp04 ($query)"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    //rincian
    $query = "select idca, sum(rptotal) as rptotal from dbmaster.t_ca1 WHERE idca IN (select distinct IFNULL(idca,'') FROM $tmp02) GROUP BY 1";
    $query = "create TEMPORARY table $tmp05 ($query)"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    
    $query = "UPDATE $tmp02 a JOIN $tmp03 b on a.divisi=b.divisi AND DATE_FORMAT(a.periode,'%Y%m')=b.bulan SET a.ssudah='C'"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    $query = "UPDATE $tmp02 a JOIN $tmp04 b on a.idca=b.idca SET a.adabukti='Y'"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    $query = "UPDATE $tmp02 a JOIN $tmp05 b on a.idca=b.idca SET a.rptotal=b.rptotal"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
...
<form method='POST' action='<?PHP echo "?module='$pmodule'&act=input&idmenu=$pidmenu"; ?>' id='d-form2' name='form2' data-parsley-validate class='form-horizontal form-label-left'>
    ...
    <div class='x_content'>
        <table id='datatablecaotcho' class='table table-striped table-bordered' width='100%'>
            <thead>
                <tr>
                    <th width='7px'>No</th>
                    <th width='70px'></th>
...
<script>
    $(document).ready(function() {
        var dataTable = $('#datatablecaotcho').DataTable( {
            //"stateSave": true,
            //"order": [[ 2, "desc" ]],
    ...
    function ProsesDataHapusCa(ket, noid){
        ...
        if (ok_) {
            ...
            if (r==true) {
                ...
                document.getElementById("d-form2").action = "module/mod_br_entrybrcashotcho/aksi_entrybrcashotcho.php?module="+module+"&act=hapus&idmenu="+idmenu+"&kethapus="+txt+"&ket="+ket+"&id="+noid;
