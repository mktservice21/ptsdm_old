...
$sql = "SELECT divisi, idrutin, DATE_FORMAT(tgl,'%d/%m/%Y') as tgl, DATE_FORMAT(bulan,'%M %Y') as bulan, DATE_FORMAT(periode1,'%d/%m/%Y') as periode1, "
        . " DATE_FORMAT(periode2,'%d/%m/%Y') as periode2, "
        . " divisi, karyawanid, nama, areaid, FORMAT(jumlah,0,'de_DE') as jumlah, keterangan, jabatanid, tgl_atasan1, tgl_atasan2, tgl_atasan3, tgl_atasan4, tgl_fin, nama_karyawan ";
...
if ($_SESSION['IDCARD']=="0000000825" OR $_SESSION['IDCARD']=="0000001587" OR $_SESSION['IDCARD']=="0000002329") {
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
    $nestedData=array();
    $idno=$row['idrutin'];
    $pnkaryawanid=$row["karyawanid"];
    $nama=$row["nama"];
    
    if ($_SESSION['KRYNONE']==$pnkaryawanid) $nama=$row["nama_karyawan"];
