<?php
    ...
    if ($module=="entrybrdcc" AND $act=="inputdatapajak" OR $act=="editdata") {
        ...
        if (empty($pidinput) AND $act=="inputdatapajak") {
            ...
            if ($ketemu==0 AND $pcbpajak=="Y") {
                ...
                $query = "UPDATE hrd.br0 SET pajak='Y', nama_pengusaha='$pekenapajak', noseri='$penoserifp', tgl_fp=$petglfp'', "
                        ...
                        . "brId='$pbrid' AND pajak<>'Y' LIMIT 1";

                $query = "UPDATE hrd.br0 SET pajak='Y' WHERE brId='$pbrid' AND pajak<>'Y' LIMIT 1";
        ...
        if (empty($pidinput) AND $act=="inputdatapajak") {
            ...
            $query = "UPDATE dbmaster.t_br0_pajak SET nama_pengusaha='$pekenapajak', noseri='$penoserifp', tgl_fp='$petglfp', "
                    ...
                    . " pph_jns='$pcbpph', pph='$pejmlpph', pph_rp='$pejmlrppph', jumlah='$pejmlusulan', pembulatan='$pejmlbulat', materai_rp='$pejmlmaterai' $fieldjasa WHERE idinput='$pidinput' AND brId='$pbrid' LIMIT 1";
        }
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo "$erropesan : br eth pajak"; exit; }
            
        
        $query = "UPDATE hrd.br0 SET jumlah='$ptotjmlminta' WHERE brId='$pbrid' LIMIT 1";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo "$erropesan : br eth"; exit; }
        ...
        $berhasil="";
    }elseif ($module=="entrybrdcc" AND $act=="hapus") {
        $pidinput=$_POST['uid'];
        
        if ($pidinput=="0" OR (double)$pidinput==0) {
            
        }else{
            $query = "select brid, jumlah FROM dbmaster.t_br0_pajak WHERE idinput='$pidinput'";
            $tampil= mysqli_query($cnmy, $query);
            ...
            $query = "select sum(jumlah) as jumlahrp FROM dbmaster.t_br0_pajak WHERE idinput<>'$pidinput' AND brid='$nbrid'";
            ...
            $query = "select jumlah as jumlahusul FROM hrd.br0 WHERE brid='$nbrid'";
            ...
            $query = "DELETE FROM dbmaster.t_br0_pajak WHERE idinput='$pidinput' LIMIT 1";
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo "$erropesan : hapus br eth"; exit; }
            
            
            if (!empty($nbrid)) {
                $query = "UPDATE hrd.br0 SET jumlah='$ntotalusul' WHERE brId='$nbrid' LIMIT 1";
                mysqli_query($cnmy, $query);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo "$erropesan : br eth"; exit; }
            }
