...
elseif ($module=='entrybrklaim' AND $act=='editnonuser')
{
    ...
    $date2="";
    if (!empty($_POST['e_tgltrans'])) $date2 = str_replace('/', '-', $_POST['e_tgltrans']);
    ...
    $plampiran= "";
    if (isset($_POST['cx_lapir'])) $plampiran=$_POST['cx_lapir'];
    
    //echo "$kodenya, $pjnspajak, $pnmpengusaha, $pnoseri, $ptglfp, real : $pnmrealisasi, $pnoslip, tgltrans : $ptgltras ($plampiran)<br/>ket : $paktivitas2";
                 
             
    $query = "update $dbname.klaim set pajak='$pjnspajak', nama_pengusaha='$pnmpengusaha', noseri='$pnoseri', "
            . " tgl_fp='$ptglfp', realisasi1='$pnmrealisasi', lampiran='', noslip='$pnoslip', "
            . " tgltrans='$ptgltras', aktivitas2='$paktivitas2' WHERE klaimId='$kodenya' LIMIT 1";
...
elseif ($module=='entrybrklaim')
{
    ...
    if ($act=='input') {
        ...
        $query="insert into $dbname.klaim (klaimId, tgl, karyawanid, distid, pengajuan, user1)values"
                . "('$kodenya', '$ptglinput', '$pkaryawan', '$pdist', '$pdivpengajuan', '$puserid')";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; mysqli_close($cnmy); exit; }
    }
