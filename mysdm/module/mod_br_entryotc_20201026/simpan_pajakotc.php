<?php
    ...
    if ($module=="entrybrotc" AND $act=="inputdatapajak" OR $act=="editdata") {
        ...
        $query = "UPDATE hrd.br_otc SET jumlah='$ptotjmlminta' WHERE brOtcId='$pbrid' LIMIT 1";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo "$erropesan : br otc"; exit; }
        ...
        if ($pidinput=="0" OR (double)$pidinput==0) {
            ...
            $query = "DELETE FROM dbmaster.t_br_otc_pajak WHERE idinput='$pidinput' LIMIT 1";
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo "$erropesan : hapus br otc"; exit; }
            
            
            if (!empty($nbrid)) {
                $query = "UPDATE hrd.br_otc SET jumlah='$ntotalusul' WHERE brOtcId='$nbrid' LIMIT 1";
                mysqli_query($cnmy, $query);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo "$erropesan : br otc"; exit; }
            }
