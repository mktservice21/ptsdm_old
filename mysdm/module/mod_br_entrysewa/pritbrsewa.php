...
<html>
    <head>
        <title>Kontrakan Rumah <?PHP echo $printdate." ".$jamnow; ?></title>
    ...
    <body>
        <div id="div1">
            <?PHP
                ...
                $query = "select * from dbmaster.v_sewa where idsewa='$_GET[brid]' order by nama, tglmulai, nama_area";
                ...
                $bulan=$row['periode'];
                $jumlahrp=number_format($row['jumlah'],0);
                ...
                $phari1=date("w", strtotime($row['tglmulai']));
                $pdate1=date("d", strtotime($row['tglmulai']));
                $pbln1=(int)date("m", strtotime($row['tglmulai']));
                $pthn1=date("Y", strtotime($row['tglmulai']));
                
                $pp01=$seminggu[$phari1]." ".$pdate1." ".$nama_bln[$pbln1]." ".$pthn1;
                
                $phari2=date("w", strtotime($row['tglakhir']));
                $pdate2=date("d", strtotime($row['tglakhir']));
                $pbln2=(int)date("m", strtotime($row['tglakhir']));
                $pthn2=date("Y", strtotime($row['tglakhir']));
                
                $pp02=$seminggu[$phari2]." ".$pdate2." ".$nama_bln[$pbln2]." ".$pthn2;
                
                $pawal=date("F Y", strtotime($row['tglmulai']));
                $pakhir=date("F Y", strtotime($row['tglakhir']));
                
                $pdivisi=$row['divisi'];
                $pjabatanid=$row['jabatanid'];
                if (empty($pjabatanid))
                    $pjabatanid = getfieldit("select jabatanId as lcfields from hrd.karyawan where karyawanId='$idkaryawan'");
                ...
                $nmatasan1 = getfieldit("select nama as lcfields from hrd.karyawan where karyawanId='$patasan1'");
                $patasan2=$row['atasan2'];
                $nmatasan2 = getfieldit("select nama as lcfields from hrd.karyawan where karyawanId='$patasan2'");
                $patasan3=$row['atasan3'];
                $nmatasan3 = getfieldit("select nama as lcfields from hrd.karyawan where karyawanId='$patasan3'");
                $patasan4=$row['atasan4'];
                $nmatasan4 = getfieldit("select nama as lcfields from hrd.karyawan where karyawanId='$patasan4'");
            ...
            <div id="kotakjudul">
                <div id="isikiri">
                    <table class='tjudul' width='100%'>
                        ...
                        <tr><td>PERIODE</td><td>:</td><td><?PHP echo "$pawal - $pakhir"; ?></td></tr>
                        <tr><td>JUMLAH</td><td>:</td><td><?PHP echo "Rp. $jumlahrp"; ?></td></tr>
