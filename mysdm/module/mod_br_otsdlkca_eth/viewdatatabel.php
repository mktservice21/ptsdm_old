...
<?php
    ...
    //mysqli_query($cnit, "DELETE FROM $tmp01 WHERE IFNULL(selisih,0)<=0");
    mysqli_query($cnit, "DELETE FROM $tmp01 WHERE IFNULL(periode_ca2,'')<>'2'");
	
    $now=date("mdYhis");
    $tmp10 =" dbtemp.DTBRRETRLCLS10_".$_SESSION['IDCARD']."_$now ";
    ...
    $query = "select DISTINCT a.bulan, a.karyawanid, b.nama, a.divisi, a.saldo, a.ca1, a.selisih 
        from dbmaster.t_brrutin_ca_close a JOIN hrd.karyawan b on a.karyawanid=b.karyawanId 
        WHERE DATE_FORMAT(a.bulan,'%Y-%m')='$periode1' AND IFNULL(a.selisih,0) > 0
        ORDER BY b.nama";
    $query = "create TEMPORARY table $tmp01 ($query)"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    
        $query="DELETE FROM $tmp01 WHERE karyawanid in (select distinct IFNULL(karyawanid,'') from $tmp10)";
        mysqli_query($cnit, $query);
        
        mysqli_query($cnit, "DELETE FROM $tmp10 WHERE IFNULL(selisih,0)<=0");
        
        $query = "INSERT INTO $tmp01 (bulan, karyawanid, nama, divisi, saldo, ca1, selisih)"
                . " select DISTINCT bulan, karyawanid, nama_karyawan nama, divisi, saldo, ca1, selisih from $tmp10"; 
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    ...
    $query = "UPDATE $tmp03 a JOIN hrd.karyawan b on a.ikaryawanid=b.karyawanid SET a.nama_transfer=b.nama";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    
    $query = "UPDATE $tmp03 SET nama_transfer=inama_karyawan WHERE IFNULL(ikaryawanid,'')=''";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    ...
    <table id='datatablepostdeth' class='table table-striped table-bordered' width="100%">
        <thead>
            <tr>
                ...
                <th align="center" nowrap>Transfer Oleh</th>
        ...
        <?PHP
            ...
            while( $row=mysqli_fetch_array($tampil) ) {
                ...
                $pigroup=$row['igroup'];
                $pnamatransferke_id=$row['ikaryawanid'];
                $pnamatransferke=$row['nama_transfer'];
                ...
                */
                ...
                $peditdata=$pnamatransferke;
                if (!empty($pigroup)) {
                    //$hapusdata="";
                    $peditdata="<a class='btn btn-success btn-xs' href='?module=outlkcaethical&act=editdata&idmenu=253&nmun=253&id=$pigroup&imst=$pnamatransferke_id'>$pnamatransferke</a>";
                }
