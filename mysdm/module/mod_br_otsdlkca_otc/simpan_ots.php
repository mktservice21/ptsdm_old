...
if ($module=="outlkcaotc") {
    ...
    if ($act=='input') {
        ...
        $ppilih_nobukti=caribuktinomor('1', $ptgl_kembali);// 1=bbm, 2=bbm
    ...
    $query = "DELETE FROM $dbname.t_brrutin_outstanding WHERE DATE_FORMAT(bulan,'%Y%m')='$pblnthn' AND ikaryawanid='$pidkry_master' AND igroup='$no_idgroup' AND divisi='OTC'";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo "Error Delete"; exit; }
        
    foreach ($_POST['txtchksama'] as $no_brid) {
        $pdivisi="OTC";//$_POST['txtiddiv'][$no_brid]
        ...
        $query="INSERT INTO $dbname.t_brrutin_outstanding (tglinput, bulan, karyawanid, divisi, saldo, ca, selisih, jumlah_kembali, kembali_rp, tgl_kembali, ots_status, keterangan, coa, userid, nama_karyawan, "
                . "igroup, ikaryawanid, rp_total, rp_total2, inama_karyawan, nobukti)VALUES"
                ...
                . "'$no_idgroup', '$pidkry_master', '$ptotal_rpmst', '$ptotal_rpmst2', '$pnama_master', '$pnobukti')";
        mysqli_query($cnmy, $query);
