...
<?php
    ...
    $query = "select b.idca, a.divisi, a.karyawanid, a.nama, a.icabangid, a.areaid, 
        ...
        CAST(NULL as CHAR(10)) as igroup, CAST(NULL as CHAR(10)) as ikaryawanid, CAST(NULL as CHAR(10)) as nama_transfer, CAST(NULL as CHAR(10)) as inama_karyawan
    ...
    $query = "UPDATE $tmp04 a JOIN hrd.karyawan b on a.ikaryawanid=b.karyawanid SET a.nama_transfer=b.nama";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    
    $query = "UPDATE $tmp04 SET nama_transfer=inama_karyawan WHERE IFNULL(ikaryawanid,'')=''";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
        ...
        <?PHP
            ...
            while( $row=mysqli_fetch_array($tampil) ) {
                ...
                */
                ...
                if (!empty($pigroup)) {
                    //$hapusdata="";
                    $peditdata="<a class='btn btn-success btn-xs' href='?module=outlkcaotc&act=editdata&idmenu=254&nmun=254&id=$pigroup&imst=$pnamatransferke_id'>$pnamatransferke</a>";
                }
