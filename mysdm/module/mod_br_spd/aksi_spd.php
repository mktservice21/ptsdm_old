...
elseif ($module=='saldosuratdana')
{
    ...
    if ($padvance=="B" OR $padvance=="V" OR $padvance=="C") {
        ...
        $query = "UPDATE $dbname.t_suratdana_br SET pilih='Y' WHERE idinput='$kodenya'";
        mysqli_query($cnmy, $query);
    }
            ...
            if (!empty($kodenya)){
                ...
                foreach ($_POST['chk_jml1'] as $no_brid) {
                    if (!empty($no_brid)) {
                        ...
                        //eksekusi input
                        $query = "INSERT INTO $dbname.t_suratdana_br1 (idinput, bridinput, kodeinput, urutan, amount, trans_ke, jml_adj, aktivitas1)VALUES"
                                . " ('$kodenya', '$no_brid', '$kodeinput', "
                                . " '$no_urutbr', '$pjmlinputpilih', '$ptrans_ke', '$pjml_adj_pilih', '$pket_adj_pilih')";
                        mysqli_query($cnmy, $query);
                        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
