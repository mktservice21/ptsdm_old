...
if ($_GET['module']=="viewdataotc"){
    ...
    $pjenispilih=$_POST['uijenispilih'];
    ...
    $ftypetgl = "  a.tglbr BETWEEN '$mytgl1' AND '$mytgl2' ";
    if ($pertipe=="T") $ftypetgl = " a.tgltrans BETWEEN '$mytgl1' AND '$mytgl2' ";
    if ($pertipe=="S") $ftypetgl = " a.tglrpsby BETWEEN '$mytgl1' AND '$mytgl2' ";
    if ($pertipe=="R") $ftypetgl = " a.tglreal BETWEEN '$mytgl1' AND '$mytgl2' ";
    ...
    $tmp03 =" dbtemp.DSETH03_".$userid."_$now ";
        ...
        $query = "select distinct a.urutan, a.idinput, IFNULL(a.bridinput,'') bridinput, a.amount, CAST('' as CHAR(1)) as sinput FROM dbmaster.t_suratdana_br1 a JOIN "
                . " dbmaster.t_suratdana_br b on a.idinput=b.idinput WHERE 1=1 "
                . "  ";
        if ($pact=="editdata") {
            $query .= " AND ( (b.stsnonaktif<>'Y' and b.divisi='OTC' AND a.kodeinput='D' and IFNULL(b.pilih,'')<>'N') OR a.idinput='$pidinput')";
        }else{
            $query .= " AND b.stsnonaktif<>'Y' and b.divisi='OTC' AND a.kodeinput='D' and IFNULL(b.pilih,'')<>'N' ";
        }
        ...
        if ($pact=="editdata") {
            $query = "UPDATE $tmp03 SET sinput='Y' WHERE idinput='$pidinput'";
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
        }
    
        
        $query = "select * FROM $tmp03 WHERE IFNULL(sinput,'')='Y'";
        ...
        $query = "ALTER table $tmp01 ADD COLUMN urutan INT(4)"; 
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
        
        
        if ($pact=="editdata") {
            $query = "UPDATE $tmp01 a JOIN $tmp02 b on a.brOtcId=b.bridinput SET a.sinput=b.sinput, a.amount=b.amount, a.urutan=b.urutan";
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
        }
        
        //echo "$pjenispilih, $pertipe";
        if ($pjenispilih=="K" OR $pertipe=="S"){
    ...
    ?>
        ...
        <div class='form-group'>
            ...
            <button type='button' class='btn btn-danger btn-xs' onclick='HitungTotalDataDariCekBox()'>Hitung Ulang</button> 
            ...
            <table id='datatable' class='datatable table nowrap table-striped table-bordered' width="100%">
                <thead style="">
                    <tr>
                        ...
                        <th width='10px' nowrap>
                            <?PHP
                            if ($pact=="editdata") {
                                echo "<input type='checkbox' id='chkall1[]' name='chkall1[]' onclick=\"PilihSemuaData('chkall1[]', 'chk_brid[]')\" value='select'>";
                            }else{
                                echo "<input type='checkbox' id='chkall1[]' name='chkall1[]' onclick=\"PilihSemuaData('chkall1[]', 'chk_brid[]')\" checked>";
                            }
                            ?>
                        </th>
                        <th width='10px' nowrap>No</th>
                        <th <?PHP echo $tr_nhidden; ?> align="center" nowrap>Jml. Input</th>
                        <th align="center" nowrap>Jumlah</th>
                        <th align="center" nowrap>Jml. Realisasi</th>
                        <th align="center" nowrap>No. Slip</th>
                        <th align="center" nowrap>Kelompok</th>
                ...
                <tbody>
                    ...
                    <?PHP
                        ...
                        while ($row= mysqli_fetch_array($tampil)) {
                            ...
                            $ptglbr =date("d/m/Y", strtotime($row['tglbr']));
                            ...
                            $chkbox = "<input type='checkbox' id='chk_brid[$pbrid]' name='chk_brid[]' value='$pbrid' onclick=\"CentangCekBoxDataBR('$pbrid', 'e_jmlusul[$pbrid]')\" $nval_chk> ";
                            
                            
                            $ptxtjmlusul="<input type='hidden' size='10px' id='e_jmlusul[$pbrid]' name='e_jmlusul[$pbrid]' class='input-sm inputmaskrp2' autocomplete='off' value='$nnjumlahpilih' Readonly>";
                            ...
                            $simpandata= "<input type='button' class='btn btn-info btn-xs' id='s-submit' value='Save Real' onclick=\"SimpanDataReal('input', $fsimpan)\">";
                        
                            
                            echo "<tr>";
                            echo "<td nowrap>$chkbox $ptxtnobrid<t/d>";
                            echo "<td nowrap>$no</td>";
                            echo "<td $tr_nhidden nowrap align='right'>$pamount</td>";
                            echo "<td nowrap align='right'>$pjumlah $ptxtjmlusul</td>";
    ...
    <script>
        ...
        function PilihSemuaData(nmbuton, data){
        ...
        function CentangCekBoxDataBR(nidbr, nmtxtjmlusul) {
            HitungTotalDataDariCekBox();
        }
        
        function HitungTotalDataDariCekBox() {
            var ekode =document.getElementById('cb_kode').value;
            if (ekode=="7") {
                return false;
            }
            
            var chk_arr1 =  document.getElementsByName('chk_brid[]');
            ...
            for(k=0;k< chklength1;k++)
            {
                if (chk_arr1[k].checked == true) {
                    ...
                    var anm_jml="e_jmlusul["+fields[0]+"]";
        ...
        function SimpanDataReal(eact, idbr, ajmlreal, anoslip, atglterima)  {
