<?php
    ...
    if ($pkaryawanid=="0000000143") $sql .=" AND CONCAT(kodeid,subkode) in ('103', '205')";
    elseif ($pkaryawanid=="0000000329") $sql .=" AND CONCAT(kodeid,subkode) in ('221')";
    else $sql .=" AND CONCAT(kodeid,subkode) in ('103', '105', '221')";
    ...
    while( $row=mysqli_fetch_array($query) ) {  // preparing an array
        ...
        $plihatview="";
        $plihatexcel="";
        if ($pkode=="2" AND $psubkode=="05") {
            $plihatview = "<a class='btn btn-info btn-xs' href='eksekusi3.php?module=reportcasewa&act=rpt&idmenu=264&ket=bukan&ispd=$idno&bln=$ptgl' target='_blank'>View</a>";
            $plihatexcel = "<a class='btn btn-success btn-xs' href='eksekusi3.php?module=reportcasewa&act=rpt&idmenu=264&ket=excel&ispd=$idno&bln=$ptgl' target='_blank'>Excel</a>";
        }else{
