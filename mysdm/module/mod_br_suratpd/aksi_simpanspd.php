...
elseif ($module=='suratpd')
{
                ...
                */
                ...
                foreach ($_POST['chk_jml1'] as $no_brid) {
                    if (!empty($no_brid)) {
                        ...
                        $pjenistransfer="";
                        if (isset($_POST['chk_tca'][$no_brid])) $pjenistransfer=$_POST['chk_tca'][$no_brid];
                        if (isset($_POST['chk_tbc'][$no_brid])) $pjenistransfer=$_POST['chk_tbc'][$no_brid];
                        if (isset($_POST['chk_tnb'][$no_brid])) $pjenistransfer=$_POST['chk_tnb'][$no_brid];
                        if (isset($_POST['chk_tva'][$no_brid])) $pjenistransfer=$_POST['chk_tva'][$no_brid];
                        if (isset($_POST['chk_tpy'][$no_brid])) $pjenistransfer=$_POST['chk_tpy'][$no_brid];
                        if (isset($_POST['chk_ttg'][$no_brid])) $pjenistransfer=$_POST['chk_ttg'][$no_brid];
                        ...
                        $query = "UPDATE $dbname.t_suratdana_br1 SET trans_ke='$pjenistransfer' WHERE bridinput='$no_brid' AND idinput='$kodenya' LIMIT 1";
                        mysqli_query($cnmy, $query);
    ...
    if ($pkode=="3" OR $pkode=="7" OR ($pkode=="2" AND $psubkode=="29x") ) {
        ...
        if ($pkode=="3") {
            $sql=  mysqli_query($cnmy, "select kodeid, subkode, divisi from $dbname.t_suratdana_br WHERE nodivisi='$pnodivisi2'");
            $nr= mysqli_fetch_array($sql);
            
            $nnkodeid2=$nr['kodeid'];
            $nnsubkode2=$nr['subkode'];
            $nndivisi2=$nr['divisi'];
            
            if (empty($nnkodeid2)) $nnkodeid2="1";
            if (empty($nnsubkode2)) $nnsubkode2="01";
            
            $query = "UPDATE $dbname.t_suratdana_br SET $setupdatetgl kodeid2=$nnkodeid2, subkode2='$nnsubkode2', divisi2='$nndivisi2', bulan2='$periode1', nomor='$pnomor2', nomor2='$pnomor2', nodivisi2='$pnodivisi2' $t_upadjtgl WHERE idinput='$kodenya'";//, userproses='$userid', tgl_proses=NOW()
        }else{
    ...
    if ($pkode=="3") {// AND $act=="input"
        $npil_divadj_=$pdivno;
        if (empty($pdivno)) $npil_divadj_=$pnodivisi2;
        $pnospdadj="";
        
        $sql=  mysqli_query($cnmy, "select nomor, tgl from $dbname.t_suratdana_br WHERE nodivisi='$npil_divadj_' AND stsnonaktif<>'Y'");
        $nr= mysqli_fetch_array($sql);
        if (empty($pnomor2)) $pnospdadj=$nr['nomor'];
        $nptgladj_="";
        if (!empty($nr['tgl'])) {
            $nptgladj_=" tgl='$nr[tgl]', ";
        }
        
        if ($act=="input") {
            
            $query = "UPDATE $dbname.t_suratdana_br SET $nptgladj_ nomor='', tglspd=null, userproses=null, tgl_proses=null, nomor2='$pnospdadj' WHERE idinput='$kodenya'";
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
            
        }else{
            $query = "UPDATE $dbname.t_suratdana_br SET $nptgladj_ nomor=nomor WHERE idinput='$kodenya'";
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
        }
        
            
        if (isset($_POST['e_tanggaladj']) AND ( !empty($_POST['e_ajsnobr2'])) OR $act=="update" ) {
            //$npdivadj=$_POST['cb_divisiadj'];
            if (!empty($_POST['e_tanggaladj'])) {
                $nptgladj = str_replace('/', '-', $_POST['e_tanggaladj']);
                $nptgladj = date("Y-m-d", strtotime($nptgladj));
                //, divisi='$npdivadj'
                $query = "UPDATE $dbname.t_suratdana_br SET tgl='$nptgladj' WHERE idinput='$kodenya' AND IFNULL(nodivisi,'')='' AND IFNULL(nodivisi2,'')<>''";
                mysqli_query($cnmy, $query);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
            }
        ...
        $query = "UPDATE $dbname.t_suratdana_br SET nodivisi=nodivisi2, nomor2='' WHERE idinput='$kodenya' AND IFNULL(nodivisi,'')='' AND IFNULL(nodivisi2,'')<>''";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
