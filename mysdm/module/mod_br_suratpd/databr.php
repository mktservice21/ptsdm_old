...
if ($_GET['module']=="viewdataanne"){
    ...
    */
    ...
    if ($pkodeid=="7") {
        ...
        if ($pact=="editdata") {
            ...
            $query = "select * from hrd.br0 WHERE (1=1 $ftypetgl $filterlampiran $fadvance $fdivisi $fil_viasby AND "
            ...
            $query = "select * from hrd.br0 WHERE (1=1 $ftypetgl $filterlampiran $fadvance $fdivisi $fil_viasby AND "
                    ...
                    . " AND user1='$pidinputuser' $nfilter_sudahada)"
                    . " OR $filter_sudah_cek";
    ...
    if ($pact=="editdata") {
        ...
        $query = "select a.*, b.urutan, b.bridinput, b.amount, b.jml_adj, b.aktivitas1 ketadj1, b.trans_ke from $tmp03 a LEFT JOIN $tmp04 b on a.brId=b.bridinput";
        ...
        $query = "select *, CAST(null as DECIMAL(10,0)) as urutan, CAST('' as CHAR(20)) as bridinput, CAST('' as CHAR(50)) as ketadj1, CAST(null as DECIMAL(20,2)) as jml_adj, CAST('' as CHAR(2)) as trans_ke from $tmp03";
        $query = "create TEMPORARY table $tmp05 ($query)"; 
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    ...
    */
        ...
        <table id='datatable' class='datatable table nowrap table-striped table-bordered' width="100%">
            <thead>
                <tr>
                    ...
                    <th align="center" nowrap>Tunai/<br/>Cash</th>
                    <th align="center" nowrap>BCA</th>
                    <th align="center" nowrap>Non<br/>BCA</th>
                    <th align="center" nowrap>Virtual<br/>Acc.</th>
                    <th align="center" nowrap>Payroll</th>
                    <th align="center" nowrap>Tagihan</th>
                </tr>
            </thead>
            <tbody>
            <?PHP
                ...
                while ($row= mysqli_fetch_array($tampil)) {
                    ...
                    $pitranske = $row['trans_ke'];
                    $ichk_sca=""; $ichk_sbc=""; $ichk_snb=""; $ichk_sva=""; $ichk_spy=""; $ichk_stg="";
                    
                    if ($pitranske=="CA") $ichk_sca="checked";
                    elseif ($pitranske=="BC") $ichk_sbc="checked";
                    elseif ($pitranske=="NB") $ichk_snb="checked";
                    elseif ($pitranske=="VA") $ichk_sva="checked";
                    elseif ($pitranske=="PY") $ichk_spy="checked";
                    elseif ($pitranske=="TG") $ichk_stg="checked";
                    
                    $chkbox_ca = "<input type='checkbox' id='chk_tca[$pbrid]' name='chk_tca[$pbrid]' value='CA' onclick=\"HapusUncheckStatus('$pbrid', 'CA')\" $ichk_sca>";
                    $chkbox_bc = "<input type='checkbox' id='chk_tbc[$pbrid]' name='chk_tbc[$pbrid]' value='BC' onclick=\"HapusUncheckStatus('$pbrid', 'BC')\" $ichk_sbc>";
                    $chkbox_nb = "<input type='checkbox' id='chk_tnb[$pbrid]' name='chk_tnb[$pbrid]' value='NB' onclick=\"HapusUncheckStatus('$pbrid', 'NB')\" $ichk_snb>";
                    $chkbox_va = "<input type='checkbox' id='chk_tva[$pbrid]' name='chk_tva[$pbrid]' value='VA' onclick=\"HapusUncheckStatus('$pbrid', 'VA')\" $ichk_sva>";
                    $chkbox_py = "<input type='checkbox' id='chk_tpy[$pbrid]' name='chk_tpy[$pbrid]' value='PY' onclick=\"HapusUncheckStatus('$pbrid', 'PY')\" $ichk_spy>";
                    $chkbox_tg = "<input type='checkbox' id='chk_ttg[$pbrid]' name='chk_ttg[$pbrid]' value='TG' onclick=\"HapusUncheckStatus('$pbrid', 'TG')\" $ichk_stg>";
                    ...
                    echo "<td nowrap>$chkbox_ca</td>";
                    echo "<td nowrap>$chkbox_bc</td>";
                    echo "<td nowrap>$chkbox_nb</td>";
                    echo "<td nowrap>$chkbox_va</td>";
                    echo "<td nowrap>$chkbox_py</td>";
                    echo "<td nowrap>$chkbox_tg</td>";
                    echo "</tr>";
    ...
    <script>
        ...
        function HapusUncheckStatus(iid, inama){
            var chkca = document.getElementById('chk_tca['+iid+']');
            var chkbc = document.getElementById('chk_tbc['+iid+']');
            var chknb = document.getElementById('chk_tnb['+iid+']');
            var chkva = document.getElementById('chk_tva['+iid+']');
            var chkpy = document.getElementById('chk_tpy['+iid+']');
            var chktg = document.getElementById('chk_ttg['+iid+']');
        ...
        function HitungTotalDariCekBoxADJ() {
            ...
            for(k=0;k< chklength1;k++)
            {
                if (chk_arr1[k].checked == true) {
                    ...
                    nTotalAdj_ =parseFloat(nTotalAdj_)+parseFloat(ajml);
                }
            }
        ...
        function HitungTotalDariCekBox() {
            ...
            nTotal_ =parseFloat(nTotal_)+parseFloat(ajml_adj);
