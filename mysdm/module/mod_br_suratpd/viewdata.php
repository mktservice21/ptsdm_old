...
if ($_GET['module']=="gantitombol") {
    ...
    $thnbulanyear= date("Y", strtotime($tgl01));
    $bl=(int)$bl;
    $blromawi="I";
    if ($bl==1) $blromawi="I";
    if ($bl==2) $blromawi="II";
    ...
    $awal=3;
        ...
        $query = "SELECT MAX(SUBSTRING_INDEX(nomor, '/', 1)) as pnomor FROM dbmaster.t_suratdana_br WHERE stsnonaktif<>'Y' AND YEAR(tglspd)='$thnbulanyear'";
    ...
    if ( ($pkode=="1" AND $psubkode=="01") OR ($pkode=="2" AND $psubkode=="20") OR ($pkode=="6" AND $psubkode=="80") OR ($pkode=="3") ) {
    ...
    $query = "SELECT MAX(SUBSTRING_INDEX(nodivisi, '/', 1)) as pnomor FROM dbmaster.t_suratdana_br WHERE stsnonaktif<>'Y' AND YEAR(tgl)='$tahuninput' AND "
            . " karyawanid='$userid' and kodeid not in ('3') and tgl>='2019-10-01'";
    $showkan= mysqli_query($cnmy, $query);
    $ketemu= mysqli_num_rows($showkan);
    if ($ketemu>0){
        $sh= mysqli_fetch_array($showkan);
    ...
    if (!empty($pnoid)) {
        ...
        if ($ketemu_b>0) {
            $ts= mysqli_fetch_array($tampil_b);
            if (!empty($ts['jumlah'])) $totalinput=(double)$totalinput+(double)$ts['jumlah'];
        }
        ...
        if ($ketemu_p>0) {
            $tp= mysqli_fetch_array($tampil_p);
            if (!empty($tp['jumlah'])) $totalinput=(double)$totalinput+(double)$tp['jumlah'];
        }
        
        //BUNGA YANG PLUS DOANG
        $query="SELECT SUM(jumlah) as jumlah from dbmaster.t_suratdana_bank WHERE idinputbank IN $pnoid AND CONCAT(kodeid, subkode) IN ('231') AND IFNULL(jumlah,0)<0";
        $tampil_pj= mysqli_query($cnmy, $query);
        $ketemu_bj= mysqli_num_rows($tampil_pj);
        if ($ketemu_bj>0) {
            $tsj= mysqli_fetch_array($tampil_pj);
            if (!empty($tsj['jumlah'])) $totalinput=(double)$totalinput+(double)$tsj['jumlah'];
        }
    ...
    $ptglbrdiv = date('d/m/Y', strtotime($hari_ini));
    ...
    $pjnsrpt="";
    if ($pkode=="2" OR $pkode=="6") $pjnsrpt="K";
    $pjens1="";
    $pjens2="";
    $pjens3="";
    $pjens4="";
    ...
    $ndisabled="disabled";
    $nchecked_="";
    if ( $pkode=="3" OR $pkode=="7" OR ($pkode=="2" AND $psubkode=="29x") ) $tgl1 = date('F Y', strtotime($hari_ini));
    if (!empty($pidinput)) {
        ...
        $ptglbrdiv = date('d/m/Y', strtotime($r['tgl']));
        
        $tgl1 = date('d F Y', strtotime($r['tgl']));
        
        $ntgl1 = date('d F Y', strtotime($r['tglf']));
        $ntgl2 = date('d F Y', strtotime($r['tglt']));
        $eperiode1=$ntgl1;
        $eperiode2=$ntgl2;
        ...
        if (!empty($r['bulan2'] AND $r['bulan2']<>"0000-00-00")) {
            ...
            if ( $pkode=="3" OR $pkode=="7" OR ($pkode=="2" AND $psubkode=="29x") ) $ptgl=$r['bulan2'];
            //$ndisabled="";
            //$nchecked_="checked";
        }
    ...
    ?>
        ...
        <script>
            ...
            function HapusBulanSPD() {
                ...
                if (chkinp.checked == true){
                    document.getElementById("e_tglberlaku").disabled = false;
                }else{
                    document.getElementById("e_tglberlaku").disabled = true;
                }
                ShowDataAjsNoSPD();
                ShowDataNoDivisiBR();
            }
        ...
        <div hidden class='form-group'>
            ...
            <div class='col-md-3'>
                <input type="checkbox" id="chk_pilihbln" name="chk_pilihbln" value="" onclick="HapusBulanSPD()" <?PHP echo $nchecked_; ?> >
                <div class='input-group date' id=''>
                    <input type="text" class="form-control" id='e_tglberlaku' name='e_tglberlaku' autocomplete='off' required='required' placeholder='d F Y' value='<?PHP echo $tgl1; ?>' <?PHP echo $ndisabled; ?>>
        ...
        <div class='form-group'>
            ...
            <div class='col-xs-5'>
                <select class='form-control input-sm' id='cb_ajsnospd' name='cb_ajsnospd' onchange=""><!--ShowDataNoDivisiBR();-->
                    ...
                    <?PHP
                    $filbulansod_adj="";
                    if (empty($ndisabled)) $filbulansod_adj=" AND ( DATE_FORMAT(tgl,'%Y-%m')='$ajsbulan' OR DATE_FORMAT(tglspd,'%Y-%m')='$ajsbulan' ) ";
                    
                    $query = "select nomor, SUM(jumlah) jumlah from dbmaster.t_suratdana_br WHERE IFNULL(stsnonaktif,'')<>'Y' "
                            . "and IFNULL(pilih,'') = 'Y' AND IFNULL(nomor,'')<>'' $filbulansod_adj "
            ...
            <div class='col-xs-5'>
                <select class='form-control input-sm' id='cb_ajsnobr' name='cb_ajsnobr'>
                    ...
                    $query = "select divisi, nodivisi, SUM(jumlah) jumlah from dbmaster.t_suratdana_br WHERE IFNULL(stsnonaktif,'')<>'Y' "
                            . " AND IFNULL(nodivisi,'')<>'' $filbulansod_adj "//AND  (userid='$_SESSION[IDCARD]' OR nodivisi='$ajsnobr')
        ...
        <?PHP if ($lewatnodivspd2==true) $ajsnobr=""; ?>
        ...
        <div class='form-group'>
            ...
            <div class='col-xs-3'>
                <input type='text' id='e_ajsnobr2' name='e_ajsnobr2' class='form-control col-md-7 col-xs-12' value='<?PHP echo $ajsnobr; ?>' placeholder="No.BR/Divisi">
            </div>
        </div>
        
        
        <script>
        $('#mytgladj01').datetimepicker({
            ignoreReadonly: true,
        ...
        <div class='form-group'>
            <label class='control-label col-md-3 col-sm-3 col-xs-12' for='' style="color:red;">Tgl. No BR/Divisi </label>
            <div class='col-md-3'>
                <div class='input-group date' id='mytgladj01'>
                    <input type="text" class="form-control" id='e_tanggaladj' name='e_tanggaladj' autocomplete='off' required='required' placeholder='dd/MM/yyyy' data-inputmask="'mask': '99/99/9999'" value='<?PHP echo $ptglbrdiv; ?>'>
                    <span class='input-group-addon'>
                        <span class='glyphicon glyphicon-calendar'></span>
                    </span>
                </div>
        ...
        <div hidden class='form-group'>
            <label class='control-label col-md-3 col-sm-3 col-xs-12' for='' style="color:red;">Divisi No BR <span class='required'></span></label>
            <div class='col-md-4'>
                <select class='form-control input-sm' id='cb_divisiadj' name='cb_divisiadj'>
                    <?PHP
                    echo "<option value='' selected>--Pilih--</option>";
                    $query = "select DivProdId from MKT.divprod WHERE br='Y' AND DivProdId NOT IN ('OTHER', '') ";
                    $tampil = mysqli_query($cnmy, $query);
                    while ($z= mysqli_fetch_array($tampil)) {
                        if ($z['DivProdId']==$pdivisiadj)
        ...
        <div class='form-group'>
            <label class='control-label col-md-3 col-sm-3 col-xs-12' for=''> <span class='required'></span></label>
            <div class='col-xs-3'>
                <span style="color:red;">*)diisi jika pilihan no br/divisi tidak ada.</span>
            </div>
        </div>
        <div class='form-group'>
            <label class='control-label col-md-3 col-sm-3 col-xs-12' for=''> <span class='required'></span></label>
            <div class='col-xs-3'>
                &nbsp;
                                    ...
                                    ?>
                                    <div hidden class='form-group'>
                                        <label class='control-label col-md-3 col-sm-3 col-xs-12' for='e_cabang'>Noslip / ID BR <span class='required'></span></label>
                                        <div class='col-xs-5'>
                                            ...
                                            <input type='hidden' class='form-control' id='e_idnobr' name='e_idnobr' value='<?PHP echo $pbrnoid; ?>' Readonly>
                                    ...
                                    <script>
                                        function getDataBRSlip(data1, data2){
                                            var inospd=document.getElementById('cb_ajsnospd').value;
                                            var inodivisi=document.getElementById('cb_ajsnobr').value;
                                            $.ajax({
                                                type:"post",
                                        ...
                                        function getDataModalBRSlip(fildnya1, fildnya2, d1, d2){
                                            ...
                                            document.getElementById(fildnya2).value="ID "+d1+", noslip : "+d2;
                                        }
                                    </script>
        ...
        <div class='form-group'>
            ...
            <script>
                $(document).ready(function() {
                    <?PHP if (!empty($pidinput) AND $pkode=="7") { ?>
                            CariDataBR();
    ...
    <?PHP
        ...
        <?PHP
        if ( ($pkode=="1" AND $psubkode=="01") OR ($pkode=="2" AND $psubkode=="20") OR ($pkode=="6" AND $psubkode=="80") ) {
    ?>
            ...
            <div class='form-group'>
                ...
                <div class='col-xs-3'>
                    ...
                    <div class='form-group'>
                        <label class='control-label col-md-3 col-sm-3 col-xs-12' for='' style="color:red;">Adjusment <span class='required'></span></label>
                        <div class='col-xs-5'>
                            <select class='form-control input-sm' id='cb_ajsnobr' name='cb_ajsnobr'>
                                <option value='' selected>-- Pilihan --</option>
                                <?PHP
            ...
            <div class='form-group'>
                ...
                <div class='col-md-3'>
                    <div class="form-group">
                            ...
                            <select class='form-control input-sm' id="cb_jenispilih" name="cb_jenispilih" onchange="ShowDivRpPCM('1')" data-live-search="true">
                                <?PHP
                                if ($pkode=="1"){
                                    echo "<option value='A' $pjens1>Advance</option>";
                                    echo "<option value='B' $pjens3>Belum Ada Kuitansi</option>";
                                    echo "<option value='V' $pjens7>Via Surabaya (BR)</option>";
                                }elseif ($pkode=="6"){
                                    echo "<option value='S' $pjens5>Kasbon Surabaya</option>";
                                }else{
                ...
                <div class='col-md-3'>
                    <input type='text' id='e_jmlusulan' name='e_jmlusulan' autocomplete='off' class='form-control col-md-7 col-xs-12 inputmaskrp2' value='<?PHP echo $jumlah; ?>' >
                                ...
                                <div class='form-group'>
                                    ...
                                    <div class='col-xs-3'>
                                        <input type='text' id='e_jmladj' name='e_jmladj' autocomplete='off' class='form-control col-md-7 col-xs-12 inputmaskrp2' value='<?PHP echo "$pjmladj"; ?>' >
                                    </div>
                                </div>
                                ...
                                <div class='form-group'>
                                    ...
                                    <div class='col-xs-3'>
                                        <input type='text' id='e_jmltotal' name='e_jmltotal' autocomplete='off' class='form-control col-md-7 col-xs-12 inputmaskrp2' value='<?PHP echo "$pjm_total"; ?>' >
                                    </div>
                                </div>
    ...
    if (!empty($tgl01)){
        ...
        $filtertgl=" AND ( DATE_FORMAT(tgl,'%Y-%m')='$ajsbulan' OR DATE_FORMAT(tglspd,'%Y-%m')='$ajsbulan' ) ";
    }
    ...
    $query = "select nomor, SUM(jumlah) jumlah from dbmaster.t_suratdana_br WHERE IFNULL(stsnonaktif,'')<>'Y' "
            . "and IFNULL(pilih,'') = 'Y' AND IFNULL(nomor,'')<>'' $filajsnospd $filtertgl"
            . "GROUP BY 1 ORDER BY 1";
    $tampil = mysqli_query($cnmy, $query);
    ...
    if (!empty($pajsnospd)) $filajsnospd=" AND IFNULL(nomor,'')='$pajsnospd' ";
    $query = "select divisi, nodivisi, SUM(jumlah) jumlah from dbmaster.t_suratdana_br WHERE IFNULL(stsnonaktif,'')<>'Y' "
            . "and IFNULL(pilih,'') = 'Y' AND IFNULL(nodivisi,'')<>'' $filajsnospd $filtertgl"
            . "GROUP BY 1,2 ORDER BY 1,2";
    $tampil = mysqli_query($cnmy, $query);
    echo "<option value='' selected>-- Pilihan --</option>";
    ...
    while ($zs= mysqli_fetch_array($tampil)) {
        ...
        $pajsdivisi=$zs['divisi'];
        if (empty($pajsdivisi)) $pajsdivisi= "ETHICAL";
        $pajsketjml = "$pajsnobr &nbsp;&nbsp;&nbsp; (Rp. $pajsjmlbr)";//$pajsdivisi - 
