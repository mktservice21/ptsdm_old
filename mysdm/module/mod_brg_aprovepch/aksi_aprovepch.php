...
if ($module=="gimicapvbrgkeluarpch") {
    ...
    if (!empty($noidbr) AND !empty($karyawanapv)) {
        ...
        if ($act=="simpan_ttdallam") {
            ...
            mysqli_query($cnmy, "update dbmaster.t_barang_keluar a JOIN dbttd.t_barang_keluar_ttd b on a.IDKELUAR=b.IDKELUAR set a.APV1='$karyawanapv', a.APV1_TGL=NOW(), b.APV1_GBR='$gbrapv' WHERE "
                    . " a.IDKELUAR in $noidbr AND IFNULL(a.PM_TGL,'')<>'' AND "
                    . " ( IFNULL(a.APV1_TGL,'')='' OR IFNULL(a.APV1_TGL,'0000-00-00')='0000-00-00' OR IFNULL(a.APV1_TGL,'0000-00-00 00:00:00')='0000-00-00 00:00:00')");
            ...
            mysqli_query($cnmy, "update dbmaster.t_barang_keluar a JOIN dbttd.t_barang_keluar_ttd b on a.IDKELUAR=b.IDKELUAR "
                    ...
                    . " on a.IDKELUAR=c.IDKELUAR AND b.IDKELUAR=c.IDKELUAR SET a.APV1=NULL, a.APV1_TGL=NULL, a.APV1_GBR=NULL WHERE a.IDKELUAR in $noidbr AND "
            ...
            mysqli_query($cnmy, "update dbmaster.t_barang_keluar set STSNONAKTIF='Y', USERID='$karyawanapv' WHERE "
                    ...
                    . " ( IFNULL(APV1_TGL,'')='' OR IFNULL(APV1_TGL,'0000-00-00')='0000-00-00' OR IFNULL(APV1_TGL,'0000-00-00 00:00:00')='0000-00-00 00:00:00')");
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
