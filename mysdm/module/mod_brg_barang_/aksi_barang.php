...
if ($module=='barangdata' AND ($act=="hapus" OR $act=="aktifkan") )
{
...
elseif ($module=='barangdata' AND $act=="hapusgambar")
{
    $pidinput=$_GET['id'];
...
elseif ($module=='barangdata')
{
    ...
    $pidgrpbrg=$_POST['cb_gprod'];
    $pidbrand=$_POST['cb_brand'];
    ...
    if ($act=="input") {
        ...
        if ($ketemu>0){
            ...
            $jml=  strlen($urut);
            $awal=$awal-$jml;
            $kodenya="B".str_repeat("0", $awal).$urut;
        }
        $pidinput=$kodenya;
        if (empty($pidinput)) {
