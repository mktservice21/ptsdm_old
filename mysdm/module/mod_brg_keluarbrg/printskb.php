...
$query = "select 
    ...
    LEFT JOIN hrd.karyawan i on a.APV1=i.karyawanid 
    LEFT JOIN dbttd.t_barang_keluar_ttd j on a.IDKELUAR=j.IDKELUAR 
    LEFT JOIN MKT.iarea k on IFNULL(a.AREAID,'')=IFNULL(k.AREAID,'') AND a.ICABANGID=k.iCabangId 
    LEFT JOIN MKT.iarea_o l on IFNULL(a.AREAID_O,'')=IFNULL(l.AREAID_O,'') AND a.ICABANGID_O=l.icabangid_o 
...
$pnamaarea=$row['NAMAAREAETH'];
if ($ppilihan=="OT") {
    $pnamacabang=$row['NAMA_CABANGOTC'];
    $pnamaarea=$row['NAMAAREAOTC'];
}

if (!empty($pnamaarea) AND $pnamaarea!=$pnamacabang) $pnamacabang .=" - $pnamaarea";

$pidcabpl=$row['ICABANGID_O'];
if (empty($pnamacabang)) {
	$pnamacabang=$pidcabpl;
	if ($pnamacabang=="JKT_RETAIL") $pnamacabang="JAKARTA RETAIL";
	if ($pnamacabang=="JKT_MT") $pnamacabang="JAKARTA - MODERN TRADE";
...
$ptglpch=$row['APV1_TGL'];
$pnamapch=$row['APV1_NAMA'];
$pgbrpch=$row['APV1_GBR'];
...
if ($ptglpch=="0000-00-00" OR $ptglpch=="0000-00-00 00:00:00") $ptglpch="";
...
<BODY>
    ...
    <table class='tjudul' width='100%'>
        ...
        if (empty($ptglpm)) {
            ...
            if (empty($ptglpch)) {
                //echo "<tr><td colspan='3'><span style='color:red;'>BELUM APPROVE PURCHASING</span></td></tr>";
            }
        }
        ?>
        <tr>
        ...
        <tr>
            <td><b>Cabang - Area</b></td><td>:</td><td><?PHP echo "$pnamacabang"; ?></td>
        </tr>
        <tr>
