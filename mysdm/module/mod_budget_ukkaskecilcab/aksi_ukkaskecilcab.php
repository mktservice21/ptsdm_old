...
if ($module=='bgtlimitkaskecilcab' AND ($act=='simpanum' OR $act=='hapusum') )
{
    ...
    for ($x=0; $x<=$jumlah_tag; $x++){
        ...
        if (isset($pid_input[$x]) AND isset($pjumlah[$x])) {
            ...
            if ($act=='simpanum') {
                $query = "INSERT INTO dbmaster.t_uangmuka_kascabang (icabangid, jumlah, userid, coa)VALUES"
    ...
    if ($module=='bgtlimitkaskecilcab' AND $act=='update' )
    {
        $puserid=$_SESSION['IDCARD'];
        ...
        if (!empty($pidcabang)) {
            $query = "delete from dbmaster.t_uangmuka_kascabang WHERE icabangid='$pidcabang' LIMIT 1";
            mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; mysqli_close($cnmy); exit; }
            

            $query = "INSERT INTO dbmaster.t_uangmuka_kascabang (icabangid, pcm, saldoawal, jmltambahan, jumlah, userid, tgltambah, ket, coa)VALUES"
