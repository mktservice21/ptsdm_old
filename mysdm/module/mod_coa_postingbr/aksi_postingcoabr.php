...
if ($module=='postingcoabr' AND $act=='input') {
    $filterid=('');
    if (!empty($_POST['chkbox_id'])){
        ...
        for ($x=0; $x<=$jumlah_tag; $x++){
            if (!empty($arr_kata[$u])){
                ...
                if (!empty($uTag)) {
                    mysqli_query($cnmy, "delete from $dbname.posting_coa_br where divisi='$divisi' and kodeid='$uTag'");
                    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
                    
                    mysqli_query($cnmy, "insert into $dbname.posting_coa_br (divisi, kodeid, COA4)values('$divisi', '$uTag', '$coa')");
                    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
                }
            }
            $u++;
...
$datasavems=SaveDataMS("dbmaster", "posting_coa_br");
