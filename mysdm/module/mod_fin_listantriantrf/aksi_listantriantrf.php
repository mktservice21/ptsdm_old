...
elseif ($module=='listantriantransfer')
{
    ...
    if ($act=="input") {
        ...
        if (!empty($pidinput)) {
            mysqli_query($cnmy, "UPDATE dbmaster.t_br_antrian SET jumlah=0 WHERE idgroup='$pidinput' AND karyawanid='$pkaryawanid'");
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; mysqli_close($cnmy); exit; }
        }
        ...
        if ((DOUBLE)$pjmlca>0) {
            ...
            $ketemua= mysqli_num_rows(mysqli_query($cnmy, "select status_trf FROM dbmaster.t_br_antrian WHERE idgroup='$pidinput' AND status_trf='CA' LIMIT 1"));
            if ($ketemua==0) {
                $query_antrian = "INSERT INTO dbmaster.t_br_antrian (karyawanid, tanggal, idinput, jumlah, nourut, status_trf, keterangan, userid, idgroup) VALUES "
                    . "('$pkaryawanid', '$ptglpengajuan', '$pidinputspd', '$pjmlca', '$pnourutca', 'CA', '$pketerangan', '$puserinput', '$pidinput')";
                mysqli_query($cnmy, $query_antrian);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; mysqli_close($cnmy); exit; }
            }
        ...
        if ((DOUBLE)$pjmlbc>0) {
            ...
            $ketemub= mysqli_num_rows(mysqli_query($cnmy, "select status_trf FROM dbmaster.t_br_antrian WHERE idgroup='$pidinput' AND status_trf='BC' LIMIT 1"));
            if ($ketemub==0) {
                $query_antrian = "INSERT INTO dbmaster.t_br_antrian (karyawanid, tanggal, idinput, jumlah, nourut, status_trf, keterangan, userid, idgroup) VALUES "
                    . "('$pkaryawanid', '$ptglpengajuan', '$pidinputspd', '$pjmlbc', '$pnourutbc', 'BC', '$pketerangan', '$puserinput', '$pidinput')";
                mysqli_query($cnmy, $query_antrian);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; mysqli_close($cnmy); exit; }
            }
        ...
        if ((DOUBLE)$pjmlnb>0) {
            ...
            $ketemuc= mysqli_num_rows(mysqli_query($cnmy, "select status_trf FROM dbmaster.t_br_antrian WHERE idgroup='$pidinput' AND status_trf='NB' LIMIT 1"));
            if ($ketemuc==0) {
                $query_antrian = "INSERT INTO dbmaster.t_br_antrian (karyawanid, tanggal, idinput, jumlah, nourut, status_trf, keterangan, userid, idgroup) VALUES "
                    . "('$pkaryawanid', '$ptglpengajuan', '$pidinputspd', '$pjmlnb', '$pnourutnb', 'NB', '$pketerangan', '$puserinput', '$pidinput')";
                mysqli_query($cnmy, $query_antrian);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; mysqli_close($cnmy); exit; }
            }
        ...
        if ((DOUBLE)$pjmlva>0) {
            ...
            $ketemud= mysqli_num_rows(mysqli_query($cnmy, "select status_trf FROM dbmaster.t_br_antrian WHERE idgroup='$pidinput' AND status_trf='VA' LIMIT 1"));
            if ($ketemud==0) {
                $query_antrian = "INSERT INTO dbmaster.t_br_antrian (karyawanid, tanggal, idinput, jumlah, nourut, status_trf, keterangan, userid, idgroup) VALUES "
                    . "('$pkaryawanid', '$ptglpengajuan', '$pidinputspd', '$pjmlva', '$pnourutva', 'VA', '$pketerangan', '$puserinput', '$pidinput')";
                mysqli_query($cnmy, $query_antrian);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; mysqli_close($cnmy); exit; }
            }
        ...
        if ((DOUBLE)$pjmlpy>0) {
            ...
            $ketemue= mysqli_num_rows(mysqli_query($cnmy, "select status_trf FROM dbmaster.t_br_antrian WHERE idgroup='$pidinput' AND status_trf='PY' LIMIT 1"));
            if ($ketemue==0) {
                $query_antrian = "INSERT INTO dbmaster.t_br_antrian (karyawanid, tanggal, idinput, jumlah, nourut, status_trf, keterangan, userid, idgroup) VALUES "
                    . "('$pkaryawanid', '$ptglpengajuan', '$pidinputspd', '$pjmlpy', '$pnourutpy', 'PY', '$pketerangan', '$puserinput', '$pidinput')";
                mysqli_query($cnmy, $query_antrian);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; mysqli_close($cnmy); exit; }
            }
        ...
        if ((DOUBLE)$pjmltg>0) {
            ...
            $ketemuf= mysqli_num_rows(mysqli_query($cnmy, "select status_trf FROM dbmaster.t_br_antrian WHERE idgroup='$pidinput' AND status_trf='TG' LIMIT 1"));
            if ($ketemuf==0) {
                $query_antrian = "INSERT INTO dbmaster.t_br_antrian (karyawanid, tanggal, idinput, jumlah, nourut, status_trf, keterangan, userid, idgroup) VALUES "
                    . "('$pkaryawanid', '$ptglpengajuan', '$pidinputspd', '$pjmltg', '$pnouruttg', 'TG', '$pketerangan', '$puserinput', '$pidinput')";
                mysqli_query($cnmy, $query_antrian);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; mysqli_close($cnmy); exit; }
            }
