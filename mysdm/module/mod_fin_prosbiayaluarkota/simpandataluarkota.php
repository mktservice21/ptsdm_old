<?PHP
    ...
    if ($module=="simpandatanya") {
        ...
        if (!empty($pidrutin) AND !empty($pnoid)) {
            ...
            $query = "UPDATE dbmaster.t_brrutin1 SET rptotal='$ptotalrp', alasanedit_fin='$palasan' $fieldsave $psavekm WHERE idrutin='$pidrutin' AND nourut=$pnourut AND nobrid='$pnoid'";
            mysqli_query($cnmy, $query);
            $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
