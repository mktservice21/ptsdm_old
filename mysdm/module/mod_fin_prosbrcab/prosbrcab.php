<?PHP
    ...
    $aksi="module/mod_fin_prosbrcab/aksi_prosbrcab.php";
    $hari_ini = date("Y-m-d");
    ...
    if (!empty($_SESSION['PSFBRCFIN_TGL1'])) $tgl_pertama = $_SESSION['PSFBRCFIN_TGL1'];
    if (!empty($_SESSION['PSFBRCFIN_TGL2'])) $tgl_akhir = $_SESSION['PSFBRCFIN_TGL2'];
    if (!empty($_SESSION['PSFBRCFIN_KET'])) $apvpilih = $_SESSION['PSFBRCFIN_KET'];
            ...
            <div class='col-md-12 col-sm-12 col-xs-12'>
                <div class='x_panel'>
                    ...
                    <div class="well" style="overflow: auto; margin-top: -5px; margin-bottom: 5px; padding-top: 10px; padding-bottom: 6px;">
                        <?PHP
                        if ($pgroupuser=="1" OR $pgroupuser=="25") {
                            echo "<input onclick=\"pilihData('validate')\" class='btn btn-dark btn-sm' type='button' name='buttonview1' value='Validate'>";
                        }
                        ?>
                        <input onclick="pilihData('approve')" class='btn btn-warning btn-sm' type='button' name='buttonview1' value='Lihat Belum Proses BR'>
                        <input onclick="pilihData('unapprove')" class='btn btn-success btn-sm' type='button' name='buttonview2' value='Lihat Sudah Proses BR'>
                        <input onclick="pilihData('pending')" class='btn btn-default btn-sm' type='hidden' name='buttonview3' value='Data Pending'>
                        <input onclick="pilihData('reject')" class='btn btn-danger btn-sm' type='hidden' name='buttonview4' value='Data Reject'>
                        <input onclick="pilihData('semua')" class='btn btn-info btn-sm' type='hidden' name='buttonview4' value='All Data'>
...
<script>
    ...
    function pilihData(ket){
        ...
        $.ajax({
            type:"post",
            url:"module/mod_fin_prosbrcab/viewdatatable.php?module="+module+"&idmenu="+idmenu,
