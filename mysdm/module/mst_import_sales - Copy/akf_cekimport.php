...
if (empty($puser)) {
    ...
    mysqli_query($cnmy, "DELETE FROM $dbname.subdist_mscust_akf");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { mysqli_close($cnmy); echo "Error DELETE CUSTOMER SDM : $erropesan"; exit; }
    
    //IT
    if ($plogit_akses==true) {
        mysqli_query($cnit, "DELETE FROM $dbname.subdist_mscust_akf");
    ...
    if ($pinsert){
        ...
        for ($ind=1;$ind<=$jum_record;$ind++){
            ...
            $query = "INSERT INTO $dbname.subdist_mscust_akf (CUSTID, CUSTNM, ALAMAT, KOTAID, "
    ...
    mysqli_query($cnmy, "CREATE TABLE $dbname.mssales (CUSTID VARCHAR(15), TGLJUAL date, NOJUAL VARCHAR(20), BRGID VARCHAR(5), HARGA DOUBLE, QBELI DOUBLE, QBONUS DOUBLE, DPL DOUBLE, _NullFlags longtext)");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { mysqli_close($cnmy); echo "Error CREATE TABLE MSBAR : $erropesan"; exit; }
