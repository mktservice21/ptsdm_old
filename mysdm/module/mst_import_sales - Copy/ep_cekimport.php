...
if (empty($puser)) {
    ...
    mysqli_query($cnmy, "CREATE TABLE $dbname.mssales (PURUTAN BIGINT PRIMARY KEY NOT NULL AUTO_INCREMENT, CUSTID VARCHAR(15), TGLJUAL date, NOJUAL VARCHAR(20), BRGID VARCHAR(5), HARGA DOUBLE, QBELI DOUBLE, QBONUS DOUBLE, DPL DOUBLE, _NullFlags longtext)");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { mysqli_close($cnmy); echo "Error CREATE TABLE MSBAR : $erropesan"; exit; }
    ...
    if ($pinsert){
        ...
        for ($ind=1;$ind<=$jum_record;$ind++){
            ...
            $query = "INSERT INTO $dbname.mssales (CUSTID, TGLJUAL, NOJUAL, BRGID, "
                    . " HARGA, QBELI, QBONUS, DPL, _NullFlags)values"
                    . " ('$pfile0', '$pfile1', '$pfile2', '$pfile3', "
