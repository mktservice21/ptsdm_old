    ...
    foreach ($objPHPExcel->getWorksheetIterator() as $worksheet){
        ...
        if (!empty($mynamesheet)) {
            ...
            for($row=2; $row<=$totalrow; $row++){
                ...
                $pfile6 = mysqli_real_escape_string($cnmy, $worksheet->getCellByColumnAndRow(7, $row)->getValue());
                $pfile7 = mysqli_real_escape_string($cnmy, $worksheet->getCellByColumnAndRow(8, $row)->getValue());
                $pfile8 = mysqli_real_escape_string($cnmy, $worksheet->getCellByColumnAndRow(9, $row)->getValue());
                $pfile9 = mysqli_real_escape_string($cnmy, $worksheet->getCellByColumnAndRow(10, $row)->getValue());
        ...
        if ($plogit_akses==true) {
            ...
            $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { mysqli_close($cnit); echo "IT... Error INSERT importsks : $erropesan"; exit; }
        }
        //END IT
