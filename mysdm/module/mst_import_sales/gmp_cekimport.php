...
if (empty($puser)) {
    ...
    mysqli_query($cnmy, "DELETE FROM $dbname.subdist_mscust_gmp");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { mysqli_close($cnmy); echo "Error DELETE CUSTOMER SDM : $erropesan"; exit; }
    
    
    //IT
    if ($plogit_akses==true) {
        mysqli_query($cnit, "DELETE FROM $dbname.subdist_mscust_gmp");
    ...
    if ($pinsert){
        ...
        for ($ind=1;$ind<=$jum_record;$ind++){
            ...
            $query = "INSERT INTO $dbname.subdist_mscust_gmp (CUSTID, CUSTNM, ALAMAT, ALAMAT2, KOTAID, "
                    . " KOTA, SEKTORID)values"
                    . " ('$pfile0', '$pfile1', '$pfile2', '$pfile3', "
                    . " '$pfile4', '$pfile5', '$pfile6')";
