...
if (empty($puser)) {
    ...
    $pbulan_per =  date("Y-m", strtotime($ptgl));
    ...
    mysqli_query($cnmy, "CREATE TABLE $dbname.msbar (PURUTAN BIGINT PRIMARY KEY NOT NULL AUTO_INCREMENT, BRGID VARCHAR(10), BRGNM VARCHAR(40), BRGID_SUB VARCHAR(15), BRGNM_SUB VARCHAR(40), UNITPCS VARCHAR(10))");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { mysqli_close($cnmy); echo "Error CREATE TABLE MSBAR : $erropesan"; exit; }
    
    mysqli_query($cnmy, "CREATE INDEX inx on $dbname.msbar (PURUTAN)");
    ...
    if ($plogit_akses==true) {
        ...
        mysqli_query($cnit, "CREATE TABLE $dbname.msbar (PURUTAN BIGINT PRIMARY KEY NOT NULL AUTO_INCREMENT, BRGID VARCHAR(10), BRGNM VARCHAR(40), BRGID_SUB VARCHAR(15), BRGNM_SUB VARCHAR(40), UNITPCS VARCHAR(10))");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { mysqli_close($cnit); echo "IT... Error CREATE TABLE MSBAR : $erropesan"; exit; }
        
        mysqli_query($cnit, "CREATE INDEX inx on $dbname.msbar (PURUTAN)");
    ...
    //echo "$target_dir dasdad<br/>";
    $jum_record=0;
    $pinsert=dbase_open(file_exists($target_dir.'msbar.dbf') ? $target_dir.'msbar.dbf' : $target_dir.'msbar.DBF',0) or die("Error! Could not open dbase database file MSBAR");
    ...
    mysqli_query($cnmy, "DELETE FROM $dbname.subdist_mscust_ljf");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { mysqli_close($cnmy); echo "Error DELETE CUSTOMER SDM : $erropesan"; exit; }
    
    //IT
    if ($plogit_akses==true) {
        mysqli_query($cnit, "DELETE FROM $dbname.subdist_mscust_ljf");
    ...
    $jum_record=0;
    $pinsert=dbase_open(file_exists($target_dir.'mscust.dbf') ? $target_dir.'mscust.dbf' : $target_dir.'mscust.DBF',0) or die("Error! Could not open dbase database file CUSTOMER");
    if ($pinsert){
        $jum_record=dbase_numrecords($pinsert);
        ...
        for ($ind=1;$ind<=$jum_record;$ind++){
            ...
            $query = "INSERT INTO $dbname.subdist_mscust_ljf (CUSTID, CUSTNM, ALAMAT, KOTAID, "
    ...
    mysqli_query($cnmy, "CREATE TABLE $dbname.mssales (PURUTAN BIGINT PRIMARY KEY NOT NULL AUTO_INCREMENT, CUSTID VARCHAR(15), TGLJUAL date, NOJUAL VARCHAR(20), BRGID VARCHAR(5), HARGA DOUBLE, QBELI DOUBLE, QBONUS DOUBLE, DPL DOUBLE, _NullFlags longtext, NODPL VARCHAR(100))");
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { mysqli_close($cnmy); echo "Error CREATE TABLE MSBAR : $erropesan"; exit; }
    
    mysqli_query($cnmy, "CREATE INDEX inx on $dbname.mssales (PURUTAN)");
    ...
    if ($plogit_akses==true) {
        ...
        mysqli_query($cnit, "CREATE TABLE $dbname.mssales (PURUTAN BIGINT PRIMARY KEY NOT NULL AUTO_INCREMENT, CUSTID VARCHAR(15), TGLJUAL date, NOJUAL VARCHAR(20), BRGID VARCHAR(5), HARGA DOUBLE, QBELI DOUBLE, QBONUS DOUBLE, DPL DOUBLE, _NullFlags longtext, NODPL VARCHAR(100))");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { mysqli_close($cnit); echo "IT... Error CREATE TABLE MSBAR : $erropesan"; exit; }
        
        mysqli_query($cnit, "CREATE INDEX inx on $dbname.mssales (PURUTAN)");
    ...
    $jum_record=0;
    $pinsert=dbase_open(file_exists($target_dir.'sales.dbf') ? $target_dir.'sales.dbf' : $target_dir.'sales.DBF',0) or die("Error! Could not open dbase database file SALES");
    if ($pinsert){
        $jum_record=dbase_numrecords($pinsert);
        ...
        for ($ind=1;$ind<=$jum_record;$ind++){
            ...
            $pfile6=$record[6];
            $pfile7=$record[7];
            if (isset($record[8])) $pfile8=trim($record[8]);
            ...
            $query = "INSERT INTO $dbname.mssales (CUSTID, TGLJUAL, NOJUAL, BRGID, "
                    . " HARGA, QBELI, QBONUS, DPL, _NullFlags, NODPL)values"
                    . " ('$pfile0', '$pfile1', '$pfile2', '$pfile3', "
                    . " '$pfile4', '$pfile5', '$pfile6', '$pfile7', '$pfile8', '$pfile10')";
    ...
    $query = "CREATE TABLE dbtemp.msbar_f (PURUTAN BIGINT(20) PRIMARY KEY NOT NULL, BRGID VARCHAR(10), BRGNM VARCHAR(40), 
            BRGID_SUB VARCHAR(15), BRGNM_SUB VARCHAR(40), UNITPCS VARCHAR(10),
            KEY (PURUTAN)
            ) ENGINE = FEDERATED CONNECTION = 'mysql://readonly:r4h4s14@192.168.88.189:3303/dbsales/msbar'";
    mysqli_query($cnms, $query);
    
    
    mysqli_query($cnms, "DROP TABLE IF EXISTS mkt.msbar");
    mysqli_query($cnms, "CREATE TABLE mkt.msbar (SELECT * FROM dbtemp.msbar_f)");
    */
    ...
    $query ="select * from $dbname.mssales where left(tgljual,7)<>'$pbulan_per'";//and cabangid='LMP' and subdist='EP'
