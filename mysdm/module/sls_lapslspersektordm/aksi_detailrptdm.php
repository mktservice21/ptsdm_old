<?php
    ...
    $query = "select * from mkt.icust WHERE icustid "
            . " IN (select distinct icustid FROM $tmp01) $filidsektor";
    $query = "create TEMPORARY table $tmp03 ($query)"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    $query = "UPDATE $tmp03 as a JOIN $tmp01 as b on a.icustid=b.icustid SET a.icabangid=b.icabangid, a.areaid=b.areaid"; 
...
<div id='n_content'>
    ...
    if ($ppilihrpt!="excel") {
        ...
        echo "<tr>";
            echo "<td>";
                echo "<a class='no-print btn button1' href='eksekusi3.php?module=$pmodule&act=input&idmenu=$pidmenu&ket=excel&ipilih=$ppilih&iprd=$pprod&pper1=$iper1&iprd=$pprod&pper2=$iper2&pper2=$iper2&pcb=$icab&pdmid=$idmid&idiv=$pdiv&qval=$pval&jns=$pjns&incpoth=$pncpoth&niddist=$piddist' target='_blank'>EXCEL</a>";
            echo "</td>";
            echo "<td>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</td>";
            echo "<td>";
            echo "</td>";
