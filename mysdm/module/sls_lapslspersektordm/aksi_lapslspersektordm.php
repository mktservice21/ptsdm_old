<?php
    ...
    ////$query = "select * from sls.icust WHERE CONCAT(icabangid,areaid,IFNULL(icustid,'')) IN (select distinct CONCAT(IFNULL(icabangid,''),IFNULL(areaid,''),IFNULL(icustid,'')) FROM $tmp01)";
    //$query = "select * from sls.icust WHERE CONCAT(icabangid,areaid,IFNULL(icustid,'')) IN $filtercabcus";
    ...
    if ($ppilhqtyval=="A") {
        ...
        if ($pjenissektor=="J")
            $tmp07="dbtemp.tmplapslssektordm07_1854_04222020112324";
        else
            $tmp07="dbtemp.tmplapslssektordm07_1854_04222020112352";
