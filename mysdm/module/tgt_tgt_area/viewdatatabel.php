<?php
    ...
    while ($row= mysqli_fetch_array($tampil)) {
        ...
        $query ="ALTER TABLE $tmp03 ADD COLUMN `$npidarea` DECIMAL(20,2)";
        mysqli_query($cnms, $query);
        $erropesan = mysqli_error($cnms); if (!empty($erropesan)) { goto hapusdata; }
        
        $query ="UPDATE $tmp03 SET `$npidarea`=(SELECT SUM($npilrptby) FROM $tmp02 WHERE areaid='$npidarea' AND $tmp02.iprodid=$tmp03.iprodid)";
        mysqli_query($cnms, $query);
        $erropesan = mysqli_error($cnms); if (!empty($erropesan)) { goto hapusdata; }
