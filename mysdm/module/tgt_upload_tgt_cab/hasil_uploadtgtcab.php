<?php
    ...
    $now=date("mdYhis");
    $tmp01 =" dbtemp.DTUPTGTCABUSR_01".$_SESSION['USERID']."_$now ";
    $tmp02 =" dbtemp.DTUPTGTCABUSR_02".$_SESSION['USERID']."_$now ";
    $tmp03 =" dbtemp.DTUPTGTCABUSR_03".$_SESSION['USERID']."_$now ";
    ...
    $query ="select DISTINCT icabangid, areaid from sls.iarea WHERE aktif='Y' AND icabangid='$pidcabpil'";
    $query = "CREATE  TABLE $tmp02($query)";
    mysqli_query($cnms, $query);
    $erropesan = mysqli_error($cnms); if (!empty($erropesan)) { mysqli_query($cnms, "DROP  TABLE $tmp01"); exit; }
                    ...
                    <div class='x_panel'>
                        ...
                        <div id='c-data'>
                            ...
                            if ($ketemu_>0) {
                                ...
                                $query ="DELETE FROM tgt.targetcab WHERE DATE_FORMAT(bulan,'%Y%m')='$pperiode_' AND icabangid='$pidcabpil'";
                                mysqli_query($cnms, $query);
                                $erropesan = mysqli_error($cnms); if (!empty($erropesan)) { echo "Error DELETE DATA : $erropesan"; mysqli_query($cnms, "DROP TABLE $tmp01"); mysqli_query($cnms, "DROP TABLE $tmp02"); mysqli_close($cnms); exit; }
                                
                                
                                $query ="INSERT INTO tgt.targetcab (bulan, divprodid, iprodid, hna, qty, value, icabangid)"
                                        . " SELECT PERIODE, DIVPRODID, KDPROD, PHNA, PQTY, PVALUE, '$pidcabpil' FROM $tmp01 ";
                                mysqli_query($cnms, $query);
                                $erropesan = mysqli_error($cnms); if (!empty($erropesan)) { echo "Error DELETE DATA : $erropesan"; mysqli_query($cnms, "DROP TABLE $tmp01"); mysqli_query($cnms, "DROP TABLE $tmp02"); mysqli_close($cnms); exit; }
                                
                                
                                //KOSONGKAN DATA DI TABEL tgt.targetarea ALL AREA sesuai periode dan cabang
                                $query ="DELETE FROM tgt.targetarea WHERE DATE_FORMAT(bulan,'%Y%m')='$pperiode_' AND icabangid='$pidcabpil'";
                                mysqli_query($cnms, $query);
                                ...
                                if (!empty($erropesan)) { 
                                    ...
                                    mysqli_query($cnms, "DELETE FROM tgt.targetcab WHERE DATE_FORMAT(bulan,'%Y%m')='$pperiode_' AND icabangid='$pidcabpil'");
                                ...
                                $query ="select DISTINCT a.icabangid, a.areaid, b.DIVPRODID divprodid, b.KDPROD iprodid, b.PHNA hna, "
                                        . " CAST(0 as DECIMAL(20,2)) as qty, CAST(0 as DECIMAL(20,2)) as value from $tmp02 a, $tmp01 b";
                                ...
                                $query ="INSERT INTO tgt.targetarea (bulan, divprodid, iprodid, hna, qty, value, icabangid, areaid, user)"
                                        . " SELECT '$ptglpilihupload' as bulan, divprodid, iprodid, hna, qty, value, icabangid, areaid, '$_SESSION[IDCARD]' FROM $tmp03 ";
                                mysqli_query($cnms, $query);
                                $erropesan = mysqli_error($cnms); 
                                if (!empty($erropesan)) { 
                                    ...
                                    mysqli_query($cnms, "DROP TABLE $tmp01");
                                    mysqli_query($cnms, "DROP TABLE $tmp02");
                                    mysqli_query($cnms, "DROP TABLE $tmp03");
