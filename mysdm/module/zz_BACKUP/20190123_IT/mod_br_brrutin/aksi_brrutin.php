...
elseif ($module=='entrybrrutin')
{
    ...
    if ($num_records) {
        for ($i=1; $i < $num_records; $i++) {
            ...
            if (!empty($bltotal)) {
                ...
                $coadet = getfieldcnit("select COA4 as lcfields from dbmaster.posting_coa_rutin where divisi='$pdivprodid' AND nobrid='$blid'");
                
                $query = "insert into $dbname.t_brrutin1 (idrutin,nobrid,qty,rp,rptotal,notes,coa) "
                        . " values ('$kodenya','$blid','$blqty','$blnilai','$bltotal','$blnote', '$coadet')";
                mysqli_query($cnmy, $query);
    ...
    if ( (int)$pjabatanid==38 OR (int)$pjabatanid==19) {
        ...
        if ($ketemu>0) {
            $ats= mysqli_fetch_array(mysqli_query($cnit, $query));
            $atasangsm=$ats["gsm"];
    ...
    if ($_SESSION['GROUP']=="28") {
        $query = "update $dbname.t_brrutin0 set tgl_atasan1=NOW(), tgl_atasan2=NOW(), tgl_atasan3=NOW() WHERE idrutin='$kodenya'";
        mysqli_query($cnmy, $query);
    }
