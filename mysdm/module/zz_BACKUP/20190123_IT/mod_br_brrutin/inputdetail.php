<div class='tbldata'>
    <table id='datatable' class='datatable table nowrap table-striped table-bordered' width="100%">
        ...
        <?PHP
        $no=1;
        $tampil = mysqli_query($cnit, "SELECT nobrid, nama, jumlah, qty FROM dbmaster.t_brid where kode=1 and aktif='Y' order by nobrid");
        while ($uc=mysqli_fetch_array($tampil)){
            ...
            if ($_GET['act']=="editdata"){
                $cari = mysqli_query($cnit, "SELECT * FROM dbmaster.t_brrutin1 where idrutin='$_GET[id]' and nobrid='$uc[nobrid]'");
        ...
        while ($uc=mysqli_fetch_array($tampil)){
            ...
            if ($_GET['act']=="editdata"){
                $uqyu=  getfieldit("select qty as lcfields from dbmaster.t_brrutin1 where idrutin='$_GET[id]' and nobrid='$uc[nobrid]'");
                $unilai=  getfieldit("select rp as lcfields from dbmaster.t_brrutin1 where idrutin='$_GET[id]' and nobrid='$uc[nobrid]'");
                $unote=  getfieldit("select notes as lcfields from dbmaster.t_brrutin1 where idrutin='$_GET[id]' and nobrid='$uc[nobrid]'");
                if (!empty($uc['qty']) AND $uc['qty']==1){
                    $total=floatval($uqyu)*floatval($unilai);
                }else{
                    $total=  getfieldit("select rptotal as lcfields from dbmaster.t_brrutin1 where idrutin='$_GET[id]' and nobrid='$uc[nobrid]'");
                    $unilai="";
                    $uqyu=$qtyjml;
                }
            }else{
