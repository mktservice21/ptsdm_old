...
$noidurut = "";
$no=1;
$tampil = mysqli_query($cnit, "SELECT nobrid, nama, jumlah, qty, groupid FROM dbmaster.t_brid where kode=1 and aktif='Y' order by nobrid");
while ($uc=mysqli_fetch_array($tampil)){
    ...
    for ($i=1; $i <=$tjml; $i++) {
        ...
        if ($_GET['act']=="editdata"){
            ...
            $cari = mysqli_query($cnit, "SELECT * FROM dbmaster.t_brrutin1 where idrutin='$_GET[id]' and nobrid='$uc[nobrid]' $noidurut order by nourut");
