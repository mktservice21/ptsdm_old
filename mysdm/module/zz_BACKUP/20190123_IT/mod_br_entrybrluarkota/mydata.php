...
$sql = "SELECT divisi, idrutin, DATE_FORMAT(tgl,'%d/%m/%Y') as tgl, DATE_FORMAT(bulan,'%M %Y') as bulan, DATE_FORMAT(periode1,'%d/%m/%Y') as periode1, "
        . " DATE_FORMAT(periode2,'%d/%m/%Y') as periode2, "
        . " divisi, karyawanid, nama, areaid, FORMAT(jumlah,0,'de_DE') as jumlah, keterangan, jabatanid, atasan1, atasan2, tgl_atasan1, tgl_atasan2, tgl_atasan3, tgl_atasan4 ";
$sql.=" FROM dbmaster.v_brrutin0_mydata ";


$sql.=" WHERE kode=2 AND stsnonaktif <> 'Y' "; //kode = 2 BIAYA LUAR KODA
//$sql.=" AND Date_format(bulan, '%Y-%m') between '$tgl1' and '$tgl2' ";
$sql.=" AND ((DATE_FORMAT(periode1,'%Y-%m') between '$tgl1' AND '$tgl2') OR (DATE_FORMAT(periode2,'%Y-%m') between '$tgl1' AND '$tgl2')) ";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	*/
	...
	$g_ats2 = getfieldcnit("select gbr_atasan2 as lcfields from dbmaster.t_brrutin0 where idrutin='$idno'");
    ...
    $gambar = getfieldcnit("select count(*) as lcfields from dbimages.img_brrutin1 where idrutin='$idno' LIMIT 1");
