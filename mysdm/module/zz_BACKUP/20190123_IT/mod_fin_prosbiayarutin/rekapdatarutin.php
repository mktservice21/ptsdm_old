...
<?php
	...
	c1.NAMA4 nama_coa,
        ...
        LEFT JOIN dbmaster.coa_level4 c1 ON c1.COA4 = b1.coa WHERE br.kode=1 AND br.stsnonaktif <> 'Y' AND br.idrutin in $noidbr ";
    
    $query = "create temporary table $tmp01 ($query)";
    mysqli_query($cnit, $query);
    // update atasan NN
    mysqli_query($cnit, "UPDATE $tmp01 set tgl_atasan1=null WHERE IFNULL(gbr_atasan1,'')=''");
    mysqli_query($cnit, "UPDATE $tmp01 set tgl_atasan2=null WHERE IFNULL(gbr_atasan2,'')=''");
    mysqli_query($cnit, "UPDATE $tmp01 set tgl_atasan3=null WHERE IFNULL(gbr_atasan3,'')=''");
    mysqli_query($cnit, "UPDATE $tmp01 set tgl_atasan4=null WHERE IFNULL(gbr_atasan4,'')=''");
    ...
    mysqli_query($cnit, $query);
    mysqli_query($cnit, "UPDATE $tmp02 a set jmlhari=null, nilairp=null");
    
    $totalfield=1;
    $query = "select * from dbmaster.t_brid where kode=1 order by nobrid";
    $tampil = mysqli_query($cnit, $query);
    while ($row=mysqli_fetch_array($tampil)) {
        $noid=$row['nobrid'];
        mysqli_query($cnit, "ALTER TABLE $tmp02 add COLUMN F$noid DECIMAL(30,2)");
    ...
    <div id="kotakjudul">
        <div id="isikiri">
            <table class='tjudul' width='100%'>
                ...
                <tr><td width="150px"><b>Rekap Biaya Rutin</b></td></tr>
            </table>
        </div>
    </div>
            ...
            <tbody>
                <?PHP
                    ...
                    while ($row=mysqli_fetch_array($tampil)) {
                        ...
                        $zz=1;
                        $query2 = "select * from dbmaster.t_brid where kode=1 order by nobrid";
