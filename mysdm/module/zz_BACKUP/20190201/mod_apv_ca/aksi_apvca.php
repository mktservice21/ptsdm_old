    ...
    if (!empty($noidbr) AND !empty($karyawanapv) AND !empty($lvlposisi)) {
        ...
        if ($act=="simpan_ttdallam") {
            ...
            if (!empty($tglatasannya)) {
                ...
                mysqli_query($cnmy, "update $dbname.t_ca0 set $idatasan4 $tglatasannya=NOW(), $gbratasannya='$img' WHERE idca in $noidbr $tglatasannya_bawah $tglatasannya_atas $stsnonaktifnya $apvfinance");
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
                
                $flter = "SELECT DISTINCT IFNULL(idca,'') FROM $dbname.t_ca0 WHERE idca in $noidbr $tglatasannya_bawah $tglatasannya_atas $stsnonaktifnya $apvfinance";
                $query = "UPDATE dbimages.t_ca0_ttd set $gbratasannya='$img' WHERE idca IN ($flter) AND idca IN $noidbr";
            ...
            if (!empty($tglatasannya)) {
                mysqli_query($cnmy, "update $dbname.t_ca0 set $idatasan4 $tglatasannya=null, $gbratasannya=null WHERE idca in $noidbr $tglatasannya_bawah $tglatasannya_atas $stsnonaktifnya $apvfinance");
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
                
                $flter = "SELECT DISTINCT IFNULL(idca,'') FROM $dbname.t_ca0 WHERE idca in $noidbr $tglatasannya_bawah $tglatasannya_atas $stsnonaktifnya $apvfinance";
                $query = "UPDATE dbimages.t_ca0_ttd set $gbratasannya=null WHERE idca IN ($flter) AND idca IN $noidbr";
                mysqli_query($cnmy, $query);
                //$erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
            ...
            mysqli_query($cnmy, "update $dbname.t_ca0 set stsnonaktif='Y', userid='$karyawanapv', "
                    . " keterangan=CONCAT(keterangan,'$kethapus',', $_SESSION[NAMALENGKAP], ', NOW()) WHERE idca in $noidbr $apvfinance");
