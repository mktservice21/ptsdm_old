...
if (isset($_GET['ca'])) {
    ...
    $edit = mysqli_query($cnmy, "SELECT * FROM dbmaster.v_ca0 WHERE idca='$idca'");
    $r    = mysqli_fetch_array($edit);
    
    $tglberlku = date('d/m/Y', strtotime($r['periode']));
    ...
    if ($_GET['act']=="editdata"){
        ...
        $edit = mysqli_query($cnmy, "SELECT * FROM dbmaster.v_sewa WHERE idsewa='$_GET[id]'");
        $r    = mysqli_fetch_array($edit);
        $idklaim=$r['idsewa'];
        $tglberlku = date('d/m/Y', strtotime($r['tglmulai']));
        $tgl1 = date('d/m/Y', strtotime($r['tglmulai']));
        $tgl2 = date('d/m/Y', strtotime($r['tglmulai']));
        $idajukan=$r['karyawanid']; 
        $nmajukan=$r['nama']; 
        $divisi=$r['divisi']; 
        $patasanid=$r['atasan1']; 
        ...
        if (!empty($r['idca'])) {
            ...
            $rutin_jumlahrp = getfieldit("select sum(jumlah) as lcfields from dbmaster.t_brrutin0 WHERE idca='$idca' AND stsnonaktif <> 'Y'");
            $sewa_jumlahrp = getfieldit("select sum(jumlah) as lcfields from dbmaster.t_sewa WHERE idca='$idca' AND idsewa <> '$idklaim' AND stsnonaktif <> 'Y'");
            if (empty($rutin_jumlahrp)) $rutin_jumlahrp = 0;
            if (empty($sewa_jumlahrp)) $sewa_jumlahrp = 0;
            $saldoca = (double)$ca_jumlahrp-(double)$rutin_jumlahrp-(double)$sewa_jumlahrp;
        }
                    ...
                    <div class='x_panel'>
                        <div class='x_content'>
                            <div class='col-md-12 col-sm-12 col-xs-12'>
                                ...
                                <div class='form-group'>
                                    <label class='control-label col-md-3 col-sm-3 col-xs-12' for=''>Periode / Tahun <span class='required'></span></label>
                                    <div class='col-xs-5'>
                                        <input type="number" class="form-control" id='e_jmlbln' name='e_jmlbln' min="1" value="<?PHP echo $jmlbln; ?>">
                                    </div>
                                </div>
                                
                                
                                <div class='form-group'>
                                    <label class='control-label col-md-3 col-sm-3 col-xs-12' for='mytgl01'>Tgl. Mulai </label>
                                    <div class='col-md-3'>
...
<script>
    ...
    function refresh_ca() {
        ...
        document.getElementById("demo-form2").action = "module/mod_br_entrysewa/refreshca.php?module="+module+"&act=editdata"+"&idmenu="+idmenu;
    ...
    function disp_confirm(pText_, ket)  {
        ...
        if (ok_) {
            ...
            if (r==true) {
                ...
                document.getElementById("demo-form2").action = "module/mod_br_entrysewa/aksi_entrysewa.php?module="+module+"&act="+ket+"&idmenu="+idmenu;
                document.getElementById("demo-form2").submit();
                return 1;
            }
        } else {
