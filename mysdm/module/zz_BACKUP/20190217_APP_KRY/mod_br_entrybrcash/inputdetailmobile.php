...
$noidurut = "";
$no=1;
$tampil = mysqli_query($cnmy, "SELECT nobrid, nama, jumlah, qty, groupid FROM dbmaster.t_brid where kode=3 and aktif='Y' order by nobrid");
while ($uc=mysqli_fetch_array($tampil)){
    ...
    for ($i=1; $i <=$tjml; $i++) {
        ...
        if ($_GET['act']=="editdata"){
            ...
            $cari = mysqli_query($cnmy, "SELECT * FROM dbmaster.t_ca1 where idca='$_GET[id]' and nobrid='$uc[nobrid]' $noidurut order by nourut");
