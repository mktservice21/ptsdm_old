...
<?php
    ...
    $tmp05 =" dbtemp.DTBRRETRL05_".$_SESSION['IDCARD']."_$now ";
	...
	a.rptotal,
        ...
        a.rptotal as ca2,
        a.rptotal as jmltrans , b.keterangan, b.nobukti  
        FROM
    ...
    mysqli_query($cnit, "UPDATE SET $tmp01 idca=''");
    //mysqli_query($cnit, "UPDATE SET $tmp01 divisi='CAN' WHERE jabatanid in ('04', '05', '08', '10', '18', '20')");
    
    
    mysqli_query($cnit, "update $tmp01 set ca1=0, ca2=0, selisih=0, jmltrans=0, tgltransreal=null");
    
    $query = "select idca, idrutin, keterangan, tgltrans, coa4, nama_coa4, icabangid_o icabangid, nama_cabang_o nama_cabang, areaid_o areaid, nama_area_o nama_area, karyawanid, nama_karyawan, divisi,
        nobrid, nama_des, deskripsi, tgl1, tgl2, notes, ca1, selisih, ca2, jmltrans, tgltransreal, nobukti, sum(jumlah) jumlah, sum(qty) qty, sum(rp) rp, sum(rptotal) rptotal
        from $tmp01 
    ...
    $filkaryawan="";
    if ($stsreport=="B") $filkaryawan=" AND karyawanid in (select distinct karyawanid from $tmp01) ";
    
    $query="select idca, karyawanid, divisi, periode, jumlah from dbmaster.t_ca0 where stsnonaktif <> 'Y' AND jenis_ca='lk' and divisi<>'OTC' AND "
            . " DATE_FORMAT(periode,'%Y-%m') between '$periode1' AND '$periode2' "
    ...
    if ($stsreport=="B") {
        mysqli_query($cnit, "DELETE FROM $tmp03 WHERE idca IN (select distinct ifnull(idca1,'') FROM dbmaster.t_brrutin_ca_close) AND DATE_FORMAT(periode,'%Y-%m') = '$periode1'");
        mysqli_query($cnit, "DELETE FROM $tmp03 WHERE idca IN (select distinct ifnull(idca2,'') FROM dbmaster.t_brrutin_ca_close) AND DATE_FORMAT(periode,'%Y-%m') = '$periode2'");
    }elseif ($stsreport=="S") {
        mysqli_query($cnit, "DELETE FROM $tmp03 WHERE idca NOT IN (select distinct ifnull(idca1,'') FROM dbmaster.t_brrutin_ca_close) AND DATE_FORMAT(periode,'%Y-%m') = '$periode1'");
        mysqli_query($cnit, "DELETE FROM $tmp03 WHERE idca NOT IN (select distinct ifnull(idca2,'') FROM dbmaster.t_brrutin_ca_close) AND DATE_FORMAT(periode,'%Y-%m') = '$periode2'");
    }
    
    
    mysqli_query($cnit, "UPDATE $tmp03 as a SET a.divisi=IFNULL((select distinct divisi from $tmp01 as b WHERE a.karyawanid=b.karyawanid LIMIT 1), a.divisi)");
    
    
    
    
    $query = "INSERT INTO $tmp02 (karyawanid, nama_karyawan, divisi)"
            . " select a.karyawanid, b.nama, a.divisi FROM $tmp03 as a JOIN hrd.karyawan b on a.karyawanid=b.karyawanId WHERE "
    ...
    $query = "update $tmp02 set idca=ifnull((select idca from $tmp03 where $tmp03.karyawanid=$tmp02.karyawanid AND DATE_FORMAT($tmp03.periode,'%Y-%m')='$periode1' LIMIT 1),'')"; 
    mysqli_query($cnit, $query);
    
    
    $query = "update $tmp02 set ca1=ifnull((select sum(jumlah) from $tmp03 where $tmp03.divisi=$tmp02.divisi AND $tmp03.karyawanid=$tmp02.karyawanid AND DATE_FORMAT($tmp03.periode,'%Y-%m')='$periode1'),0)"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    
    $query = "update $tmp02 set ca2=ifnull((select sum(jumlah) from $tmp03 where $tmp03.divisi=$tmp02.divisi AND $tmp03.karyawanid=$tmp02.karyawanid AND DATE_FORMAT($tmp03.periode,'%Y-%m')='$periode2'),0)"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    
    $query = "create temporary table $tmp04 (select distinct idrutin, karyawanid, divisi, jumlah from $tmp01)"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    
    $query = "update $tmp02 set jumlah=ifnull((select sum(jumlah) from $tmp04 where $tmp04.divisi=$tmp02.divisi AND $tmp04.karyawanid=$tmp02.karyawanid),0)"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    ...
    if ($ketemucls>0){
        mysqli_query($cnit, "UPDATE $tmp02 set ca1=0, ca2=0 WHERE idrutin not in (select IFNULL(idrutin,'') from dbmaster.t_brrutin_ca_close)");
    }
        
    
    $query="INSERT INTO $tmp02 (karyawanid, nama_karyawan, divisi, ca2)
        ...
        <table id='datatable2' class='table table-striped table-bordered example_2' border="1px solid black">
            ...
            <tbody>
                <?PHP
                    ...
                    $query = "select * from $tmp02 order by divisi, nama_karyawan, karyawanid, idrutin";
