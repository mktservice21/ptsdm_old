        ...
        <form method='POST' action='<?PHP echo "$aksi?module=$_GET[module]&act=input&idmenu=$_GET[idmenu]"; ?>' id='demo-form2' name='form1' data-parsley-validate class='form-horizontal form-label-left'>
            ...
            <div class='col-md-12 col-sm-12 col-xs-12'>
                <div class='x_panel'>
                    ...
                    <div <?PHP echo $tutupnospd; ?> class='form-group'>
                        <label class='control-label col-md-3 col-sm-3 col-xs-12' for=''><input type="checkbox" id="chk_sel" name="chk_sel" onClick="PilihData('chk_sel')" value="<?PHP echo $valuechktgl; ?>" <?PHP echo $pilihcektanggal; ?>> <span class='required'></span></label>
                        <div class='col-md-3'>
                            <div class="form-group">
                                <div class='input-group date' id=''>
                                    <input type='text' id='tglto' name='bulan2' required='required' class='form-control' placeholder='tgl akhir' value='<?PHP echo $tgl_terakhir; ?>' placeholder='dd mmm yyyy' Readonly>
                                    <span class="input-group-addon">
                                       <span class="glyphicon glyphicon-calendar"></span>
                                    </span>
                                </div>
                    ...
                    <div class='form-group'>
                        ...
                        <div class='col-md-3'>
                            <div class="form-group">
                                <div class='input-group date' id=''>
                                    <select class='form-control input-sm' id="cb_jenis" name="cb_jenis" data-live-search="true">
...
<script>
    function PilihData(nmbuton){
        var button = document.getElementById(nmbuton);
        if(button.value == 'select'){
            document.getElementById('tglto').value="";
            button.value = 'deselect'
        }else{
            document.getElementById('tglto').value=document.getElementById('tglfrom').value;
            button.value = 'select';
        }
    }  
</script>
...
<script>
    ...
    function KlikDataTabel() {
        var ket="";
        var eselpilih=document.getElementById('chk_sel');
        ...
        var elampiran=document.getElementById('cb_jenis').value;
        var eidc=<?PHP echo $_SESSION['USERID']; ?> ;
        
        var pilihtgl="";
        //if (eselpilih=="on" OR eselpilih=="select") var pilihtgl="pilih";
        if (eselpilih.checked == true) var pilihtgl="pilih";
        ...
        $.ajax({
            ...
            data:"eket="+ket+"&uperiode1="+etgl1+"&udivisi="+edivisi+"&uidc="+eidc+"&ucabang="+"&utglf="+etglf+"&utglt="+etglt+"&uidinput="+eid+"&upilihtgl="+pilihtgl+"&ulampiran="+elampiran,
            success:function(data){
                $("#c-data").html(data);
                $("#loading").html("");
    ...
    function HitungTotalJumlah() {
        ...
        var nobridL=CariKataID("chkbox_idL[]");
        ...
        */
        ...
        $.ajax({
            ...
            data:"ufila="+nobridA+"&ufilb="+nobridB+"&ufilc="+nobridC+
                    ...
                    "&ufilg="+nobridG+"&ufilh="+nobridH+"&ufili="+nobridI+"&ufilj="+nobridJ+"&ufilk="+nobridK+"&ufill="+nobridL,
            success:function(data){
                $("#loading2").html("");
        ...
        $(function() {
            $('#e_tglberlaku').datepicker({
                ...
                onSelect: function(dateStr) {
                    ShowDataKode();
                    document.getElementById('tglfrom').value=document.getElementById('e_tglberlaku').value;
                    <?PHP if ($_SESSION['DIVISI']=="OTC" AND $_GET['act']=="editdata") { ?> 
                        KlikDataTabel();
                    <?PHP } ?>
                } 
            });
