<?PHP
    ...
    $etglpilih=$_POST['upilihtgl'];
    $edivisi=$_POST['udivisi'];
    $eidinput=$_POST['uidinput'];
    $jenis=$_POST['ulampiran'];
...
<?PHP
    ...
    if (empty($edivisi) OR ($edivisi == "OTC")) {
        ...
        $sql = "select 'D' kodeinput, 'BROTC' pengajuan, 'OTC' divisi, tglbr, icabangid_o, brOtcId, subpost, kodeid, COA4, noslip,
            ...
            FROM hrd.br_otc WHERE brOtcId not in (SELECT DISTINCT ifnull(brOtcId,'') from hrd.br_otc_reject) $filtertgl $filteridsudah $filterlampiran ";
    ...
    //gaji SPG otc
    $filtertgl = " AND DATE_FORMAT(a.tglbr,'%Y-%m-%d') between '$periode1' AND '$periode2' ";
    $filteridsudah = " AND a.idbrspg NOT IN (SELECT DISTINCT ifnull(bridinput,'') FROM dbmaster.t_suratdana_br1 WHERE kodeinput='I' $filtereditid)";
    //if (!empty($eidinput)) $filteridsudah="";
    if (!empty($edivisi) AND ($edivisi <> "*") AND ($edivisi <> "0")) $fdivisi = " AND b.divisi='$edivisi' ";
    $sql = "SELECT
        'L' kodeinput,  'GAJI SPG' pengajuan, 'OTC' as divisi,
        a.tglbr,
        a.icabangid,
        br.idbrspg,
        br.coa4,
        ...
        JOIN mkt.spg b ON a.id_spg = b.id_spg WHERE a.stsnonaktif<>'Y' AND ifnull(br.rptotal,0)<>0 $filtertgl $filteridsudah
        GROUP BY 1,2,3,4,5,6";
    
    $query = "INSERT INTO $tmp01 (kodeinput, pengajuan, divisi, tgl, icabangid, idinput, coa4, karyawanid, keterangan, jumlah) $sql";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { 
        mysqli_query($cnit, $droptable01);
        echo $erropesan; exit;
    ...
    <table id='datatableinputb' class='table table-striped table-bordered' width='100%'>
        ...
        <?PHP
            ...
            while ($rowp= mysqli_fetch_array($tampilp)) {
                ...
                if ($pkodeinp=="L") $namapengajuannya="GAJI SPG OTC";
                ...
                echo "<td nowrap></td>";
                echo "<td nowrap colspan=4><b><u>$namapengajuannya</u></b></td>";
...
<?PHP
    ...
    echo "<input type='checkbox' value='0-L' name='chkbox_idL[]' id='chkbox_idL[]' class='cekbr'>";
    echo "</div>";
?>
