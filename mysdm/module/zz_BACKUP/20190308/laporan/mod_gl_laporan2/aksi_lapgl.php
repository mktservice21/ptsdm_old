...
<?PHP
    ...
    //A = DCC, DSS & NON ,- B = KLAIM ==> belum pakai divisi ,- C = KAS ,- D = BR OTC ,- E = Budget Biaya Rutin Gelondongan COA
    //F = RUTIN ,- G = LUAR KOTA ,- H = CA RUTIN ,- I = CA LUAR KOTA
    
    $query = "UPDATE $tmp02 set debit=0";
    mysqli_query($cnit, $query);
    
    $query = "UPDATE $tmp02 set kredit=ifnull(realisasi,0) WHERE kodeinput IN ('A', 'D')";
    mysqli_query($cnit, $query);
    
    $query = "UPDATE $tmp02 set kredit=ifnull(jumlah,0) WHERE kodeinput NOT IN ('A', 'D')";
    mysqli_query($cnit, $query);
    ...
    <?PHP if ($erpttipe=="D") { ?>
        ...
        <table id='datatable2' class='table table-striped table-bordered example_2' border="1px solid black">
            ...
            <?PHP
                ...
                $query = "select distinct DATE_FORMAT(tgl,'%Y%m') tglbln, DATE_FORMAT(tgl,'%M %Y') bulan from $tmp02 order by DATE_FORMAT(tgl,'%Y%m')";
                $tampilbln=mysqli_query($cnit, $query);
                while ($rowb= mysqli_fetch_array($tampilbln)) {
                    $ptglbln=$rowb['tglbln'];
                    $pbulan=$rowb['bulan'];
                    echo "<tr><td colspan=14>&nbsp;&nbsp;<b>$pbulan</b></td></tr>";
                    
                    $query = "select distinct divisi from $tmp02 WHERE DATE_FORMAT(tgl,'%Y%m')='$ptglbln' order by divisi";
                    ...
                    while ($row= mysqli_fetch_array($tampil)) {
                        ...
                        $query2 = "select distinct divisi, coa4 from $tmp02 WHERE DATE_FORMAT(tgl,'%Y%m')='$ptglbln' AND divisi='$pdivisi' order by divisi, coa4";
                        ...
                        while ($row2= mysqli_fetch_array($tampil2)) {
                            ...
                            $query3 = "select * from $tmp02 WHERE DATE_FORMAT(tgl,'%Y%m')='$ptglbln' AND divisi='$pdivisi' AND coa4='$pcoa4' order by divisi, coa4, pengajuan, tgl";
                ...
                echo "<tr>";
                echo "<td colspan=10><b>GRAND TOTAL</b></td>";
