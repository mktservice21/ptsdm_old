...
if ($_GET['module']=="caribuktisudahada"){
    ...
    $query = "SELECT br.divisi, sum(br.jumlah) jumlah 
        FROM dbmaster.t_brrutin0 AS br WHERE br.kode=2 AND br.stsnonaktif <> 'Y' AND br.divisi <> 'OTC' $fperiode $fstsapv $filstatuscls GROUP BY 1";
    ...
    if ($records>0){
        while ($sh= mysqli_fetch_array($result)) {
            ...
            if (!empty($kodenya)){
                $query = "INSERT INTO $dbname.t_suratdana_br (idinput, divisi, kodeid, subkode, tgl, nodivisi, jumlah, userid, coa4, tglf, tglt, sts)values"
                        . "('$kodenya', '$ndivisi', '$pkode', '$psubkode', '$tglinput', '$pdivno', '$pjumlah', '$userid', '$pcoa', '$myinpperiode1', '$myinpperiode2', '$stsreport')";
                mysqli_query($cnmy, $query);
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo "Error..."; exit; }
