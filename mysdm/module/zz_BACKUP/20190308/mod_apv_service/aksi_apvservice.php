    ...
    if (!empty($noidbr) AND !empty($karyawanapv) AND !empty($lvlposisi)) {
        ...
        if ($act=="simpan_ttdallam") {
            ...
            if (!empty($tglatasannya)) {
                ...
                mysqli_query($cnmy, "update $dbname.t_service_kendaraan set $idatasan4 $tglatasannya=NOW(), $gbratasannya='$img' WHERE idservice in $noidbr $tglatasannya_bawah $tglatasannya_atas $stsnonaktifnya $apvfinance");
            ...
            if (!empty($tglatasannya)) {
                mysqli_query($cnmy, "update $dbname.t_service_kendaraan set $idatasan4 $tglatasannya=null, $gbratasannya=null WHERE idservice in $noidbr $tglatasannya_bawah $tglatasannya_atas $stsnonaktifnya $apvfinance");
                $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
            ...
            mysqli_query($cnmy, "update $dbname.t_service_kendaraan set stsnonaktif='Y', userid='$karyawanapv', "
                    . " keterangan=CONCAT(keterangan,'$kethapus',', $_SESSION[IDCARD], ', NOW()) WHERE idservice in $noidbr $apvfinance");
