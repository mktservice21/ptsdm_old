...
$kodeid=$_GET['kodeid'];
$subkodeid=$_GET['subkodeid'];
$cekhanya=$_GET['cekhanya'];

if (!empty($kodeid)) {
    if ($kodeid=='06')
        $sql.=" and subpost=$kodeid ";
    else
        $sql.=" and subpost='$kodeid' ";
}
if (!empty($subkodeid)) $sql.=" and kodeid='$subkodeid' ";
if ((int)$cekhanya==0) $sql.=" and ifnull(COA4,'')<>'' ";
if ((int)$cekhanya==1) $sql.=" and ifnull(COA4,'')='' ";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
    $nestedData=array();
    $nestedData[] = $no;
    $nestedData[] = "<input type='checkbox' value='$row[brOtcId]' name='chkbox_id[]' id='chkbox_id[]' class='cekbr'>";
