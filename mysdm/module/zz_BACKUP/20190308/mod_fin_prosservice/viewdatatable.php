...
<?php
    ...
    $_SESSION['PROSCASERVICE_TIPE'] = $isitipe;
    $_SESSION['PROSCASERVICE_KET'] = $cket;
    $_SESSION['PROSCASERVICE_TGL1'] = $mytgl1;
    $_SESSION['PROSCASERVICE_TGL2'] = $mytgl2;
    $_SESSION['PROSCASERVICE_KRY'] = $karyawan;
    $_SESSION['PROSCASERVICE_LVL'] = $lvlposisi;
    $_SESSION['PROSCASERVICE_DIV'] = $divisi;
    $_SESSION['PROSCASERVICE_STSAPV'] = $stsapv;
...
<form method='POST' action='' id='d-form2' name='d-form2' data-parsley-validate class='form-horizontal form-label-left'>
    ...
    <div class='x_content'>
        ...
        <table id='datatableproscaisi' class='table table-striped table-bordered' width='100%'>
            ...
            <tbody>
                <?PHP
                $sql = "SELECT idservice, DATE_FORMAT(tgl,'%d %M %Y') as tgl, DATE_FORMAT(tglservice,'%M %Y') as bulan, DATE_FORMAT(tglservice,'%d/%m/%Y') as periode, "
                        . " divisi, karyawanid, nama, areaid, nama_area, FORMAT(jumlah,0,'de_DE') as jumlah, keterangan, "
                        . " ppn ";
                $sql.=" FROM dbmaster.v_service_kendaraan ";
                $sql.=" WHERE 1=1  ";
                $sql.=" AND Date_format(tglservice, '%Y-%m') between '$tgl1' and '$tgl2' ";
                ...
                while ($row= mysqli_fetch_array($tampil)) {
                    ...
                    $print="<a title='Print / Cetak' href='#' class='btn btn-info btn-xs' data-toggle='modal' "
                        . "onClick=\"window.open('eksekusi3.php?module=entrybrservicekendaraan&brid=$idno&iprint=print',"
...
<script>
    ...
    function ProsesData(ket, cekbr){
        ...
        $.ajax({
            type:"post",
            url:"module/mod_fin_prosservice/aksi_prosservice.php?module="+module+"&idmenu="+idmenu+"&act="+ket,
    ...
    function disp_confirm(pText_, act)  {
        ...
        if (ok_) {
            ...
            if (r==true) {
                ...
                document.getElementById("d-form2").action = "module/mod_fin_prosservice/aksi_prosservice.php?module="+module+"&idmenu="+idmenu+"&act="+act;
