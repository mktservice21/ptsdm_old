...
$sql = "SELECT brId, DATE_FORMAT(tgl,'%d %M %Y') as tgl, DATE_FORMAT(tgltrans,'%d %M %Y') as tgltrans, DATE_FORMAT(tgltrm,'%d %M %Y') as tgltrm, "
        ...
        . "noslip, aktivitas1 ";
$sql.=" FROM dbmaster.v_br0 ";//v_br0_non
//$sql.=" WHERE Date_format(sys_now, '%Y%m')>=DATE_FORMAT((DATE_SUB(curdate(), INTERVAL 2 MONTH)),'%Y%m') and (br = '' and br<>'N') ";// tidak ada
$sql.=" WHERE 1=1 ";
...
if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
    ...
    $sql.=" OR aktivitas2 LIKE '%".$requestData['search']['value']."%' ";
    $sql.=" OR realisasi1 LIKE '%".$requestData['search']['value']."%' ";
    $sql.=" OR jumlah LIKE '%".$requestData['search']['value']."%' ";
    $sql.=" OR jumlah1 LIKE '%".$requestData['search']['value']."%' ";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
    ...
    $nestedData[] = ""
            ...
            */
            ...
            . "<a class='btn btn-info btn-xs' href='?module=$_GET[module]&act=editterima&idmenu=$_GET[idmenu]&nmun=$_GET[nmun]&id=$row[brId]'>Terima</a> "
            . "<a class='btn btn-default btn-xs' href='?module=$_GET[module]&act=edittransfer&idmenu=$_GET[idmenu]&nmun=$_GET[nmun]&id=$row[brId]'>Realisasi</a>"
