...
elseif ($module=='datakendaraan')
{
    ...
    if ($act=='input')
    {
        mysqli_query($cnmy, "INSERT INTO $dbname.t_kendaraan (nopol, jenis, merk, tipe, tglbeli, userid)"
                . " VALUES ('$kodenya', '$pjenis', '$pmerk', '$ptipe', '$pp01', '$userid')");
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
    }
    elseif ($act=='update')
    {
        mysqli_query($cnmy, "UPDATE $dbname.t_kendaraan SET nopol='$kodenya', jenis='$pjenis', "
                . " merk='$pmerk', tipe='$ptipe', tglbeli='$pp01', userid='$userid' WHERE nopol='$idlama'");
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
    }
