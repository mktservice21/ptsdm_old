<?PHP
    ...
    if ($_GET['ket']=="excel") {
        ...
        header("Content-Disposition: attachment; filename=REALISASI BIAYA LUAR KOTA OTC.xls");
    }
...
<html>
<head>
    <title>REALISASI BIAYA LUAR KOTA OTC</title>
...
<?php
    ...
    $tmp04 =" dbtemp.DTBRROTCCOA04_".$_SESSION['IDCARD']."_$now ";
    $tmp05 =" dbtemp.DTBRROTCCOA05_".$_SESSION['IDCARD']."_$now ";
	...
	a.rptotal,
        ...
        a.rptotal as ca2,
        a.rptotal as jmltrans, b.nama_karyawan, b.atasan1, b.atasan2, b.atasan3, b.atasan4 
        FROM
        ...
        WHERE b.stsnonaktif <> 'Y' AND b.kode = 2 and b.divisi='OTC' $fperiode";
    ...
    $query = "select tgltrans, coa4, nama_coa4, icabangid_o icabangid, nama_cabang_o nama_cabang, areaid_o areaid, nama_area_o nama_area, karyawanid, nama_kry, divisi,
        nobrid, nama_des, deskripsi, tgl1, tgl2, notes, ca1, selisih, ca2, jmltrans, tgltransreal,
        nama_karyawan, atasan1, atasan2, atasan3, atasan4, 
        sum(jumlah) jumlah, sum(qty) qty, sum(rp) rp, sum(rptotal) rptotal
        from $tmp01 
    ...
    $query="select karyawanid, periode, jumlah, nama_karyawan, atasan1, atasan2, atasan3, atasan4 from dbmaster.t_ca0 where stsnonaktif <> 'Y' AND jenis_ca='lk' and divisi='OTC' AND "
    ...
    $query = "update $tmp03 set nama_kry=nama_karyawan WHERE karyawanid='0000002200'"; 
    mysqli_query($cnit, $query);
    ...
    //karyawan yang tidak ada di BL
    
    $query = "select b.periode, b.karyawanid, c.nama nama_kry, nama_karyawan, a.nourut, a.idca, a.nobrid,a.coa coa4, d.NAMA4 nama_coa4, b.divisi,
        a.qty, a.rp, a.rptotal, a.tgl1, a.tgl2, a.notes,
        b.atasan1, b.atasan2, b.atasan3, b.atasan4, e.nama nama_des,
	b.icabangid_o icabangid,
	g.nama nama_cabang,
	b.areaid_o areaid,
	h.nama nama_area, b.jumlah  
        from dbmaster.t_ca1 a 
        JOIN dbmaster.t_ca0 b on a.idca=b.idca
        JOIN hrd.karyawan c on b.karyawanid=c.karyawanId
        JOIN dbmaster.coa_level4 d on a.coa=d.COA4 
        JOIN dbmaster.t_brid e ON a.nobrid = e.nobrid
        LEFT JOIN MKT.icabang_o g ON b.icabangid_o = g.icabangid_o
        LEFT JOIN MKT.iarea_o h ON b.icabangid_o = h.icabangid_o
        AND b.areaid_o = h.areaid_o
        WHERE IFNULL(b.stsnonaktif,'')<>'Y' AND b.divisi='OTC' AND DATE_FORMAT(b.periode,'%Y-%m') between '$periode1' AND '$periode2' AND "
            . " (b.karyawanid not in (select distinct karyawanid from $tmp01) OR b.karyawanid='0000002200')";
    
    
    
    $query = "select b.periode, b.karyawanid, c.nama nama_kry, b.nama_karyawan, b.idca, b.divisi,
        b.atasan1, b.atasan2, b.atasan3, b.atasan4,
	b.icabangid_o icabangid,
	g.nama nama_cabang,
	b.areaid_o areaid,
	h.nama nama_area, b.jumlah  
        from dbmaster.t_ca0 b 
        JOIN hrd.karyawan c on b.karyawanid=c.karyawanId
        LEFT JOIN MKT.icabang_o g ON b.icabangid_o = g.icabangid_o
        LEFT JOIN MKT.iarea_o h ON b.icabangid_o = h.icabangid_o
        AND b.areaid_o = h.areaid_o
    ...
    $query = "update $tmp05 set nama_kry=nama_karyawan WHERE karyawanid='0000002200'"; 
    mysqli_query($cnit, $query);
    
    $query = "update $tmp05 a JOIN dbmaster.t_karyawan_kontrak b ON a.nama_karyawan=b.nama AND"
            . " a.atasan1=b.atasan1 AND a.atasan2=b.atasan2 AND a.atasan3=b.atasan3 AND a.atasan4=b.atasan4 "
            . " SET a.karyawanid=b.id WHERE a.karyawanid='0000002200'";
    mysqli_query($cnit, $query);
    
    $query = "delete from $tmp05 WHERE karyawanid IN (select distinct karyawanid from $tmp02)";
    mysqli_query($cnit, $query);
    
    $query = "INSERT INTO $tmp02 (icabangid, nama_cabang, areaid, nama_area, karyawanid, nama_kry, divisi "
            . " )"
            . " select distinct icabangid, nama_cabang, areaid, nama_area, "
            . " karyawanid, nama_kry, divisi FROM $tmp05";
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan;  }
    
    
    //END karyawan yang tidak ada di BL
    ...
    //karyawan yang tidak ada di BL
    $query = "update $tmp02 set ca1=ifnull((select sum(jumlah) from $tmp05 where $tmp05.karyawanid=$tmp02.karyawanid AND DATE_FORMAT($tmp05.periode,'%Y-%m')='$periode1'),0) WHERE IFNULL(ca1,0)=0"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    
    $query = "update $tmp02 set ca2=ifnull((select sum(jumlah) from $tmp05 where $tmp05.karyawanid=$tmp02.karyawanid AND DATE_FORMAT($tmp05.periode,'%Y-%m')='$periode2'),0) WHERE IFNULL(ca2,0)=0"; 
    mysqli_query($cnit, $query);
    $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
    ...
    <div id="kotakjudul">
        <div id="isikiri">
            <table class='tjudul' width='100%'>
                ...
                <tr><td width="210px"><b>Realisasi Biaya Luar Kota OTC Per </b></td><td><?PHP echo "$per1 "; ?></td></tr>
            </table>
        </div>
        <div id="isikanan">
        ...
        <table id='datatable2' class='table table-striped table-bordered example_2' border="1px solid black">
            ...
            <tbody>
                <?PHP
                    ...
                    $query = "select * from $tmp02 order by nama_kry, karyawanid";
                        ...
                        while ($reco <= $records) {
                            ...
                            while (($reco <= $records) AND $row['karyawanid']==$pkaryawanid) {
                                ...
                                if ($sudahlewat==false) {
                                    ...
                                    $pjumlahtrans=$row['ca2'];
                                    if ((double)$pselisih<0) {
                                        $pjumlahtrans=(0-(double)$pselisih)+(double)$row['ca2'];
                                    }
                                    if (empty($pjumlahtrans)) $pjumlahtrans=0;
                                    $totjumlahtrsf=(double)$totjumlahtrsf+(double)$pjumlahtrans;
