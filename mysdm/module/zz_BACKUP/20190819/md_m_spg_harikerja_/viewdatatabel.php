...
<form method='POST' action='<?PHP echo "?module=$_POST[module]&act=input&idmenu=$_POST[idmenu]"; ?>' id='demo-form2' name='form1' data-parsley-validate class='form-horizontal form-label-left'>
    ...
    <div class='x_content'>
        <b>Jumlah Hari Kerja : <?PHP echo $jmlkerja; ?> Hari</b>
            ...
            <tbody>
                <?PHP
                    ...
                    while ($sp= mysqli_fetch_array($tampil)) {
                        ...
                        $finjhksistem="<input type='hidden' size='5px' id='txtjkhsis$no' name='txtjkhsis$no' class='input-sm inputmaskrp2' autocomplete='off' value='$jmlkerja'>";
                        
                        
                        $fsimpan="'txtidspg$no', 'txtbulan$no', 'txtcabang$no', 'txtjmlhk$no', 'txtketerangan$no', 'txtjmlsakit$no', 'txtjmlizin$no', 'txtjmlalpa$no', 'txtjkhsis$no', 'txtjmluc$no'";
                            ...
                            for($xa=0;$xa<=$jmldata_ar;$xa++) {
                                if (isset($n_idarea[$xa]) AND isset($n_nmarea[$xa])) {
                                    if (!empty(trim($n_idarea[$xa])) AND !empty(trim($n_nmarea[$xa]))) {
                                        $n_sel_ar="";
                                        if ($n_idarea[$xa]==$pareaidspg) $n_sel_ar="selected";
                        ...
                        //END AREA
                        $fsimpan_dataspg="'txtidspg$no', 'txtcabang$no', 'cb_areaid$no', 'txtpenempatan$no'";
                ...
                <div class='x_content'>
                    <div class='col-md-12 col-sm-12 col-xs-12'>
                        ...
                        <div class='form-group'>
                            ...
                            <div class='col-xs-9'>
                                <div class="checkbox">
                                    <button type='button' class='btn btn-danger' onclick='ValidasiDataInput()'>Validate</button>
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    function SimpanData(eact, aidspg, abulan, acabang, aharikerja, aketerangan, asakit, aizin, aalpa, ahksistem, auc)  {
    ...
    function SimpanDataAreaPenempatan(eact, aidspg, acabang, aarea, apenempatan)  {
        var espg =document.getElementById(aidspg).value;
        var ecabang =document.getElementById(acabang).value;
        ...
        if (ok_) {
            ...
            if (r==true) {
                ...
                $.ajax({
                    ...
                    data:"uidspg="+espg+"&ucabang="+ecabang+"&uarea="+earea+"&upenempatan="+epenempatan,
                    success:function(data){
                        if (data.length > 1) {
                            alert(data);
                        }
