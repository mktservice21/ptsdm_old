<?php
    ...
    if (isset($_GET['iprint'])) {
        if ($_GET['iprint']=="print") {
            include "pritnbrrutin.php";
        }elseif ($_GET['iprint']=="lihatgambar") {
            include "module/mod_br_brrutin/lihatgambar.php";
        }elseif ($_GET['iprint']=="lihatkunjungan") {
            include "module/mod_br_brrutin/lihatkunjungan.php";
        }elseif ($_GET['iprint']=="bukti") {
			
			
            include "config/koneksimysqli.php";
            $query = "select gambar from dbimages.img_brrutin1 where idrutin='$_GET[brid]' LIMIT 1";
            $tampil= mysqli_query($cnmy, $query);
            $i= mysqli_fetch_array($tampil);
            $gambar=$i['gambar'];
            if (!empty($gambar) OR $_GET['brid']=="BRT0000997" OR $_GET['brid']=="BRT0001158") {
                include "module/mod_br_brrutin/lihatbukti2.php";
            }else{
                if ($_SESSION['MOBILE']=="Y") {
                    include "module/mod_br_brrutin/lihatbukti2.php";
                }else{
                    include "module/mod_br_brrutin/lihatbukti.php";
                }
            }
    ...
    if ($_GET['ket']=="excel") {
        ...
        header("Content-Disposition: attachment; filename=Data Biaya Rutin $printdate $jamnow.xls");
    }
...
<html>
    ...
    <body>
        <?PHP
            ...
            $now=date("mdYhis");
            $tmp01 =" dbtemp.DTBRARRUT01_$_SESSION[IDCARD]$now ";
            $tmp02 =" dbtemp.DTBRARRUT02_$_SESSION[IDCARD]$now ";
        ...
        ?>
        <div id="div1">
            <center><h2><u>DATA BIAYA RUTIN</u></h2></center>
                    ...
                    while ($reco <= $records) {
                        ...
                        while ( ($reco<=$records) and ($periode == $row['bulan'])) {
                            ...
                            if ($records2) {
                                ...
                                echo "<tr>";
                                echo "<td align='right' colspan=5>Rp.</td>";
