...
//$aaa = " '', -- Pilihan --, '', Y, $_SESSION[STSADMIN], $fildiv, $_SESSION[LVLPOSISI], $tampilbawahan, $_SESSION[IDCARD], $jabatan_, $_SESSION[AKSES_REGION], $filkaryawncabang, '', $hanyasatukaryawan ";
//echo $aaa;exit;
...
if (isset($_GET['ca'])) {
    $hanyasatukaryawan = "";
    $idca = $_GET['ca'];
    $edit = mysqli_query($cnmy, "SELECT * FROM dbmaster.v_ca0 WHERE idca='$idca' AND stsnonaktif <> 'Y'");
    ...
    $rutin_jumlahrp = getfield("select sum(jumlah) as lcfields from dbmaster.t_brrutin0 WHERE idca='$idca' AND stsnonaktif <> 'Y'");
    ...
    if ($_GET['act']=="editdata"){
        ...
        $edit = mysqli_query($cnmy, "SELECT * FROM dbmaster.v_brrutin0 WHERE kode=1 AND idrutin='$_GET[id]' $filygmengajukan");
        $r    = mysqli_fetch_array($edit);
        $idklaim=$r['idrutin'];
        ...
        $papprovefin=$r['tgl_fin'];
        $pgbratasan1=$r['gbr_atasan1'];
        $pgbratasan2=$r['gbr_atasan2'];
        $pgbratasan3=$r['gbr_atasan3'];
        $pgbratasan4=$r['gbr_atasan4'];
        ...
        if ($lvlpengajuan=="FF1" OR $lvlpengajuan=="FF2" OR $lvlpengajuan=="FF3" OR $lvlpengajuan=="FF4") {
            if (!empty($pgbratasan1)) $sudahapv="sudah";
            if (!empty($pgbratasan2)) $sudahapv="sudah";
            if (!empty($pgbratasan3)) $sudahapv="sudah";
            if (!empty($pgbratasan4)) $sudahapv="sudah";
        }
        
        
        if ($_SESSION['DIVISI']!="OTC") {
            if (!empty($papprovefin)) $sudahapv="sudah";
        }
		
		/*
        if ($lvlpengajuan=="FF1") {
            
            $cariapvff1 = getfield("select karyawanid as lcfields from dbmaster.t_karyawan_apv where karyawanid='$patasan1' and status='SPV'");
            ...
            if (empty($patasan1) AND empty($t_ats2) AND empty($t_ats3)) $sudahapv="";
		...
		*/
        ...
        if (!empty($r['idca'])) {
            ...
            $rutin_jumlahrp = getfield("select sum(jumlah) as lcfields from dbmaster.t_brrutin0 WHERE idca='$idca' AND idrutin <> '$idklaim' AND stsnonaktif <> 'Y'");
            $sewa_jumlahrp = getfield("select sum(jumlah) as lcfields from dbmaster.t_sewa WHERE idca='$idca' AND stsnonaktif <> 'Y'");
            if (empty($rutin_jumlahrp)) $rutin_jumlahrp = 0;
            if (empty($sewa_jumlahrp)) $sewa_jumlahrp = 0;
            $saldoca = (double)$ca_jumlahrp-(double)$rutin_jumlahrp-(double)$sewa_jumlahrp;
        ...
        if ($divisi=="OTC") {
            $edittambah = mysqli_query($cnmy, "SELECT * FROM dbmaster.t_brrutin2 WHERE idrutin='$idklaim'");
...
$hanyakaryawanaktif_f="Y";
if ($_SESSION['GROUP']=="1" OR $_SESSION['GROUP']=="28") $hanyakaryawanaktif_f="";
                                        ...
                                        <select class='form-control input-sm' id='e_atasan' name='e_atasan' onchange="ShowAtasanDM('e_atasan')">
                                            ...
                                            if ($_SESSION['GROUP']==33 OR $_SESSION['GROUP']==32 OR $_SESSION['GROUP']==27) {
                                                ...
                                                if ($_GET['act']=="editdata"){
													...
													$query = "select karyawanId, nama from hrd.karyawan where karyawanId='$patasan1'";
                                                    ...
                                                    echo "<option value='' selected>blank_</option>";
                                                    while($a=mysqli_fetch_array($tampil)){  
                                                        if (($a['karyawanId']==$patasan1) AND !empty($patasan1)){
                                    ...
                                    <div class='col-xs-9'>
                                        ...
                                        if ($_SESSION['GROUP']==33 OR $_SESSION['GROUP']==32 OR $_SESSION['GROUP']==27) {
                                            ...
                                            if ($_GET['act']=="editdata"){
												...
												$query = "select karyawanId, nama from hrd.karyawan where karyawanId='$patasan2'";
                                                $query .=" order by nama, karyawanId";
                                                $tampil = mysqli_query($cnmy, $query);
                                                $ketemu = mysqli_num_rows($tampil);
                                    ...
                                    <div class='col-xs-9'>
                                        ...
                                        if ($_SESSION['GROUP']==33 OR $_SESSION['GROUP']==32 OR $_SESSION['GROUP']==27) {
                                            ...
                                            if ($_GET['act']=="editdata"){
												...
												$query = "select karyawanId, nama from hrd.karyawan where karyawanId='$patasan3'";
                                                $query .=" order by nama, karyawanId";
                                                $tampil = mysqli_query($cnmy, $query);
                                                $ketemu = mysqli_num_rows($tampil);
    ...
    function TampilkanDataKaryawan(){
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                CariAtasan('');
                var enopol =document.getElementById('e_nopol').value;
                if (enopol.length>0) {
                    //alert(enopol);
                    ikedaraan.style.display = 'block';
                }else{
                    ikedaraan.style.display = 'none';
                }
            }
        });
    }
...
<script>
    ...
    function showDataByAll(pkar, pjabatan, prank, plevel, pdivisi) {
        ...
        if (pdivisi=="OTC") {
            ...
            CariAtasan(''); return false; //baru cari atasan dari data karyawan_posisi
    ...
    function showCA(ecar, eperiode) {
        ...
        $.ajax({
            type:"post",
            url:"module/mod_br_entrybrcash/viewdatams.php?module=viewdatacashadvance",
    ...
    function showKodePeriode() {
        ...
        $.ajax({
            type:"post",
            url:"module/mod_br_entrybrcash/viewdatams.php?module=getkodeperiode",
            data:"ubulan="+ibulan,
            success:function(data){
                $("#e_periode").html(data);
            }
...
<script>
    ...
    function findTotal(){
        ...
        tot =parseInt(a1)+parseInt(a2)+parseInt(a3)+parseInt(a4)+parseInt(a5)+parseInt(a6)
            ...
            +parseInt(a12)+parseInt(a13)+parseInt(a14)+parseInt(a15)+parseInt(a16)+parseInt(a17)+parseInt(a18);
