...
<?PHP
    ...
    $query = "select * from dbmaster.t_brrutin_ca_close WHERE DATE_FORMAT(bulan,'%Y-%m')='$periode1' $filterstsclose";
    ...
    if ($ketemu==0){
        ...
        $query = "create temporary table $tmp04 (idrutin VARCHAR(10), ket VARCHAR(3))";
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }

        $query = "select DISTINCT idrutin, 'LK1' ket from dbmaster.t_brrutin_ca_close WHERE ifnull(idrutin,'') <> '' AND DATE_FORMAT(bulan,'%Y-%m')='$periode1' $filterstsclose";
        $query .= " UNION select idca1, 'CA1' ket from dbmaster.t_brrutin_ca_close WHERE ifnull(idca1,'') <> '' AND DATE_FORMAT(bulan,'%Y-%m')='$periode1' $filterstsclose";
        $query .= " UNION select idca2, 'CA2' ket from dbmaster.t_brrutin_ca_close WHERE ifnull(idca2,'') <> '' AND DATE_FORMAT(bulan,'%Y-%m')='$periode1' $filterstsclose";
        $query = "INSERT INTO $tmp04 (idrutin, ket) $query"; 
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }


        $query="select '' as nourut, a.divisi, a.idrutin, a.karyawanid, a.keterangan, a.jumlah, a.jumlah totalrutin, a.idrutin idca1, 
            a.jumlah ca1, a.idrutin idca2, a.jumlah ca2 from dbmaster.t_brrutin0 a 
        ...
        $query="select a.divisi, a.idca, a.karyawanid, a.periode, a.keterangan, a.jumlah from dbmaster.t_ca0 a where a.stsnonaktif <> 'Y' AND "
                . " a.jenis_ca='lk' and a.divisi<>'OTC' AND "
        ...
        if ($stsreport=="B") {
            mysqli_query($cnit, "DELETE FROM $tmp01 WHERE idrutin IN (select distinct ifnull(idrutin,'') FROM $tmp04 WHERE ket='LK1')");
            mysqli_query($cnit, "DELETE FROM $tmp02 WHERE idca IN (select distinct ifnull(idrutin,'') FROM $tmp04 WHERE ket <> 'LK1')");
        }elseif ($stsreport=="C" OR $stsreport=="S") {
            mysqli_query($cnit, "DELETE FROM $tmp01 WHERE idrutin NOT IN (select distinct ifnull(idrutin,'') FROM $tmp04 WHERE ket='LK1')");
            mysqli_query($cnit, "DELETE FROM $tmp02 WHERE idca NOT IN (select distinct ifnull(idrutin,'') FROM $tmp04 WHERE ket <> 'LK1')");
        }
        ...
        $query = "INSERT INTO $tmp03 (karyawanid, divisi)"
                . " select DISTINCT a.karyawanid, a.divisi FROM $tmp02 as a WHERE "
                . " a.karyawanid not in (select distinct IFNULL(karyawanid,'') from $tmp01)"; 
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }



        $query = "ALTER TABLE $tmp03 ADD nama_karyawan VARCHAR(200), ADD jumlah DECIMAL(20,2), ADD totalrutin DECIMAL(20,2), ADD idca1 VARCHAR(10), ADD idca2 VARCHAR(10),"
                . " ADD ca1 DECIMAL(20,2), ADD ca2 DECIMAL(20,2), ADD nourut VARCHAR(1)";
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }


        mysqli_query($cnit, "UPDATE $tmp03 a SET a.nama_karyawan=(select b.nama from hrd.karyawan b WHERE a.karyawanid=b.karyawanId)");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }

        mysqli_query($cnit, "UPDATE $tmp03 a SET a.jumlah=IFNULL((select SUM(b.jumlah) jumlah from $tmp01 b WHERE a.karyawanid=b.karyawanid AND a.idrutin=b.idrutin),0)");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
        mysqli_query($cnit, "UPDATE $tmp03 a SET a.totalrutin=IFNULL((select SUM(b.jumlah) jumlah from $tmp01 b WHERE a.karyawanid=b.karyawanid),0)");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }

        $nfilterkhusus = "";
        if ($periode1=="2019-01") $nfilterkhusus=" WHERE a.karyawanid <> '0000001755' ";
        mysqli_query($cnit, "UPDATE $tmp03 a SET a.idca1=IFNULL((select idca from $tmp02 b WHERE a.karyawanid=b.karyawanid AND DATE_FORMAT(b.periode,'%Y-%m')='$periode1' LIMIT 1),'') $nfilterkhusus");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }

        mysqli_query($cnit, "UPDATE $tmp03 a SET a.idca2=IFNULL((select idca from $tmp02 b WHERE a.karyawanid=b.karyawanid AND DATE_FORMAT(b.periode,'%Y-%m')='$periode2' LIMIT 1),'') $nfilterkhusus");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }

        //khusus venti
        if ($periode1=="2019-01") {
            $query = "update $tmp03 set idca1='CA00000333', idca2='CA00000333' WHERE idrutin in ('BLK0002024')";
            mysqli_query($cnit, $query);
            $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }

            $query = "update $tmp03 set idca1='CA00000319', idca2='CA00000319' WHERE idrutin in ('BLK0001448', 'BLK0001467')";
            mysqli_query($cnit, $query);
            $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
        }

        mysqli_query($cnit, "UPDATE $tmp03 a SET a.ca1=IFNULL((select SUM(b.jumlah) jumlah from $tmp02 b WHERE b.idca=a.idca1),0)");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }

        mysqli_query($cnit, "UPDATE $tmp03 a SET a.ca2=IFNULL((select SUM(b.jumlah) jumlah from $tmp02 b WHERE b.idca=a.idca2),0)");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }

        //khusus venti
        if ($periode1=="2019-01") {
            mysqli_query($cnit, "UPDATE $tmp03 a SET a.ca1=IFNULL((select SUM(b.jumlah) jumlah from $tmp02 b WHERE b.karyawanid=a.karyawanid AND DATE_FORMAT(b.periode,'%Y-%m')='$periode1'),0) WHERE karyawanid='0000001755'");
            $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
        }


        $query = "select idrutin from $tmp04";
        $ketemucls= mysqli_num_rows(mysqli_query($cnit, $query));
        if ($ketemucls>0){

            //cari salah satu yang sudah closing
            $query = "select idrutin from $tmp04 WHERE idrutin in (select distinct IFNULL(idrutin,'') FROM $tmp03 WHERE IFNULL(idrutin,'') <> '')";
            $ketemuslhsatu= mysqli_num_rows(mysqli_query($cnit, $query));
            if ($ketemuslhsatu==0){
                mysqli_query($cnit, "UPDATE $tmp03 set ca1=0, ca2=0, idca1='', idca2=''");
                $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
            }
            ...
            $query = "select tgltrans from dbmaster.t_brrutin0 where 
                idrutin in (select distinct IFNULL(idrutin,'') from $tmp03) AND 
        ...
        mysqli_query($cnit, "DELETE FROM $tmp03 WHERE IFNULL(jumlah,0)=0 AND IFNULL(ca1,0)=0 AND IFNULL(ca2,0)=0 AND IFNULL(totalrutin,0)=0");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }


        mysqli_query($cnit, "ALTER TABLE $tmp03 CHANGE nourut nourut INT(10)AUTO_INCREMENT PRIMARY KEY;");
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }


        mysqli_query($cnit, "drop temporary table $tmp01");

        $query = "create TEMPORARY table $tmp01 (select * from $tmp03)"; 
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
        ...
        $query = "select a.divisi, a.idrutin, a.karyawanid, b.nama as nama_karyawan, a.credit jumlah, 
            a.saldo totalrutin, a.idca1, a.idca2, a.ca1, a.ca2, a.nourut, '' keterangan
            from $tmp05 a JOIN hrd.karyawan b on a.karyawanid=b.karyawanId ";
        $query = "create TEMPORARY table $tmp01 ($query)"; 
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
...
<?PHP
    mysqli_query($cnit, "DELETE FROM dbmaster.tmp_lk_closing WHERE idsession='$_SESSION[IDSESI]'");
        ...
        $query = "INSERT INTO dbmaster.tmp_lk_closing (nourut, tglinput, bulan, idrutin, idca1, idca2, userid, idsession, jumlah, totalrutin, ca1, ca2, karyawanid, divisi, sts) "
                ...
                . " '$_SESSION[IDSESI]', jumlah, totalrutin, ca1, ca2, karyawanid, divisi, '$stspilihan' sts FROM $tmp01";
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; exit; }
