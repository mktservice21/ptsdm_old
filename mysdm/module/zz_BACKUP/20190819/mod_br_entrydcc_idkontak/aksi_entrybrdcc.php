...
// Hapus
if ($module=='entrybrdcc' AND $act=='hapus')
{
...
elseif ($module=='entrybrdcc' AND ($act=='editterima' OR $act=='edittransfer' OR $act=='input' OR $act=='update'))
{
    ...
    $piddokter=$_POST['e_iddokter'];
    $pdokter=$_POST['e_iddokter'];
    ...
    if (!empty($prprealisasi)) $prprealisasi = str_replace("'", " ", $prprealisasi);
    $prprealisasi2=$_POST['e_realisasi2'];
    if (!empty($prprealisasi2)) $prprealisasi2 = str_replace("'", " ", $prprealisasi2);
    ...
    if (trim($prprealisasi)==trim($prprealisasi2)){
        ...
        if (trim($prekreal1)==trim($prekreal12)){
            ...
            //bank
            //$query="insert into $dbname2.t_kontak_bank_eth(idkontak, bank, bankcab, bankrek)values('$prprealisasikd', '$pbankreal1', '$pcabreal1', '$prekreal1')";
            //mysqli_query($cnmy, $query);
            //$pkdbankreal = getfieldcnit("select MAX(id) as lcfields from $dbname2.t_kontak_bank_eth");
        }
    }else{
        
        $query="insert into $dbname2.t_kontak_realisasi_eth(nama)values('$prprealisasi')";
        mysqli_query($cnmy, $query);
        $prprealisasikd = getfieldcnit("select MAX(idkontak) as lcfields from $dbname2.t_kontak_realisasi_eth");
    ...
    if ($act=='input') {
        ...
        $query="insert into $dbname.br0 (brId, tgl, karyawanid, kode, dokterid, aktivitas1, aktivitas2, jumlah, icabangid, divprodid)values"
                . "('$kodenya', '$ptglinput', '$pkaryawan', '$pkode', '$piddokter', '$paktivitas1', '$paktivitas2', '$prpnya', '$pidcabang', '$pdivprodid')";
        mysqli_query($cnmy, $query);
    ...
    if ($nama_kd == 'DCC') {
        ...
        $query_ar = "select areaId from hrd.karyawan where karyawanId='$pmr'";
    }
    ...
    $query = "update $dbname.br0 set areaid='$areaid' where brId='$kodenya' ";
    mysqli_query($cnmy, $query);
    
    //$query = "update $dbname.br0 set trmskid='$trmskid', tglacc='$periode3' where brId='$kodenya' ";
    //mysqli_query($cnmy, $query);
    
    //$query = "update $dbname.br0 set dokter='$pdokter' where brId='$kodenya' ";
    //mysqli_query($cnmy, $query);
    ...
    //cari daerah
    $pcabangytd=  getfieldcnit("select distinct idcabang as lcfields from dbmaster.cabangytd where icabangid='$pidcabang'");
    $pjabtanid="";
    $pcarijbt=$pkaryawan;
    if (!empty($pmr)) $pcarijbt=$pmr;
    
    $pjabatanid=  getfieldcnit("select distinct jabatanid as lcfields from hrd.karyawan where karyawanId='$pcarijbt'");
