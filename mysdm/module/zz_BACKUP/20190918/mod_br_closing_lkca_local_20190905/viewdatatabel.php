                ...
                <div class='x_content'>
                    <div class='col-md-12 col-sm-12 col-xs-12'>
                        ...
                        $query = "select idinput, nodivisi from dbmaster.t_suratdana_br WHERE IFNULL(stsnonaktif,'') <> 'Y' AND divisi <> 'OTC' 
                            AND CONCAT(kodeid,subkode)='221' AND sts='$stsreport' and DATE_FORMAT(tglf,'%Y%m')='$bulan'";
...
<script>
    ...
    function disp_confirm(pText_, eact)  {
        ...
        //alert(eiddanabank); return false;
        ...
        if (ok_) {
            ...
            if (r==true) {
                ...
                $.ajax({
                    ...
                    data:"unourut="+enourut+"&utgltrans="+etgltrans+"&unobukti="+enobukti+"&usudahpernah="+esudah+"&uact="+eact+"&unodivbr="+enodivbr+"&uiddanabank="+eiddanabank+"&usaldoreal="+esaldoreal,
                    success:function(data){
                        $("#loading2").html("");
                        alert(data);
                        HideHitungSave();
