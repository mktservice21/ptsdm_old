...
if ($_GET['module']=="viewdatabrinput"){
    ...
    */
    ...
    if ($pact=="editdata") {
        ...
        $query = "select a.*, b.urutan, b.bridinput, b.trans_ke from $tmp03 a LEFT JOIN $tmp04 b on a.brId=b.bridinput";
        ...
        $query = "select *, CAST(null as DECIMAL(10,0)) as urutan, CAST('' as CHAR(20)) as bridinput, CAST('' as CHAR(2)) as trans_ke from $tmp03";
        $query = "create TEMPORARY table $tmp05 ($query)"; 
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        ...
        <table id='datatable' class='datatable table nowrap table-striped table-bordered' width="100%">
            ...
            <?PHP
                ...
                while ($row= mysqli_fetch_array($tampil)) {
                    ...
                    $pnmselecturut="<select id='cb_urut[$pbrid]' name='cb_urut[$pbrid]' onChange=\"cekBoxDataBR('$pbrid', 'cb_urut[$pbrid]', 'chk_jml1[$pbrid]')\">$purut_opt</select>";
                    
                    $chkbox_bca = "<input type='checkbox' id='chk_transke[$pbrid]' name='chk_transke[$pbrid]' value='NB' $ncheck_trans>";//NB = NON BCA
                    
                    $chkbox = "<input type='checkbox' id='chk_jml1[$pbrid]' name='chk_jml1[]' value='$pbrid' onclick=\"CentangCekBoxDataBR('$pbrid', 'cb_urut[$pbrid]', 'chk_jml1[$pbrid]')\" $ncheck_sudah>";
    ...
    <script type="text/javascript">
        function cekBoxDataBR(nidbr, nmcb, nmchk) {
            ...
            BukaTutupTabelInput(nidbr, nmchk, ecb_br);
            //alert(nmcb+", ada, "+nmchk);
        }
        
        function CentangCekBoxDataBR(nidbr, nmcb, nmchk) {
            HitungTotalDariCekBox();
        ...
        function BukaTutupTabelInput(nidbr, nmchk, nurutan) {
            var nm_r = "tb_"+nidbr;
