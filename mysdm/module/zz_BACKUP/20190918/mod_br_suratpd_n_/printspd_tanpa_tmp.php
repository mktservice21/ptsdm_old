    ...
    <body>
        <div id="div1">
            <?PHP
                ...
                $query = "select tglspd from dbmaster.t_suratdana_br WHERE IFNULL(tglspd,'')<>'' AND stsnonaktif<>'Y' AND nomor='$nospd' LIMIT 1";
            ...
            <table id='example_2' class='table table-striped table-bordered example_2' width="100%" border="0px">
                ...
                <?PHP
                    ...
                    $query = "select distinct kodeid, nama from dbmaster.t_kode_spd "
                            . " WHERE kodeid in (select distinct kodeid from dbmaster.t_suratdana_br WHERE stsnonaktif<>'Y' AND nomor='$nospd')"
                            . "ORDER BY 1";
                    $result = mysqli_query($cnmy, $query);
                    WHILE ($r = mysqli_fetch_array($result)) {
                        ...
                        if ($pkode=="02") $pnama = "KLAIM - PETTY CASH 900 JUTA";
                        
                        $totalkodeid=0;
                        $query = "select sum(jumlah) jumlah from dbmaster.t_suratdana_br WHERE stsnonaktif<>'Y' AND "
