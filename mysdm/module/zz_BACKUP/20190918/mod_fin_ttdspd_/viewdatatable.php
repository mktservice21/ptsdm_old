    ...
    if ($pses_grpuser=="1" OR $pses_grpuser=="24" OR $pses_grpuser=="25") {
        if ($pses_grpuser=="25") {
            ...
            $n_filterkaryawan=" AND CONCAT(kodeid,subkode) NOT IN ('225', '226', '227', '228', '229', '230', '231', '232', '233', '234', '235', '350') "
                    . " AND (divisi<>'OTC' OR (divisi='OTC' AND CONCAT(kodeid, subkode) IN ('103', '221')) ) ";
        ...
        if ($pses_grpuser=="3" OR $pses_grpuser=="23" OR $pses_grpuser=="28") {
            ...
            if (strtoupper($cket)=="APPROVE") {
                if ($pses_grpuser=="25") {//anne
                    ...
                    $sql .=" AND ( (IFNULL(tgl_apv1,'')<>'' AND CONCAT(kodeid, subkode) NOT IN ('103', '221')) OR "
                            . " (IFNULL(tgl_apv1,'')='' AND CONCAT(kodeid, subkode) IN ('103', '221')) ) ";
                }else{
                }
                ...
                if ($pses_grpuser=="26") {//saiful
                    $sql.=" AND ( (IFNULL(tgl_apv2,'')<>'' AND CONCAT(kodeid, subkode) NOT IN ('103', '221')) OR "
                            . " (IFNULL(tgl_apv1,'')<>'' AND CONCAT(kodeid, subkode) IN ('103', '221')) )";
                }else{
                    $sql.=" AND IFNULL(tgl_apv2,'')<>'' ";
                }
                ...
                while ($row= mysqli_fetch_array($tampil)) {
                    ...
                    if (strtoupper($cket)=="APVDIRFIN") {
                        ...
                        if ($pses_grpuser=="3" OR $pses_grpuser=="23" OR $pses_grpuser=="28") {
                            ...
                            if ($pses_grpuser=="26" AND ( ($pkode=="1" AND $psubkode=="03") OR ($pkode=="2" AND $psubkode=="21") ) ) {//saiful untuk rutin dan lk ceker anne
                                if (!empty($napv2)) $cekbox="";
                            }
