 ...
 */
    ...
    $fidinbatal=$row["brOtcId"];
    $btnbatal="";
    if ($pbatal=="Y") {
        $fidinbatal="<span style='color:red;'><a href='#' title='$palasanbatal'>".$row["brOtcId"]."</a></span>";
    }else{
        $btnbatal="<input type='button' class='btn btn-warning btn-xs' value='Batal' onClick=\"ProsesData('batal', '$row[brOtcId]')\">";
    }
    if (empty($tipeisi)) 
        $kettipeisi = $no;
    else{
