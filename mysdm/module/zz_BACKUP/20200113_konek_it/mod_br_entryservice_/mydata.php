...
$sql = "SELECT idservice, nopol, DATE_FORMAT(tgl,'%d %M %Y') as tgl, DATE_FORMAT(tglservice,'%d/%m/%Y') as periode, "
        . " divisi, karyawanid, nama, areaid, nama_area, FORMAT(jumlah,0,'de_DE') as jumlah, keterangan, FORMAT(km,0,'de_DE') km "
...
if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
    $sql.=" AND ( idservice LIKE '%".$requestData['search']['value']."%' ";
    $sql.=" OR nama LIKE '%".$requestData['search']['value']."%' ";
    $sql.=" OR nopol LIKE '%".$requestData['search']['value']."%' ";
    $sql.=" OR nama_area LIKE '%".$requestData['search']['value']."%' ";
    $sql.=" OR DATE_FORMAT(tgl,'%d %M %Y') LIKE '%".$requestData['search']['value']."%' ";
    $sql.=" OR DATE_FORMAT(tglservice,'%d %M %Y') LIKE '%".$requestData['search']['value']."%' ";
