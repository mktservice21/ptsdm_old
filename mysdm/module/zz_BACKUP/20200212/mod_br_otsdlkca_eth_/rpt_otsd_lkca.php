<?PHP
    ...
    if ($ppilihrpt=="excel") {
        ...
        header("Content-Disposition: attachment; filename=REPORT LK CA.xls");
    }
    ...
    $query = "select a.bulan, a.karyawanid, b.nama nama_karyawan, a.divisi, a.idrutin, a.idca1, a.idca2, a.credit saldo, a.saldo jumlah, a.ca1, a.selisih, a.ca2, a.jmltrans 
        from dbmaster.t_brrutin_ca_close a JOIN hrd.karyawan b on a.karyawanid=b.karyawanId 
        WHERE DATE_FORMAT(a.bulan,'%Y-%m')='$periode1'";
    $query = "create TEMPORARY table $tmp01 ($query)"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
    
    $query="select * from dbmaster.t_brrutin1 WHERE idrutin IN (select distinct IFNULL(idrutin,'') FROM $tmp01)";
    ...
    $query = "select a.bulan, a.karyawanid, a.nama_karyawan, a.divisi, a.idrutin, a.idca1 idca, a.idca2, a.jumlah, a.saldo, 
        a.ca1, a.selisih, a.ca2, a.jmltrans,
    ...
    $query = "UPDATE $tmp04 a SET a.jml_ots=(select sum(b.kembali_rp) FROM $tmp03 b WHERE a.karyawanid=b.karyawanid AND coa='$pcoa_1')"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }

    $query = "UPDATE $tmp04 a SET a.jml_adj=(select sum(b.kembali_rp) FROM $tmp03 b WHERE a.karyawanid=b.karyawanid AND coa='$pcoa_2')"; 
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
