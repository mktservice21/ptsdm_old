...
if ($_GET['module']=="viewdatabrinput"){
    ...
    */
    ...
    $query = "select distinct a.bridinput from dbmaster.t_suratdana_br1 a JOIN dbmaster.t_suratdana_br b "
            . " on a.idinput=b.idinput WHERE b.stsnonaktif<>'Y' AND a.kodeinput IN ('A', 'B', 'C') $filter_kwitasnsi $nfilterid_sudah";
    ...
    if ($pact=="editdata") {
        ...
        $query = "select a.*, b.urutan, b.bridinput, b.trans_ke, b.jml_adj, b.aktivitas1 ketadj1 from $tmp03 a LEFT JOIN $tmp04 b on a.brId=b.bridinput";
        ...
        $query = "select *, CAST(null as DECIMAL(10,0)) as urutan, CAST('' as CHAR(20)) as bridinput, CAST('' as CHAR(2)) as trans_ke, CAST(null as DECIMAL(20,2)) jml_adj, CAST('' as CHAR(10)) as ketadj1 from $tmp03";
        $query = "create TEMPORARY table $tmp05 ($query)"; 
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        ...
        <table id='datatable' class='datatable table nowrap table-striped table-bordered' width="100%">
            ...
            <?PHP
                ...
                while ($row= mysqli_fetch_array($tampil)) {
                    ...
                    $chkbox = "<input type='checkbox' id='chk_jml1[$pbrid]' name='chk_jml1[]' value='$pbrid' onclick=\"CentangCekBoxDataBR('$pbrid', 'cb_urut[$pbrid]', 'chk_jml1[$pbrid]', 'chk_adj[$pbrid]', 'txt_adj[$pbrid]', 'chk_transke[$pbrid]', 'txt_adj_ket[$pbrid]')\" $ncheck_sudah>";
