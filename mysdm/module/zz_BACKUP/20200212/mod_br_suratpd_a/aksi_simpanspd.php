...
elseif ($module=='suratpd')
{
    ...
    if ($pkode=="3" OR $pkode=="7" OR ($pkode=="2" AND $psubkode=="29x") ) {
        ...
        if ($pkode=="3") {
            ...
            $query = "UPDATE $dbname.t_suratdana_br SET $setupdatetgl kodeid2=$nnkodeid2, subkode2='$nnsubkode2', divisi2='$nndivisi2', bulan2='$periode1', nomor='$pnomor2', nomor2='$pnomor2', nodivisi2='$pnodivisi2', userproses='$userid', tgl_proses=NOW() $t_upadjtgl WHERE idinput='$kodenya'";
        }else{
    ...
    if ($pkode=="3" AND $act=="input") {
        ...
        $query = "UPDATE $dbname.t_suratdana_br SET nomor='', tglspd=null, userproses=null, tgl_proses=null WHERE idinput='$kodenya'";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
        
        $query = "UPDATE $dbname.t_suratdana_br SET nodivisi=nodivisi2 WHERE idinput='$kodenya' AND IFNULL(nodivisi,'')='' AND IFNULL(nodivisi2,'')<>''";
        mysqli_query($cnmy, $query);
        $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
