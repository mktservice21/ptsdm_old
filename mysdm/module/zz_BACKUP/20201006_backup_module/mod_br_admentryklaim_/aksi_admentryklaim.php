...
elseif ($module=='bgtadmentrybrklaim')
{
    ...
    foreach ($_POST['chk_kodeid'] as $no_brid) {
        ...
        if ((DOUBLE)$ptotkuranglebih==0 AND (DOUBLE)$pdet_rpklaim==0 AND (DOUBLE)$pdet_rpreal==0 AND (DOUBLE)$pdet_rptolakan==0 AND empty($pdet_notes)) {
        }else{
            $pbolehsimpan=true;
        }
    }
    ...
    foreach ($_POST['chk_kodeid'] as $no_brid) {
        ...
        if ((DOUBLE)$pdet_rpklaim==0 AND (DOUBLE)$pdet_rpreal==0 AND (DOUBLE)$pdet_rptolakan==0 AND empty($pdet_notes)) {
        }else{
            ...
            $pinsert_data_detail[] = "('$kodenya', '$no_brid', '$pdet_nmcabang', '$pdet_rpklaim', '$pdet_rpreal', '$pdet_rptolakan', '$pdet_notes')";
    ...
    if ($pbolehsimpan==true) {
        ...
        if ($act=='input') {
            ...
            $query="insert into $dbname.klaim (klaimId, tgl, karyawanid, distid, pengajuan, "
                    ...
                    . " region, bulan, periode1, periode2, jmlkuranglebih, ketkuranglebih)values"
                    ...
                    . " '$pregion', '$pbulan', '$pperiode1', '$pperiode2', '$ptotkuranglebih', '$pketkurleb')";
            mysqli_query($cnmy, $query); $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; mysqli_close($cnmy); exit; }
        }
        ...
        if ($padaqueryinsert==true) {
            $query_detail="INSERT INTO $dbname.klaim_d (klaimId, idcab, nm_cab, jumlah1, jumlah2, jumlah3, notes) VALUES ".implode(', ', $pinsert_data_detail);
