<?PHP
   ...
   */
    ...
    $query = "INSERT INTO $tmp03 (idrutin, idots, tgl_kembali, kembali_rp, divisi, karyawanid, bulan, "
            . " coa, NAMA4, nobukti, keterangan, kodeid, subkode, nomor, nodivisi)"
            . " select a.idrutin, a.idots, a.tgl_kembali, a.kembali_rp, a.divisi, a.karyawanid, a.bulan, "
            . " a.coa, a.NAMA4, a.nobukti, a.keterangan, b.kodeid, b.subkode, b.nomor, b.nodivisi FROM "
    ...
    $query = "UPDATE $tmp04 a SET a.kredit=(select sum(b.jumlah) FROM $tmp01 b WHERE b.stsinput IN ('K', 'T') AND a.nomor=b.nomor AND a.nodivisi=b.nodivisi AND a.idinput=b.idinput)";
    mysqli_query($cnmy, $query);
    $erropesan = mysqli_error($cnmy); if (!empty($erropesan)) { echo $erropesan; exit; }
