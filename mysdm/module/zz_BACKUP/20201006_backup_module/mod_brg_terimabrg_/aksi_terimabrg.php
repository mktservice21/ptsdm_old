...
if ($module=='gimicterimabarang')
{
    ...
    if ($act=="hapus") {
        ...
        if ($act=="input") {
            $sql=  mysqli_query($cnmy, "select max(IDTERIMA) as NOURUT from dbmaster.t_setup_barang WHERE TAHUN='$ptahun'");
        ...
        foreach ($_POST['chkbox_br'] as $pbarangid) {
            ...
            if ((DOUBLE)$pjml>0) {
                $pinsert_data_detail[] = "('$pkodeid', '$pbarangid', '$pjml')";
                $psimpandata=true;
            }
        ...
        if ($psimpandata == true) {
            ...
            $query_detail="INSERT INTO dbmaster.t_barang_terima_d (IDTERIMA, IDBARANG, JUMLAH) VALUES ".implode(', ', $pinsert_data_detail);
