...
<body>
	...
	$query = "select * from hrd.br0
        ...
        $pf_kodeid="04";
        //DSS (kode='700-01-04' or kode='700-02-04' or kode='700-04-04')
        $query = "INSERT INTO $tmp03 (tahun, kodeid, divisi, jumlah) 
                 select DATE_FORMAT($periodeby_br,'%Y') tahun, '$pf_kodeid' kodeid, divprodid, sum(jumlah) jumlah FROM 
                 $tmp04 WHERE 
                 kode in (select distinct kode_akun from dbmaster.t_budget_kode_d WHERE kodeid='$pf_kodeid')
                 GROUP BY 1,2,3";
        mysqli_query($cnit, $query);
        $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo $erropesan; goto hapusdata; }
        
        $pf_kodeid="05";
        //DCC (kode='700-01-03' or kode='700-02-03' or kode='700-04-03')
        $query = "INSERT INTO $tmp03 (tahun, kodeid, divisi, jumlah) 
                 select DATE_FORMAT($periodeby_br,'%Y') tahun, '$pf_kodeid' kodeid, divprodid, sum(jumlah) jumlah FROM 
                 $tmp04 WHERE 
                 kode in (select distinct kode_akun from dbmaster.t_budget_kode_d WHERE kodeid='$pf_kodeid')
        ...
        $pf_kodeid="06";
        //Gimmic GIMIK GIMMIC PROMOSI (kode='700-01-01' or kode='700-02-01' or kode='700-04-01')
        $query = "INSERT INTO $tmp03 (tahun, kodeid, divisi, jumlah) 
                 select DATE_FORMAT($periodeby_br,'%Y') tahun, '$pf_kodeid' kodeid, divprodid, sum(jumlah) jumlah FROM 
                 $tmp04 WHERE 
                 kode in (select distinct kode_akun from dbmaster.t_budget_kode_d WHERE kodeid='$pf_kodeid')
        ...
        $pf_kodeid="07";
        //IKLAN kode='700-01-06' or kode='700-04-06'
        $query = "INSERT INTO $tmp03 (tahun, kodeid, divisi, jumlah) 
                 select DATE_FORMAT($periodeby_br,'%Y') tahun, '$pf_kodeid' kodeid, divprodid, sum(jumlah) jumlah FROM 
                 $tmp04 WHERE 
                 kode in (select distinct kode_akun from dbmaster.t_budget_kode_d WHERE kodeid='$pf_kodeid')
        ...
        $pf_kodeid="08";
        //SIMPOSIUM (kode='700-01-08' or kode='700-02-05' OR kode='700-04-05')
        $query = "INSERT INTO $tmp03 (tahun, kodeid, divisi, jumlah) 
                 select DATE_FORMAT($periodeby_br,'%Y') tahun, '$pf_kodeid' kodeid, divprodid, sum(jumlah) jumlah FROM 
                 $tmp04 WHERE 
                 kode in (select distinct kode_akun from dbmaster.t_budget_kode_d WHERE kodeid='$pf_kodeid')
        ...
        $pf_kodeid="10";
        // HO (kode='700-01-099' or kode='700-02-099' or kode='700-04-099')
        $query = "INSERT INTO $tmp03 (tahun, kodeid, divisi, jumlah) 
                 select DATE_FORMAT($periodeby_br,'%Y') tahun, '$pf_kodeid' kodeid, divprodid, sum(jumlah) jumlah FROM 
                 $tmp04 WHERE 
                 kode in (select distinct kode_akun from dbmaster.t_budget_kode_d WHERE kodeid='$pf_kodeid')
