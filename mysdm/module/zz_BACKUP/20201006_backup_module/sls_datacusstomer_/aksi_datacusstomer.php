...
if ($pmodule=="simpanperubahancust") {
    ...
    if (!empty($pnid) AND !empty($pnstatus)) {
        ...
        $sql_update = "UPDATE MKT.icust SET istatus='$pnstatus' WHERE CONCAT(IFNULL(iCabangId,''),IFNULL(areaId,''),IFNULL(iCustid,''))='$pnid' LIMIT 1";
        mysqli_query($cnit, $sql_update); $erropesan = mysqli_error($cnit); if (!empty($erropesan)) { echo "A DB ".$erropesan; exit; }
        
        $sql_update_new = "UPDATE sls.icust SET istatus='$pnstatus' WHERE CONCAT(IFNULL(iCabangId,''),IFNULL(areaId,''),IFNULL(iCustid,''))='$pnid' LIMIT 1";
