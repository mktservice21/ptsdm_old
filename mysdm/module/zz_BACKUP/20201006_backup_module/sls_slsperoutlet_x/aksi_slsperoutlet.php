...
$query = "select * from sls.mr_sales2 WHERE DATE_FORMAT(tgljual,'%Y%m') BETWEEN '$pbln1' AND '$pbln2' "
...
$query = "select a.*, IFNULL(b.nama,'') as custnm, b.isektorid, c.nama as prodnm from $tmp01 a "
        . " LEFT JOIN sls.icust b on a.icabangid=b.icabangid AND a.icustid=b.icustid "
...
$query = "select tgljual, fakturid, initial, ecabangid, isektorid, icustid, custnm, iprodid, prodnm, sum(qty) qty, sum(hna*qty) as tvalue  
        from $tmp02 
    ...
    ?>
        ...
        <table id='datatable2' class='table table-striped table-bordered' width="100%" border="1px solid black">
            ...
            <tbody>
                ...
                $ptotvalue=0;
                $no=1;
                $query = "select distinct isektorid, icustid, custnm FROM $tmp03 order by custnm";
                ...
                while ($row1= mysqli_fetch_array($tampil1)) {
                    ...
                    echo "<td nowrap><b>$npnmcust</b></td>";
                    ...
                    $query = "select * FROM $tmp03 WHERE icustid='$npidcust' AND isektorid='$npisektor' order by custnm, prodnm, initial, ecabangid";
                ...
                while ($row1= mysqli_fetch_array($tampil1)) {
                    ...
                    */
                    ...
                    $query = "select icustid, custnm, iprodid, prodnm, sum(qty) qty, sum(tvalue) tvalue "
                            . " FROM $tmp03 WHERE icustid='$npidcust' AND isektorid='$npisektor' GROUP BY 1,2,3,4 order by custnm, prodnm";
                    $tampil2= mysqli_query($cnms, $query);
                    while ($row2= mysqli_fetch_array($tampil2)) {
