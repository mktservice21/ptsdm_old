...
<?php
    ...
    echo "<table width='90%' align='center' border='0' class='table table-striped table-bordered'>";
        echo "<tr>";
            echo "<td valign='top'>";
                ...
                echo "<tr><td><b>SALES YTD <u>$nminitialdist</u> CABANG & DIVISI BY VALUE</b></td></tr>";
    ...
    $now=date("mdYhis");
    $tmp0 ="DTSLSCABDIVDISTVDISTYTD00".$karyawanid."_$now$milliseconds";
    $tmp1 ="DTSLSCABDIVDISTVDISTYTD01".$karyawanid."_$now$milliseconds";
    $tmp2 ="DTSLSCABDIVDISTVDISTYTD02".$karyawanid."_$now$milliseconds";
    $tmp3 ="DTSLSCABDIVDISTVDISTYTD03".$karyawanid."_$now$milliseconds";
    ...
    $query = "select date_format(s.bulan,'%Y%m') thnbln, d.initial, s.idcbg icabangid, s.divprodid, SUM(s.qty) qty, SUM(s.qty*s.hna) /1000 as tvalue 
        from sls.ytd_dist s
    ...
    //echo "$fbln1 - $fbln2 dan $fblngw1 - $fblngw2"; exit;
    ...
    foreach ($results1 as $r1) {
        ...
        //sales skrang
        $query="update $tmp0 set $fsales1=ifnull((select sum(tvalue) from $tmp1 where $tmp0.icabangid=$tmp1.icabangid and $tmp1.divprodid='$fdivisi' and $tmp1.thnbln between '$fbln1' AND '$fbln2'),0)";
        $results1 = DB::query($query);
        //sales lalu
        $query="update $tmp0 set $fsales2=ifnull((select sum(tvalue) from $tmp1 where $tmp0.icabangid=$tmp1.icabangid and $tmp1.divprodid='$fdivisi' and $tmp1.thnbln between '$fblngw1' AND '$fblngw2'),0)";
        $results1 = DB::query($query);
        //target
    ...
    //sales skrang
    $query="update $tmp0 set $fsales1=ifnull((select sum(tvalue) from $tmp1 where $tmp0.icabangid=$tmp1.icabangid and $tmp1.thnbln between '$fbln1' AND '$fbln2'),0)";
    $results1 = DB::query($query);
    //sales lalu
    $query="update $tmp0 set $fsales2=ifnull((select sum(tvalue) from $tmp1 where $tmp0.icabangid=$tmp1.icabangid and $tmp1.thnbln between '$fblngw1' AND '$fblngw2'),0)";
    $results1 = DB::query($query);
    //target
...
<table id="datatable1" class="display  table table-striped table-bordered" style="width:100%" border='0px'>
    <thead>
        ...
        <tr>-->
            ...
            foreach ($results1 as $r1) {
                $fdivisi=$r1['divprodid'];
                echo "<th valign='top'><u>$fdivisi</u>&nbsp;<br/>Sales YTD $nminitialdist $thnbln</th>";
                echo "<th valign='top'><u>$fdivisi</u>&nbsp;<br/>Sales YTD $nmreg $thnbln</th>";
                echo "<th valign='top'><u>$fdivisi</u>&nbsp;<br/>Ctr %</th>";
                echo "<th valign='top'><u>$fdivisi</u>&nbsp;<br/>Sales YTD $nminitialdist $thnbln2</th>";
                echo "<th valign='top'><u>$fdivisi</u>&nbsp;<br/>Growth %</th>";
                
            }
            ?>
            <th valign='top'>Sales YTD <?PHP echo $nminitialdist." ".$thnbln; ?></th>
            <th valign='top'>Sales YTD <?PHP echo $nmreg." ".$thnbln; ?></th>
            <th valign='top'>Ctr %</th>
            <th valign='top'>Sales YTD <?PHP echo $nminitialdist." ".$thnbln2; ?></th>
