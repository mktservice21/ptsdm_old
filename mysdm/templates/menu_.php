...
if (isset($_GET['idmenu'])) {
    ...
    $query = "select DISTINCT PARENT_ID from dbmaster.sdm_menu WHERE ID='$pidsubmenu' AND IFNULL(KRITERIA,'')='Y'";
...
$query = "select a.ID id, a.ID_GROUP id_group,
	...
	b.URUTAN urutan
        ...
        WHERE b.PUBLISH='Y' AND b.PARENT_ID='0' AND a.ID_GROUP='$igru' AND IFNULL(b.KRITERIA,'')='Y' 
...
if ($ketemu>0){
    if ($_SESSION['UKHUSUS']=="Y") {
        ...
        $query2 = "select a.ID id, a.ID_GROUP id_group,
                ...
                from dbmaster.sdm_groupmenu a
                LEFT JOIN dbmaster.sdm_menu b on a.ID=b.ID
                WHERE b.PUBLISH='Y' AND a.ID_GROUP='$igru' AND b.M_KHUSUS='Y' AND IFNULL(b.KRITERIA,'')='Y' 
    ...
    while ($row= mysqli_fetch_array($tampil)) {
        ...
        <li><a><i class="fa <?PHP echo $gbr; ?>"></i> <?PHP echo "$row[judul]"; ?> <span class="fa fa-chevron-down"></span></a>
            ...
            $query3 = "select a.ID id, a.ID_GROUP id_group,
                    ...
                    from dbmaster.sdm_groupmenu a
                    LEFT JOIN dbmaster.sdm_menu b on a.ID=b.ID
                    WHERE b.PUBLISH='Y' AND a.ID_GROUP='$igru' AND b.PARENT_ID='$row[id]' $filparntnot  $pria $pmarsis  AND IFNULL(b.KRITERIA,'')='Y' 
                    ORDER BY b.URUTAN, b.ID
                    ";
...
?>
        ...
        <?PHP
        if ((int)$igru==1 OR (int)$igru==23 OR (int)$igru==26 OR (int)$igru==41){
        ?>
        ...
        <?PHP
        $igru=$_SESSION['GROUP'];
        if ((int)$igru==1 OR (int)$igru==22 OR ((int)$igru==28 AND $_SESSION['IDCARD']=="0000000329")){
        ?>
